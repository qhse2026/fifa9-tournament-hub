...
(() => {
  ...
  const VERSION = "4.0.0-phase9-authority.1";
  ...
  function buildConfig(career, fixture) {
    ...
    return {
      matchId: fixture.id,
      mode: "FULL_AUTHORITY",
      seed: `${career.id}|${fixture.id}|ME4-PHASE9`,
      home: {
        id: fixture.homeId,
  ...
  function publishOfficialEvent(legacy, snapshot, report) {
    ...
    const visibleTypes = new Set(["SHOT", "SHOT_BLOCKED", "SHOT_OFF_TARGET", "WOODWORK", "SAVE_HELD", "SAVE_PARRIED", "REBOUND", "GOAL", "CORNER_AWARDED", "SET_PIECE_ORGANIZED", "SET_PIECE_DELIVERY", "DIRECT_FREE_KICK_AWARDED", "INDIRECT_FREE_KICK_AWARDED", "THROW_IN_AWARDED", "PENALTY_AWARDED", "DIRECT_FREE_KICK_TAKEN", "PENALTY_TAKEN", "GOALKEEPER_CLAIM", "GOALKEEPER_PUNCH", "AERIAL_DUEL_WON", "SET_PIECE_CLEARED", "SET_PIECE_COUNTER_STARTED", "PRESS_TRIGGERED", "PRESS_BROKEN", "COUNTER_PRESS_REGAIN", "OFFSIDE", "CUTBACK_DEFENDED", "ATTACKING_ROLE_RUN", "KILLER_PASS_ATTEMPT", "HOLD_UP_SUCCESS", "HOLD_UP_LOST", "TACTICAL_CHANGE", "SUBSTITUTION", "EMERGENCY_ATTACK", "MANAGER_RECOMMENDATION", "FATIGUE_WARNING", "CRAMP", "KNOCK", "MUSCLE_TIGHTNESS", "MUSCLE_INJURY", "CONTACT_INJURY", "INJURY_SUBSTITUTION", "PLAYER_RECOVERED"]);
    ...
    const labels = {
      ...
      MUSCLE_INJURY: "KAS SAKATLIĞI", CONTACT_INJURY: "TEMAS SAKATLIĞI", INJURY_SUBSTITUTION: "SAKATLIK DEĞİŞİKLİĞİ", PLAYER_RECOVERED: "OYUNCU TOPARLANDI"
    };
    const item = {
      id: `me4p6-${event.id}`,
      source: "ME4_PHASE9",
  ...
  function emitSnapshot() {
    ...
    globalThis.dispatchEvent(new globalThis.CustomEvent("fifa-match-engine-v4-phase9", { detail: getSnapshot() }));
  ...
  function enable() {
    if (!runtime.enabled) {
      ...
      restartTimer();
      console.info(`[Match Engine V4] Phase 9 physical condition, fatigue and injury authority enabled · ${VERSION}`);
    }
    poll();
    return getSnapshot();
  }
...
(() => {
  ...
  const VERSION = "4.0.0-phase9-live.1";
  const STORAGE_KEY = "fifa-me4-phase9-physical-condition";
  ...
  const clamp = (value, min, max) => Math.max(min, Math.min(max, Number(value) || 0));
  const adapter = () => globalThis.FIFA_MATCH_ENGINE_V4_PHASE9 || globalThis.FIFA_MATCH_ENGINE_V4_PHASE8 || globalThis.FIFA_MATCH_ENGINE_V4_PHASE7 || globalThis.FIFA_MATCH_ENGINE_V4_SHADOW;
  const room = () => globalThis.FIFA_MANAGER_ROOM;
  ...
  function ensureHud(pitch, simulation, phase9Snapshot) {
    ...
    if (!hud) {
      ...
      hud.innerHTML = `<span>ME4 <b>PHYSICAL · MATCH LOAD AI</b></span><span data-me4-phase>—</span><span data-me4-ball class="me4-ball-state">BALL —</span><span data-me4-authority>AUTH —</span><span data-me4-pass>PASS —</span><span data-me4-rest>REST DEF —</span><span data-me4-defence>PRESS —</span><span data-me4-manager>MANAGER —</span><span data-me4-physical>PHYSICAL —</span><button type="button" data-me4-toggle class="active">V4 PHASE 9</button>`;
    ...
    if (toggleButton) {
      ...
      toggleButton.textContent = runtime.enabled ? "V4 PHASE 9" : "LEGACY GÖRÜNTÜ";
    }
  }
  ...
  function boot() {
    ...
    startLoop();
    applyFrame(true);
    console.info(`[Match Engine V4] Phase 9 physical condition, fatigue and injury authority ready · ${VERSION}`);
  }
