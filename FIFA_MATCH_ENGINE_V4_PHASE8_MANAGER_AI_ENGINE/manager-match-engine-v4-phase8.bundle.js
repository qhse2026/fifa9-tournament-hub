(() => {
  ...
  function createBench(config, side, rating, rng) {
    ...
    return templates.map((group, index) => {
      ...
      return {
        ...
        attributes: { ...normalizedAttributes(base, positionalAdjustments(group)), ...(row.attributes || {}) },
  ...
  function chooseSubstitution(engine, side, reasonHint = null) {
    const team = teamBySide(engine, side);
    const state = engine.managerStates[side];
    if (!team || !state || state.substitutionsUsed >= 5 || engine.clockSeconds - state.lastSubstitutionAt < 420) return null;
    ...
    const candidates = team.players.filter(player => player.group !== "GK" && player.id !== ownerId).map(player => {
      ...
      const fatigueNeed = Math.max(0, 73 - player.energy) * 1.8 + player.fatigue * .35;
      const tacticalNeed = reasonHint === "TACTICAL" && ["CM","AM","WING","ST"].includes(player.group) ? 18 : 0;
      return { player, score: fatigueNeed + cardRisk + tacticalNeed };
    }).sort((a,b)=>b.score-a.score);
    const outgoing = candidates[0];
    if (!outgoing || (outgoing.score < 18 && !reasonHint)) return null;
    const options = team.bench.filter(row=>!row.used).map(row=>({row, score:roleCompatibility(outgoing.player.group,row.group)+row.attributes.stamina*.16+row.attributes.decisions*.10})).sort((a,b)=>b.score-a.score);
    const incoming = options[0]?.row;
    if (!incoming || options[0].score < 42) return null;
    return { outgoing: outgoing.player, incoming, reason: outgoing.player.yellow ? "CARD_RISK" : outgoing.player.energy < 74 ? "FATIGUE" : (reasonHint || "TACTICAL") };
  }
  ...
  function makeSubstitution(engine, side, outgoingId, incomingId, reason = "MANAGER_SUBSTITUTION") {
    ...
    const previous = { name: outgoing.name, sourcePlayerId: outgoing.sourcePlayerId || outgoing.id, role: outgoing.role, attributes: { ...outgoing.attributes }, behaviour: { ...outgoing.behaviour } };
  ...
  function createEngine(config = {}) {
    ...
    const engine = {
      ...
      debugSetPlayerEnergy(playerId, energy) { const player=getPlayer(engine,playerId); if(!player) throw new Error("invalid player"); player.energy=clamp(Number(energy),35,100); player.fatigue=clamp((100-player.energy)/.92,0,100); return engine.getSnapshot(); },
      debugSetPlayerYellow(playerId, yellow = true) { const player=getPlayer(engine,playerId); if(!player) throw new Error("invalid player"); player.yellow=Boolean(yellow); return engine.getSnapshot(); },
...
(() => {
  ...
  const VERSION = "4.0.0-phase8-authority.1";
  ...
  function buildConfig(career, fixture) {
    ...
    return {
      matchId: fixture.id,
      mode: "FULL_AUTHORITY",
      seed: `${career.id}|${fixture.id}|ME4-PHASE8`,
      home: {
        id: fixture.homeId,
  ...
  function publishOfficialEvent(legacy, snapshot, report) {
    ...
    const visibleTypes = new Set(["SHOT", "SHOT_BLOCKED", "SHOT_OFF_TARGET", "WOODWORK", "SAVE_HELD", "SAVE_PARRIED", "REBOUND", "GOAL", "CORNER_AWARDED", "SET_PIECE_ORGANIZED", "SET_PIECE_DELIVERY", "DIRECT_FREE_KICK_AWARDED", "INDIRECT_FREE_KICK_AWARDED", "THROW_IN_AWARDED", "PENALTY_AWARDED", "DIRECT_FREE_KICK_TAKEN", "PENALTY_TAKEN", "GOALKEEPER_CLAIM", "GOALKEEPER_PUNCH", "AERIAL_DUEL_WON", "SET_PIECE_CLEARED", "SET_PIECE_COUNTER_STARTED", "PRESS_TRIGGERED", "PRESS_BROKEN", "COUNTER_PRESS_REGAIN", "OFFSIDE", "CUTBACK_DEFENDED", "ATTACKING_ROLE_RUN", "KILLER_PASS_ATTEMPT", "HOLD_UP_SUCCESS", "HOLD_UP_LOST", "TACTICAL_CHANGE", "SUBSTITUTION", "EMERGENCY_ATTACK", "MANAGER_RECOMMENDATION"]);
  ...
  function applyV4OfficialState(fixture) {
    ...
    legacy.matchEngineV4 = {
      ...
      contract: {
        players: "V4", passingBall: "V4", passControl: "V4", shots: "V4", goalkeeper: "V4", goals: "V4", score: "V4", setPieces: "V4", aerialDuels: "V4", defence: "V4", pressing: "V4", offsideLine: "V4", attackingIdentity: "V4", playerRoles: "V4", boxOccupation: "V4", managerAI: "V4", substitutions: "V4", matchManagement: "V4", managerAI: "V4", substitutions: "V4", matchManagement: "V4",
  ...
  function emitSnapshot() {
    ...
    globalThis.dispatchEvent(new globalThis.CustomEvent("fifa-match-engine-v4-phase8", { detail: getSnapshot() }));
  ...
  function enable() {
    if (!runtime.enabled) {
      ...
      restartTimer();
      console.info(`[Match Engine V4] Phase 8 manager AI and match management authority enabled · ${VERSION}`);
    }
    poll();
    return getSnapshot();
  }
...
(() => {
  ...
  const VERSION = "4.0.0-phase8-live.1";
  const STORAGE_KEY = "fifa-me4-phase8-manager-ai";
  ...
  const clamp = (value, min, max) => Math.max(min, Math.min(max, Number(value) || 0));
  const adapter = () => globalThis.FIFA_MATCH_ENGINE_V4_PHASE8 || globalThis.FIFA_MATCH_ENGINE_V4_PHASE7 || globalThis.FIFA_MATCH_ENGINE_V4_SHADOW;
  const room = () => globalThis.FIFA_MANAGER_ROOM;
  ...
  function ensureHud(pitch, simulation, phase8Snapshot) {
    ...
    if (!hud) {
      ...
      hud.innerHTML = `<span>ME4 <b>MANAGER · MATCH AI</b></span><span data-me4-phase>—</span><span data-me4-ball class="me4-ball-state">BALL —</span><span data-me4-authority>AUTH —</span><span data-me4-pass>PASS —</span><span data-me4-rest>REST DEF —</span><span data-me4-defence>PRESS —</span><span data-me4-manager>MANAGER —</span><button type="button" data-me4-toggle class="active">V4 PHASE 8</button>`;
    ...
    const authorityValue = phase8Snapshot?.authority || simulation?.ballAuthority || "V4_FULL_AUTHORITY";
    ...
    if (toggleButton) {
      ...
      toggleButton.textContent = runtime.enabled ? "V4 PHASE 8" : "LEGACY GÖRÜNTÜ";
    }
  }
  ...
  function applyFrame(force = false) {
    ...
    const phase8Snapshot = adapter()?.getSnapshot?.();
    const simulation = phase8Snapshot?.simulation;
    const mount = document.getElementById("managerMatchMount");
    const pitch = mount?.querySelector?.(".manager-2d-pitch");
    if (!pitch) return false;
    ensureStyles();
    ensureHud(pitch, simulation, phase8Snapshot);
  ...
  function getStatus() {
    const phase8 = adapter()?.getSnapshot?.() || null;
    return {
      version: VERSION,
      enabled: runtime.enabled,
  ...
  function boot() {
    ...
    startLoop();
    applyFrame(true);
    console.info(`[Match Engine V4] Phase 8 manager AI and match management authority ready · ${VERSION}`);
  }
