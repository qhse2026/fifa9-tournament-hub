(() => {
  ...
  document.addEventListener("submit", async event => {
    if (event.target.id === "memberLoginForm") {
      event.preventDefault();
      ...
      try {
        ...
        if (!result?.isAdmin && !result?.playerProfile) { await cloud()?.signOut?.(); throw new Error("Bu hesap henüz bir oyuncu profiline bağlanmamış."); }
        availabilityLoadedAt = 0; await refreshAvailability(true); ctx()?.toast?.(result.isAdmin ? "Yönetici girişi başarılı." : `Hoş geldin, ${result.playerProfile.player_name}.`, "success"); ctx()?.refreshView?.();
      } catch (error) { ctx()?.toast?.(error.message, "error"); button.disabled = false; button.textContent = "Giriş Yap"; }
      return;
    }
