(() => {
  ...
  function deviceKey() {
    ...
    value = window.crypto?.randomUUID?.() || `v22-${Date.now()}-${Math.random().toString(36).slice(2)}-${Math.random().toString(36).slice(2)}`;
  ...
  function emptySummary(matchId) {
    return { matchId, home:0, draw:0, away:0, total:0, myVote:"", top:"", topPct:0 };
  }

  function aggregateRows(matchIds, rows) {
  ...
  async function fetchSummaries(matchIds) {
    ...
    const { data, error } = await sb
      .from("fifa9_community_votes")
      .select("match_id, voter_key, selection")
      .eq("tournament_id", tournamentId)
      .in("match_id", ids);
    if (error) throw error;
    return aggregateRows(ids, data || []);
  }
  ...
  function applySummary(poll, summary) {
    ...
    for (const key of ["home","draw","away"]) {
      ...
      const button = poll.querySelector(`[data-community-vote="${key}"]`);
      if (button) button.classList.toggle("selected", summary.myVote === key);
    }
    ...
    if (verdict) {
      if (total) {
        ...
        verdict.textContent = poll.dataset.open === "true" ? "İlk oyu sen kullan." : "Bu maç için kayıtlı topluluk oyu yok.";
      }
    }
    if (poll.dataset.open === "true") {
      setPollStatus(poll, summary.myVote ? `Oyun: ${names[summary.myVote]}. Maç başlayana kadar değiştirebilirsin.` : "Cihaz başına bir oy; maç başlayana kadar değiştirilebilir.");
    } else {
  ...
  function applyError(poll, error) {
    ...
    const friendly = /relation .*fifa9_community_votes.*does not exist|Could not find the table/i.test(message)
      ? "v22 Supabase SQL kurulumu bekleniyor."
      : message;
  ...
  async function vote(poll, selection) {
    if (!poll || poll.dataset.open !== "true") return;
    const matchId = poll.dataset.matchId;
    if (!matchId || !["home","draw","away"].includes(selection)) return;
  ...
  function ensureRealtime() {
    ...
    channel = sb.channel("fifa9-community-votes-v22")
  ...
  setTimeout(() => hydrate(document), 1200);
