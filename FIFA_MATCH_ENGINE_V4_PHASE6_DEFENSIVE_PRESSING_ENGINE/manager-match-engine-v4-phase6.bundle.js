(() => {
  ...
  function findPassOptions(engine, owner) {
    ...
    return team.players.filter(player => player.id !== owner.id).map(receiver => {
      ...
      return { receiver, d, forward, laneOpponents, receiverSpace, shadowPenalty, score };
  ...
  function resolveCarry(engine, owner) {
    ...
    const carryDistance = engine.rng.range(2.2, 6.8) * (0.75 + owner.attributes.acceleration / 180) * (underStructuredPress ? 0.82 : 1);
...
(() => {
  ...
  const VERSION = "4.0.0-phase6-authority.1";
  ...
  function buildConfig(career, fixture) {
    ...
    return {
      matchId: fixture.id,
      mode: "FULL_AUTHORITY",
      seed: `${career.id}|${fixture.id}|ME4-PHASE6`,
      home: {
        id: fixture.homeId,
  ...
  function attach(career, fixture, targetSecond = 0) {
    ...
    runtime.engine.setBallAuthority(false, { source: "PHASE6_FULL_AUTHORITY", nextDecisionIn: 0.8 });
  ...
  function applyOfficialStats(target = {}, source = {}) {
    const keys = [
      ...
      "pressures", "successfulPressures", "pressureRegains", "forcedTurnovers", "forcedErrors", "doubleTeams", "passingLanesBlocked", "counterPressRegains", "transitionDelays", "offsideTraps", "offsidesWon", "cutbacksDefended", "boxEntriesDenied", "defensiveLineBreaks", "offsides", "possession"
    ];
  ...
  function publishOfficialEvent(legacy, snapshot, report) {
    ...
    const visibleTypes = new Set(["SHOT", "SHOT_BLOCKED", "SHOT_OFF_TARGET", "WOODWORK", "SAVE_HELD", "SAVE_PARRIED", "REBOUND", "GOAL", "CORNER_AWARDED", "SET_PIECE_ORGANIZED", "SET_PIECE_DELIVERY", "DIRECT_FREE_KICK_AWARDED", "INDIRECT_FREE_KICK_AWARDED", "THROW_IN_AWARDED", "PENALTY_AWARDED", "DIRECT_FREE_KICK_TAKEN", "PENALTY_TAKEN", "GOALKEEPER_CLAIM", "GOALKEEPER_PUNCH", "AERIAL_DUEL_WON", "SET_PIECE_CLEARED", "SET_PIECE_COUNTER_STARTED", "PRESS_TRIGGERED", "PRESS_BROKEN", "COUNTER_PRESS_REGAIN", "OFFSIDE", "CUTBACK_DEFENDED"]);
    ...
    const labels = {
      ...
      PRESS_TRIGGERED: "PRES TETİKLENDİ", PRESS_BROKEN: "PRES KIRILDI", COUNTER_PRESS_REGAIN: "KARŞI PRES KAZANIMI", OFFSIDE: "OFSAYT", CUTBACK_DEFENDED: "GERİ PAS SAVUNULDU"
    };
    const item = {
      id: `me4p6-${event.id}`,
      source: "ME4_PHASE6",
  ...
  function applyV4OfficialState(fixture) {
    ...
    legacy.matchEngineV4 = {
      ...
      contract: {
        players: "V4", passingBall: "V4", passControl: "V4", shots: "V4", goalkeeper: "V4", goals: "V4", score: "V4", setPieces: "V4", aerialDuels: "V4", defence: "V4", pressing: "V4", offsideLine: "V4",
  ...
  if (typeof module !== "undefined" && module.exports) module.exports = API;
  globalThis.FIFA_MATCH_ENGINE_V4_PHASE6 = API;
  globalThis.FIFA_MATCH_ENGINE_V4_SHADOW = API;
})();
(() => {
  "use strict";

  const VERSION = "4.0.0-phase6-live.1";
  const STORAGE_KEY = "fifa-me4-phase6-defensive-authority";
  ...
  const clamp = (value, min, max) => Math.max(min, Math.min(max, Number(value) || 0));
  const adapter = () => globalThis.FIFA_MATCH_ENGINE_V4_PHASE6 || globalThis.FIFA_MATCH_ENGINE_V4_SHADOW;
  const room = () => globalThis.FIFA_MANAGER_ROOM;
  ...
  function ensureHud(pitch, simulation, phase6Snapshot) {
    ...
    if (!hud) {
      ...
      hud.innerHTML = `<span>ME4 <b>DEFENCE · PRESSING AI</b></span><span data-me4-phase>—</span><span data-me4-ball class="me4-ball-state">BALL —</span><span data-me4-authority>AUTH —</span><span data-me4-pass>PASS —</span><span data-me4-rest>REST DEF —</span><span data-me4-defence>PRESS —</span><button type="button" data-me4-toggle class="active">V4 PHASE 6</button>`;
    ...
    const authorityValue = phase6Snapshot?.authority || simulation?.ballAuthority || "V4_FULL_AUTHORITY";
    ...
    const activeDefence = simulation?.defensiveAI?.home?.active ? simulation.defensiveAI.home : simulation?.defensiveAI?.away?.active ? simulation.defensiveAI.away : null;
    if (defence) defence.textContent = activeDefence ? `PRESS ${activeDefence.trap} · ${activeDefence.triggerScore}${activeDefence.doubleTeam ? " · 2V1" : ""}` : "PRESS BLOCK";
    const toggleButton = hud.querySelector("[data-me4-toggle]");
    if (toggleButton) {
      toggleButton.classList.toggle("active", runtime.enabled);
      toggleButton.textContent = runtime.enabled ? "V4 PHASE 6" : "LEGACY GÖRÜNTÜ";
    }
  }
  ...
  function applyFrame(force = false) {
    ...
    const phase6Snapshot = adapter()?.getSnapshot?.();
    const simulation = phase6Snapshot?.simulation;
    const mount = document.getElementById("managerMatchMount");
    const pitch = mount?.querySelector?.(".manager-2d-pitch");
    if (!pitch) return false;
    ensureStyles();
    ensureHud(pitch, simulation, phase6Snapshot);
  ...
  function boot() {
    ...
    startLoop();
    applyFrame(true);
    console.info(`[Match Engine V4] Phase 6 defensive and pressing authority ready · ${VERSION}`);
  }
  ...
  if (typeof module !== "undefined" && module.exports) module.exports = API;
  globalThis.FIFA_MATCH_ENGINE_V4_PHASE6_LIVE = API;
