(() => {
  ...
  const VERSION = '5.1.0';
  const BUILD = '510000';
  const STATE_KEY = 'infantino-spatial-ai-v510';
  ...
  const DEFAULT_ADMIN_ADDRESS = 'Çağlar Bey';
  const WAKE_WORD = 'infantino';
  ...
  const state = {
    ...
    handsFree: safeState.handsFree !== false,
    micReady: false,
    wakeEnabled: false,
    conversationActive: false,
    conversationDeadline: 0,
    address: safeState.address || DEFAULT_ADMIN_ADDRESS,
  ...
  function savePrefs() {
    localStorage.setItem(STATE_KEY, JSON.stringify({ muted: state.muted, handsFree: state.handsFree, address: state.address, contextPlayer: state.contextPlayer }));
  }
  ...
  function renderTranscript() {
    ...
    if (!root) return;
    root.innerHTML = state.messages.map(msg => `<div class="orion-msg ${msg.role === 'user' ? 'user' : 'ai'}"><span class="tag">${msg.role === 'user' ? (tr() ? 'SEN' : 'YOU') : 'INFANTINO'}</span>${escapeHTML(msg.text)}${msg.rich?.length ? `<div class="orion-rich">${msg.rich.map(row => `<article><strong>${escapeHTML(row.left)}</strong><span>${escapeHTML(row.right)}</span></article>`).join('')}</div>` : ''}</div>`).join('');
  ...
  function voiceScore(voice) {
    ...
    let score = lang.startsWith(tr() ? 'tr' : 'en') ? 60 : 0;
    if (/natural|premium|neural|online/.test(name)) score += 45;
    if (/google|microsoft|samsung/.test(name)) score += 24;
    if (/ahmet|tolga|male|erkek/.test(name)) score += 12;
  ...
  function humanizeSpeech(text) {
    return String(text || '')
      .replace(/#/g, tr() ? 'sıra ' : 'rank ')
      .replace(/\bPPG\b/g, tr() ? 'puan ortalaması' : 'points per game')
      .replace(/\bFIFA\s*(\d+)/gi, 'FIFA $1')
      .replace(/\s+/g, ' ')
      .trim();
  }

  function maybeResumeConversation(delay = 260) {
    if (!state.conversationActive || state.speaking) return;
    if (Date.now() >= state.conversationDeadline) {
      endConversationSession(false);
      return;
    }
    clearTimeout(conversationTimer);
    conversationTimer = setTimeout(() => startConversationListening(), delay);
  }

  async function speak(text) {
    if (!text) { maybeResumeConversation(); return false; }
    if (state.muted) { maybeResumeConversation(80); return false; }
    const externalVoice = window.INFANTINO_VOICE_PROVIDER;
    if (externalVoice?.speak && typeof externalVoice.speak === 'function') {
      try {
        stopWakeRecognition();
        stopActiveRecognition();
        setSpeaking(true);
        await externalVoice.speak(humanizeSpeech(text), { lang: tr() ? 'tr-TR' : 'en-US', address: state.address });
        setSpeaking(false);
        if (state.conversationActive) maybeResumeConversation(320); else if (state.wakeEnabled) scheduleWakeRestart(480);
        return true;
      } catch (error) {
        console.warn('Infantino external voice provider failed; falling back to browser voice.', error);
        setSpeaking(false);
      }
    }
    if (!('speechSynthesis' in window)) { maybeResumeConversation(80); return false; }
    return new Promise(resolve => {
      try {
        ...
        const done = () => {
          ...
          updateVoiceStatus();
          resolve(true);
          if (state.conversationActive) maybeResumeConversation(320);
          else if (state.wakeEnabled) scheduleWakeRestart(480);
        };
  ...
  function setSpeaking(value) {
    ...
    document.querySelector('#orionWave')?.classList.toggle('active', value || state.listening || state.wakeListening);
    updateVoiceStatus();
  }

  function reply(text, rich = [], options = {}) {
    addMessage('ai', text, rich);
    if (options.speak !== false) return speak(text);
    maybeResumeConversation(80);
  ...
  async function executeCommand(raw) {
    ...
    let phrase = normalize(input).replace(/^(infantino|hey infantino|ok infantino)\s+/, '');
    const data = getData();
    ...
    if (/^(dur|bekle|uyku|standby|sus|stop|go to sleep)$/.test(phrase)) {
      endConversationSession(true);
      return;
    }
    if (/hands free kapat|eller serbest kapat|wake word kapat/.test(phrase)) {
      stopHandsFree();
      return reply(tr() ? `Hands-free modu kapattım ${state.address}.` : `Hands-free mode is off, ${state.address}.`);
    }
    if (/hands free ac|eller serbest ac|wake word ac/.test(phrase)) {
      startHandsFree({ forcePermission: false });
      return reply(tr() ? `Hands-free modu aktif ${state.address}. “Infantino” dediğinizde uyanacağım.` : `Hands-free mode is active, ${state.address}. Say “Infantino” to wake me.`);
    }
  ...
  function isAdminSession() {
    try { return Boolean(app()?.isAdmin?.() || window.FIFA_CLOUD?.isAdmin?.()); }
    catch (_) { return false; }
  }

  function isWakeWord(text) {
    const tokens = normalize(text).split(' ').filter(Boolean);
    return tokens.some(token => token === WAKE_WORD || ['infanteno','infantinu','infantinoo','infantina','infantinio'].includes(token));
  }

  function extractWakeCommand(text) {
    const raw = String(text || '').trim();
    const parts = raw.split(/\s+/);
    const index = parts.findIndex(part => isWakeWord(part));
    if (index < 0) return '';
    return parts.slice(index + 1).join(' ').replace(/^[,.:;!?\s]+/, '').trim();
  }

  function stopActiveRecognition() {
    clearTimeout(listenTimeout);
    if (state.recognition) {
      try { state.recognition.onend = null; state.recognition.onerror = null; state.recognition.stop(); } catch (_) {}
    }
  ...
  function stopWakeRecognition() {
    ...
    if (state.wakeRecognition) {
      try { state.wakeRecognition.onend = null; state.wakeRecognition.onerror = null; state.wakeRecognition.stop(); } catch (_) {}
    }
  ...
  function showPermissionGate(message = '') {
    let gate = document.querySelector('#infantinoPermissionGate');
    if (!gate) {
      gate = document.createElement('button');
      gate.type = 'button';
      gate.id = 'infantinoPermissionGate';
      gate.className = 'infantino-permission-gate';
      gate.addEventListener('click', () => startHandsFree({ forcePermission: true, fromGesture: true }));
      document.body.appendChild(gate);
    }
    gate.innerHTML = `<b>INFANTINO HANDS-FREE</b><span>${escapeHTML(message || (tr() ? 'Bir kez dokun · mikrofonu etkinleştir' : 'Tap once · enable microphone'))}</span>`;
    gate.classList.add('show');
  }

  function hidePermissionGate() {
    document.querySelector('#infantinoPermissionGate')?.classList.remove('show');
  }

  function updateVoiceStatus() {
    const top = document.querySelector('#orionTopStatus');
    const listen = document.querySelector('#orionListenStatus');
    const launcher = document.querySelector('.orion-launcher');
    const voice = ('speechSynthesis' in window) ? preferredVoice() : null;
    launcher?.classList.toggle('standby', state.wakeListening && !state.conversationActive);
    launcher?.classList.toggle('conversation', state.conversationActive);
    let status = tr() ? 'HAZIR' : 'READY';
    if (!isAdminSession()) status = tr() ? 'YÖNETİCİ GİRİŞİ BEKLENİYOR' : 'WAITING FOR ADMIN';
    else if (state.speaking) status = tr() ? 'KONUŞUYOR' : 'SPEAKING';
    else if (state.listening && state.conversationActive) status = tr() ? 'DİNLİYOR · KONUŞMA OTURUMU' : 'LISTENING · CONVERSATION';
    else if (state.wakeListening) status = tr() ? 'BEKLEMEDE · “INFANTINO” DEYİN' : 'STANDBY · SAY “INFANTINO”';
    else if (state.handsFree && state.micReady) status = tr() ? 'HANDS-FREE HAZIR' : 'HANDS-FREE READY';
    else if (state.handsFree) status = tr() ? 'MİKROFON HAZIRLANIYOR' : 'PREPARING MICROPHONE';
    if (listen) listen.textContent = status;
    if (top) top.textContent = `${status}${voice ? ` · ${voice.name}` : ''}`;
    const hf = document.querySelector('[data-infantino-handsfree]');
    if (hf) {
      hf.classList.toggle('active', state.handsFree);
      hf.textContent = state.handsFree ? (tr() ? '◉ HANDS-FREE AÇIK' : '◉ HANDS-FREE ON') : (tr() ? '○ HANDS-FREE KAPALI' : '○ HANDS-FREE OFF');
    }
  }

  function scheduleWakeRestart(delay = 450) {
    clearTimeout(wakeRestartTimer);
    if (!state.wakeEnabled || !state.handsFree || state.conversationActive || state.speaking || document.visibilityState !== 'visible' || !isAdminSession()) return;
    wakeRestartTimer = setTimeout(startWakeRecognitionOnce, delay);
  }

  function startWakeRecognitionOnce() {
    if (!state.recognitionSupported || !state.wakeEnabled || !state.handsFree || state.conversationActive || state.speaking || state.wakeRecognition || document.visibilityState !== 'visible' || !isAdminSession()) return;
    ...
    recognition.onresult = event => {
      ...
      const candidates = result ? [...result].map(item => item.transcript || '') : [];
      const heard = candidates.find(isWakeWord);
      if (!heard) return;
      activated = true;
      const command = extractWakeCommand(heard);
      activateInfantino(command);
    };
    recognition.onerror = event => {
      const code = String(event.error || '');
      if (['not-allowed','service-not-allowed','audio-capture'].includes(code)) {
        state.wakeEnabled = false;
        showPermissionGate(speechErrorMessage(code));
      }
    };
    recognition.onend = () => {
      state.wakeRecognition = null;
      state.wakeListening = false;
      updateVoiceStatus();
      if (!activated) scheduleWakeRestart(isMobile() ? 650 : 400);
    };
    try { recognition.start(); }
    catch (_) { state.wakeRecognition = null; scheduleWakeRestart(900); }
  }

  async function startHandsFree(options = {}) {
    state.handsFree = true;
    savePrefs();
    if (!state.recognitionSupported) {
      updateVoiceStatus();
      showPermissionGate(tr() ? 'Bu tarayıcı wake-word ses tanımayı desteklemiyor.' : 'This browser does not support wake-word speech recognition.');
      return false;
    }
    if (!isAdminSession()) {
      updateVoiceStatus();
    ...
    if (!state.micReady || options.forcePermission) {
      const permission = await ensureMicrophonePermission();
      if (!permission.ok) {
        showPermissionGate(tr() ? 'Mikrofon izni gerekli · tekrar dokun' : 'Microphone permission required · tap again');
        return false;
      }
      state.micReady = true;
    }
  ...
  async function activateInfantino(pendingCommand = '') {
    stopWakeRecognition();
    stopActiveRecognition();
    state.conversationActive = true;
    state.conversationDeadline = Date.now() + 30000;
    openSpatial();
    updateVoiceStatus();
    const greeting = tr() ? `Buyurun ${state.address}.` : `Yes, ${state.address}.`;
    await reply(greeting);
    if (pendingCommand) {
      state.conversationDeadline = Date.now() + 30000;
      executeCommand(pendingCommand);
    } else {
      maybeResumeConversation(220);
    }
  }

  function endConversationSession(announce = false) {
    state.conversationActive = false;
    state.conversationDeadline = 0;
    clearTimeout(conversationTimer);
    stopActiveRecognition();
    updateVoiceStatus();
    if (announce) {
      reply(tr() ? `Elbette ${state.address}. Beklemedeyim.` : `Of course, ${state.address}. Standing by.`);
  ...
  function startConversationListening() {
    if (!state.conversationActive || state.speaking || state.listening) return;
    if (Date.now() >= state.conversationDeadline) { endConversationSession(false); return; }
    if (document.visibilityState !== 'visible') return;
    const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SR) { endConversationSession(false); return; }
    const recognition = new SR();
    ...
    recognition.onstart = () => {
      setListening(true);
      clearTimeout(listenTimeout);
      listenTimeout = setTimeout(() => { try { recognition.stop(); } catch (_) {} }, isMobile() ? 8500 : 10500);
    };
    recognition.onresult = event => {
      const text = String(event.results?.[event.results.length - 1]?.[0]?.transcript || '').trim();
      if (!text) return;
      gotResult = true;
      state.conversationDeadline = Date.now() + 30000;
    ...
    recognition.onend = () => {
      ...
      state.recognition = null;
      setListening(false);
      if (!gotResult && state.conversationActive) maybeResumeConversation(isMobile() ? 550 : 300);
    };
    try { recognition.start(); }
    catch (_) { state.recognition = null; setListening(false); maybeResumeConversation(900); }
  }

  async function beginConversationSession(options = {}) {
    if (!isAdminSession() && !options.manual) return false;
    const permission = await ensureMicrophonePermission();
    if (!permission.ok) { showPermissionGate(); return false; }
    state.micReady = true;
    state.conversationActive = true;
    state.conversationDeadline = Date.now() + 30000;
    stopWakeRecognition();
  ...
  async function announceAdminLogin() {
    if (!isAdminSession()) return;
    renderShell();
    await reply(tr() ? `Hoş geldiniz ${state.address}. Infantino çevrimiçi.` : `Welcome ${state.address}. Infantino is online.`);
    if (state.handsFree) startHandsFree().catch(() => showPermissionGate());
  }

  function watchAdminState() {
    clearInterval(adminWatchTimer);
    const check = () => {
      const current = isAdminSession();
      if (current && !lastAdminState) announceAdminLogin();
      if (!current && lastAdminState) {
        state.wakeEnabled = false;
        state.conversationActive = false;
        stopWakeRecognition();
        stopActiveRecognition();
      }
      lastAdminState = current;
      updateVoiceStatus();
    };
    check();
    adminWatchTimer = setInterval(check, 1200);
  }
  ...
  async function startListening() {
    ...
    await beginConversationSession({ manual: true });
  }

  function setListening(value) {
    state.listening = value;
    document.querySelector('.orion-launcher')?.classList.toggle('listening', value);
    document.querySelector('#orionWave')?.classList.toggle('active', value || state.speaking || state.wakeListening);
    updateVoiceStatus();
  }
  ...
  function renderSpatialWorld() {
    ...
    core.innerHTML = `<div class="spatial-orbit o1"></div><div class="spatial-orbit o2"></div>
      <div class="spatial-core"><div><span>SPATIAL AI</span><strong>FIFA UNIVERSE<br>V5.1</strong><small>${leader ? `${leader.name} · #${leader.rank}` : 'INFANTINO ONLINE'}</small></div></div>
  ...
  function renderShell() {
    ...
    launcher.type = 'button'; launcher.className = 'orion-launcher'; launcher.setAttribute('aria-label', 'Infantino Spatial AI'); launcher.innerHTML = '<span class="orion-glyph">IF</span>';
    ...
    shell.innerHTML = `<header class="spatial-ai-topbar">
      <div class="spatial-ai-brand"><i>IF</i><div><strong>INFANTINO · SPATIAL AI</strong><small>FIFA UNIVERSE V5.1 · HANDS-FREE CONTEXT ENGINE</small></div></div>
      <div class="spatial-ai-status"><i></i><span id="orionTopStatus">ONLINE · LOCAL INTELLIGENCE</span></div>
      <button type="button" data-infantino-handsfree>${state.handsFree ? (tr()?'◉ HANDS-FREE AÇIK':'◉ HANDS-FREE ON') : (tr()?'○ HANDS-FREE KAPALI':'○ HANDS-FREE OFF')}</button>
    ...
    <div class="spatial-ai-body">
      <section class="spatial-stage" id="spatialStage">
        ...
        <div class="spatial-help"><span>${tr()?'Sürükle: evreni döndür':'Drag: rotate universe'}</span><span>${tr()?'Tekerlek: zoom':'Wheel: zoom'}</span><span>Ctrl+Shift+Space: INFANTINO</span></div>
        <aside class="orion-comparison" id="orionComparison"></aside>
      </section>
      <aside class="orion-console">
        <header><div class="orb" id="orionConsoleOrb"></div><div><strong>INFANTINO</strong><small><span id="orionListenStatus">${tr()?'HAZIR':'READY'}</span> · ${state.recognitionSupported ? (tr()?'SES AKTİF':'VOICE READY') : (tr()?'YAZILI MOD':'TEXT MODE')}</small></div><button type="button" data-orion-clear>⌫</button></header>
        ...
        <div class="orion-input-zone"><div class="infantino-wake-hint"><b>WAKE WORD · INFANTINO</b><span>${tr()?'Yönetici oturumunda eller-serbest dinleme':'Hands-free listening during admin session'}</span></div><form id="orionCommandForm"><div class="orion-input-row"><button type="button" class="listen" data-orion-listen>◉</button><input id="orionCommandInput" autocomplete="off" placeholder="${tr()?'Infantino’ya bir şey söyle veya yaz…':'Tell Infantino what you need…'}"/><button type="submit">↗</button></div></form><button type="button" class="orion-keyboard-mic" data-orion-keyboard-mic>${tr()?'⌨️ Telefon klavyesi mikrofonu':'⌨️ Keyboard microphone'}</button>
    ...
    addMessage('ai', tr() ? 'Infantino hazır. Yönetici girişi sonrası “Infantino” dediğinizde otomatik uyanırım. Yazılı komutlar da her zaman kullanılabilir.' : 'Infantino ready. After administrator sign-in, say “Infantino” to wake me automatically. Text commands remain available.');
    bindUI();
  }
  ...
  function openSpatial() {
    ...
    renderSpatialWorld();
    updateContextDisplay();
    if (!isMobile() && !state.conversationActive) setTimeout(() => document.querySelector('#orionCommandInput')?.focus(), 80);
  }
  ...
  function closeSpatial(preserveSpeech = false) {
    ...
    if (state.listening && !state.conversationActive) stopActiveRecognition();
    if (!preserveSpeech && !state.conversationActive && 'speechSynthesis' in window) speechSynthesis.cancel();
  }
  ...
  function bindUI() {
    ...
    document.querySelector('[data-infantino-handsfree]')?.addEventListener('click', () => { if (state.handsFree && state.wakeEnabled) stopHandsFree(); else startHandsFree({ forcePermission: false, fromGesture: true }); });
  ...
  function bindGlobal() {
    ...
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState !== 'visible') { stopWakeRecognition(); stopActiveRecognition(); return; }
      if (state.conversationActive && Date.now() < state.conversationDeadline) maybeResumeConversation(350);
      else if (state.wakeEnabled) scheduleWakeRestart(350);
    });
    if ('speechSynthesis' in window) speechSynthesis.onvoiceschanged = () => updateVoiceStatus();
  ...
  function boot() {
    if (window.__FIFA_SPATIAL_AI_V510__) return;
    window.__FIFA_SPATIAL_AI_V510__ = true;
    renderShell(); bindGlobal(); watchAdminState(); updateVoiceStatus();
    window.INFANTINO = { open: openSpatial, close: closeSpatial, ask: executeCommand, listen: startListening, wake: startHandsFree, sleep: stopHandsFree, data: getData, version: VERSION, setAddress: value => { state.address = String(value || DEFAULT_ADMIN_ADDRESS); savePrefs(); updateVoiceStatus(); }, setVoiceProvider: provider => { window.INFANTINO_VOICE_PROVIDER = provider || null; updateVoiceStatus(); } };
    window.ORION = window.INFANTINO;
  }
