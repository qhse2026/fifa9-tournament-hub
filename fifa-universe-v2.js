(() => {
  ...
  let passportChartMetric = localStorage.getItem("fifa-universe-v2-passport-chart") || "gfPerMatch";
...
function fpiTicker() {
  ...
  function mean(values) {
    const rows = (values || []).map(value => Number(value)).filter(Number.isFinite);
    return rows.length ? rows.reduce((sum, value) => sum + value, 0) / rows.length : 0;
  }

  function passportMetricMeta(key) {
    const map = {
      attack: {
        label: ui("Hücum", "Attack"),
        unit: "/100",
        description: ui("Turnuva bazında hücum verimi", "Tournament-by-tournament attacking output")
      },
      defense: {
        label: ui("Savunma", "Defence"),
        unit: "/100",
        description: ui("Turnuva bazında savunma güveni", "Tournament-by-tournament defensive security")
      },
      bigMatch: {
        label: ui("Büyük Maç", "Big Match"),
        unit: "/100",
        description: ui("Baskı maçlarında üretilen performans", "Performance delivered in pressure matches")
      },
      mp: {
        label: "MP",
        unit: "",
        description: ui("Turnuva bazında oynanan maç", "Matches played by tournament")
      },
      gfPerMatch: {
        label: ui("Attığı Gol Ort.", "Avg Goals For"),
        unit: ui(" gol", " goals"),
        description: ui("Turnuvalardaki gol ortalaması", "Goals scored per match across tournaments")
      },
      gaPerMatch: {
        label: ui("Yediği Gol Ort.", "Avg Goals Against"),
        unit: ui(" gol", " goals"),
        description: ui("Turnuvalardaki yenen gol ortalaması", "Goals conceded per match across tournaments")
      }
    };
  ...
  function passportPerformanceAnalytics(player, data) {
    ...
    const editions = [...(player?.editions || [])].sort((a, b) => a.edition - b.edition).map(edition => {
      const editionEntries = [...(edition.entries || [])];
      const games = Number(edition.games || editionEntries.length || 0);
      const gfPerMatch = games ? Number(edition.gf || 0) / games : 0;
      const gaPerMatch = games ? Number(edition.ga || 0) / games : 0;
      const ppg = games ? Number(edition.points || 0) / games : 0;
      const cleanSheetRate = games ? editionEntries.filter(entry => Number(entry.ga) === 0).length / games : 0;
      const bigEntries = editionEntries.filter(entry => Number(entry.stageWeight || 0) >= 1.18);
      const bigGames = bigEntries.length;
      const bigPoints = bigEntries.reduce((sum, entry) => sum + Number(entry.points || 0), 0);
      const bigPpg = bigGames ? bigPoints / bigGames : ppg;
      const bigGoalDiff = bigGames ? bigEntries.reduce((sum, entry) => sum + Number(entry.gf || 0) - Number(entry.ga || 0), 0) / bigGames : ((Number(edition.gf || 0) - Number(edition.ga || 0)) / Math.max(1, games));
      const attack = clamp(32 + gfPerMatch * 22 + ppg * 11 + ((Number(edition.gf || 0) - Number(edition.ga || 0)) / Math.max(1, games)) * 4, 0, 100);
      const defense = clamp(48 + (1.8 - gaPerMatch) * 20 + cleanSheetRate * 34 + ppg * 6, 0, 100);
      const bigMatch = clamp(42 + bigPpg * 18 + bigGoalDiff * 8 + bigGames * 3, 0, 100);
      return {
        edition: Number(edition.edition || 0),
        label: `FIFA ${Number(edition.edition || 0)}`,
        games,
        gf: Number(edition.gf || 0),
        ga: Number(edition.ga || 0),
        ppg,
        gfPerMatch,
        gaPerMatch,
        attack,
    ...
    const games = Number(player?.games || entries.length || 0);
    const avgGF = games ? Number(player?.gf || 0) / games : 0;
    const avgGA = games ? Number(player?.ga || 0) / games : 0;
    const cleanSheetRate = games ? entries.filter(entry => Number(entry.ga) === 0).length / games : 0;
    const bigEntries = entries.filter(entry => Number(entry.stageWeight || 0) >= 1.18);
    const bigGames = bigEntries.length;
    const bigPoints = bigEntries.reduce((sum, entry) => sum + Number(entry.points || 0), 0);
    const bigPpg = bigGames ? bigPoints / bigGames : Number(player?.ppg || 0);
    const bigGoalDiff = bigGames ? bigEntries.reduce((sum, entry) => sum + Number(entry.gf || 0) - Number(entry.ga || 0), 0) / bigGames : Number(player?.gdPerMatch || 0);
    const attack = clamp(32 + avgGF * 22 + Number(player?.ppg || 0) * 11 + Number(player?.gdPerMatch || 0) * 4, 0, 100);
    const defense = clamp(48 + (1.8 - avgGA) * 20 + cleanSheetRate * 34 + Number(player?.ppg || 0) * 6, 0, 100);
    const bigMatch = clamp(42 + bigPpg * 18 + bigGoalDiff * 8 + Math.min(8, bigGames) * 3, 0, 100);
    const honours = {
      titles: (data?.honours || []).filter(item => item.competition === "oruc" && normalize(item.winner) === player.key),
  ...
  function passportChartSection(analytics, selectedMetric) {
    const metric = passportMetricMeta(selectedMetric);
    const rows = analytics.editions || [];
    const maxValue = Math.max(1, ...rows.map(row => Number(row[selectedMetric] || 0)));
    return `<section class="v2-passport-chart"><header><div><span>PERFORMANCE GRAPH</span><h3>${esc(metric.label)}</h3><p>${esc(metric.description)}</p></div><div class="v2-passport-chart-actions">${[
      ["gfPerMatch", ui("Attığı Gol", "Goals For")],
      ["gaPerMatch", ui("Yediği Gol", "Goals Against")],
      ["attack", ui("Hücum", "Attack")],
      ["defense", ui("Savunma", "Defence")],
      ["bigMatch", ui("Büyük Maç", "Big Match")],
      ["mp", ui("Maç", "Matches")]
    ].map(([key, label]) => `<button type="button" class="${selectedMetric === key ? "active" : ""}" data-v2-passport-chart="${key}">${esc(label)}</button>`).join("")}</div></header><div class="v2-passport-bars">${rows.length ? rows.map(row => {
      const value = Number(row[selectedMetric] || 0);
      const width = clamp(value / maxValue * 100, 6, 100);
      const display = ["attack", "defense", "bigMatch"].includes(selectedMetric) ? `${value.toFixed(1)}/100` : selectedMetric === "mp" ? `${value.toFixed(0)}` : `${value.toFixed(2)}`;
      return `<article><div><span>${esc(row.label)}</span><strong>${display}</strong></div><i><em style="width:${width}%"></em></i></article>`;
  ...
  function renderPlayers(mount) {
    ...
    mount.innerHTML = `<div class="v2-page">${routeNav("playershub")}${modeBar(data, "playershub")}${pageIntro("UNIVERSAL PLAYER PASSPORT", ui("Bir oyuncu. Bütün kariyer.", "One player. The whole career."), ui("Aktif turnuva, Player Standing, tüm zamanlar, kariyer evresi, takım kullanımı ve rekabet geçmişi tek kalıcı profilde.", "Live tournament, Player Standing, all-time history, career state, team usage and rivalry history in one permanent profile."), `<label class="v2-page-player-select"><span>${ui("OYUNCU SEÇ", "SELECT PLAYER")}</span><select id="v2PlayerSelect">${data.players.map(row => `<option value="${esc(row.name)}" ${row.key === player.key ? "selected" : ""}>${esc(row.name)}</option>`).join("")}</select></label>`)}
      ...
      <section class="v2-passport-performance-strip"><button type="button" data-v2-passport-chart="attack" class="${passportChartMetric === "attack" ? "active" : ""}"><span>${ui("Hücum", "Attack")}</span><b>${passport.aggregate.attack.toFixed(1)}</b><small>${ui("turnuva hücum puanı", "tournament attack rating")}</small></button><button type="button" data-v2-passport-chart="defense" class="${passportChartMetric === "defense" ? "active" : ""}"><span>${ui("Savunma", "Defence")}</span><b>${passport.aggregate.defense.toFixed(1)}</b><small>${ui("turnuva savunma puanı", "tournament defence rating")}</small></button><button type="button" data-v2-passport-chart="bigMatch" class="${passportChartMetric === "bigMatch" ? "active" : ""}"><span>${ui("Büyük Maç", "Big Match")}</span><b>${passport.aggregate.bigMatch.toFixed(1)}</b><small>${ui("baskı maçı performansı", "pressure match performance")}</small></button><button type="button" data-v2-passport-chart="mp" class="${passportChartMetric === "mp" ? "active" : ""}"><span>${ui("Oynadığı Maç", "Matches Played")}</span><b>${passport.aggregate.games}</b><small>${ui("kariyer toplamı", "career total")}</small></button><button type="button" data-v2-passport-chart="gfPerMatch" class="${passportChartMetric === "gfPerMatch" ? "active" : ""}"><span>${ui("Ort. Attığı Gol", "Avg Goals For")}</span><b>${passport.aggregate.avgGF.toFixed(2)}</b><small>${ui("tıklayın: turnuva gol grafiği", "click: tournament goal chart")}</small></button><button type="button" data-v2-passport-chart="gaPerMatch" class="${passportChartMetric === "gaPerMatch" ? "active" : ""}"><span>${ui("Ort. Yediği Gol", "Avg Goals Against")}</span><b>${passport.aggregate.avgGA.toFixed(2)}</b><small>${ui("tıklayın: turnuva yenen gol grafiği", "click: tournament conceded chart")}</small></button></section>
  ...
  function installListeners() {
    ...
    document.addEventListener("click", event => {
      ...
      const passportChartButton = event.target.closest?.("[data-v2-passport-chart]");
      if (passportChartButton) {
        passportChartMetric = passportChartButton.dataset.v2PassportChart || "gfPerMatch";
        localStorage.setItem("fifa-universe-v2-passport-chart", passportChartMetric);
        app()?.refreshView?.();
        return;
      }
