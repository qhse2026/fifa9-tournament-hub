(() => {
  ...
  function homePreviewMarkup(data) {
    ...
    return `<div class="su-home-portal-core"><div class="su-home-orbit"></div><div class="su-home-sun"><span>IF</span><b>SPATIAL<br>UNIVERSE</b></div>${leaders.map((p,i)=>`<i style="left:${coords[i]?.[0]||50}%;top:${coords[i]?.[1]||50}%" title="${esc(p.name)}"></i>`).join('')}</div><div class="su-home-portal-copy"><span>FIFA UNIVERSE V5.3 · SPATIAL PREVIEW</span><h3>${isTR()?'Evren artık ana sayfada yaşıyor.':'The universe now lives on the home screen.'}</h3><p>${latest?`${esc(latest.homeName)} ${latest.homeScore}-${latest.awayScore} ${esc(latest.awayName)} · ${esc(latest.stage)}`:(isTR()?'Yeni maç sonucu bekleniyor.':'Awaiting a new result.')}</p><div><button data-su-home-open="galaxy">✦ GALAXY</button><button data-su-home-open="theatre">▶ MATCH THEATRE</button><button data-su-home-open="museum">♛ MUSEUM</button></div></div>`;
  }
