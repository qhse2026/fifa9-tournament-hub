(() => {
  ...
  function issueRefereeSanction(engine, sanction) {
    ...
    if (!card) {
      if (sanction.severity >= profile.redThreshold || sanction.denialOfGoalOpportunity) card = "RED";
      else if (sanction.severity >= profile.yellowThreshold || sanction.tactical) card = "YELLOW";
      else return null;
    }
    if (card === "WARNING") {
    ...
    if (card === "YELLOW") {
      offender.yellowCards = Number(offender.yellowCards || 0) + 1;
  ...
  function foulSeverity(engine, offender, victim, context = {}) {
    ...
    const tactical = Boolean(context.transition || (progress > 48 && defendersBehind <= 3 && pressure > 32));
    const denialOfGoalOpportunity = Boolean(progress > 73 && defendersBehind <= 1 && !context.penalty && victim.group !== "GK");
    let severity = 29 + aggression * .18 + tackleRisk * .16 + pressure * .10 + Number(context.intensity || 0) * 24 + engine.rng.range(-17,17);
    if (tactical) severity += 8;
    if (denialOfGoalOpportunity) severity += 23;
    if (context.reckless) severity += 22;
    if (context.fromBehind) severity += 12;
  ...
  function refereeBehaviourChecks(engine) {
    ...
    ["home","away"].forEach(side => {
      ...
      if (wasting > referee.profile.timeWastingTolerance && engine.rng.chance((wasting-referee.profile.timeWastingTolerance)/240)) {
        const candidate = team.players.find(p=>p.group==="GK") || team.players[0];
