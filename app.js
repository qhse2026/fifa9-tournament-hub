(() => {
  ...
  function cacheState() {
    const safe = window.FIFA_STORAGE_SAFE?.setMainState?.(state);
    if (safe === false) throw new Error("Yerel depolama dolu; güvenli sıkıştırma sonrasında da kayıt yapılamadı.");
    if (safe == null) localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
  }
...
${shareData.url}`)}`;
  ...
  document.addEventListener("click", event => {
    ...
    if (type === "open-fpi-centre") {
      intelligenceSection = "elo";
      navTo("intelligence");
      return;
    }
    if (type === "select-fpi-player") {
      intelligenceFpiPlayer = action.dataset.playerName || "";
      if (activeView === "intelligence") renderIntelligenceCentre();
      return;
    }
