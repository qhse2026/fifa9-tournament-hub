(() => {
  ...
  const FIFA10_FIXED_GROUPS = Object.freeze({
    A: Object.freeze(["Kerim Özmen", "Ercan Köseoğlu", "Emre Çınar", "Ersin Darıcı", "Şafak Denar"]),
    B: Object.freeze(["Oğuzhan Dindar", "Aziz Sarıoğlu", "Sergei Smirnov", "Affan Volkan Akbal"]),
  ...
  if (fifa10TransferImport) {
    state.fifa10StandaloneOperations = fifa10TransferImport;
    cacheState();
  }
  ...
  let intelligencePlayerEdition = 10;
  ...
  function defaultState() {
    return {
      schemaVersion: 1,
      seasonSystem: defaultSeasonSystem(),
      current: {
        edition: 9,
        ...
        finalChapter: defaultFinalChapter(),
        live: {
          active: null,
          archive: {}
        },
  ...
  function mergeState(raw) {
    ...
    const merged = {
      ...
      seasonSystem: {
        ...
        ...(raw.seasonSystem || {}),
        fifa10Draft: {
          ...fresh.seasonSystem.fifa10Draft,
          ...(raw.seasonSystem?.fifa10Draft || {}),
          settings: {
            ...fresh.seasonSystem.fifa10Draft.settings,
            ...(raw.seasonSystem?.fifa10Draft?.settings || {})
          },
      ...
      current: {
        ...
        finalChapter: mergeFinalChapter(raw.current?.finalChapter),
  ...
  function readFifa10TransferFromLocation() {
    const prefix = "#fifa10-transfer=";
    if (!String(location.hash || "").startsWith(prefix)) return null;
    try {
      const encoded = String(location.hash).slice(prefix.length);
      if (!encoded || encoded.length > 120000) throw new Error("Aktarım paketi geçersiz.");
      ...
      Object.entries(payload?.results || {}).forEach(([key, value]) => {
        if (!/^[ABC]-[123]-\d+-\d+$/.test(key) || !value || typeof value !== "object") return;
        const homeScore = Number(value.homeScore);
        const awayScore = Number(value.awayScore);
        if (!Number.isInteger(homeScore) || !Number.isInteger(awayScore) || homeScore < 0 || awayScore < 0 || homeScore > 99 || awayScore > 99) return;
        results[key] = {
          homeScore,
          awayScore,
          homeTeam: String(value.homeTeam || "").slice(0, 80),
          awayTeam: String(value.awayTeam || "").slice(0, 80)
        };
      });
      ...
      console.warn("FIFA 10 result transfer could not be read", error);
      history.replaceState(history.state, "", `${location.pathname}${location.search}`);
      return null;
    }
  }
  ...
  function fifa10IntegratedDraw() {
    return state.seasonSystem?.fifa10Draft?.draw || null;
  }

  function fifa10IntegratedParticipants() {
    const draw = fifa10IntegratedDraw();
    return Array.isArray(draw?.participants) ? draw.participants : [];
  }

  function fifa10PlayerName(id) {
    return fifa10IntegratedParticipants().find(player => player.id === id)?.name?.trim() || "";
  }

  function playerName(id) {
    const fifa10Name = fifa10PlayerName(id);
    if (fifa10Name) return fifa10Name;
  ...
  function activeCompetitionPlayerNames() {
    const fifa10 = fifa10IntegratedParticipants().map(player => player.name?.trim()).filter(Boolean);
    return fifa10.length ? [...new Set(fifa10)] : filledParticipants().map(player => player.name.trim()).filter(Boolean);
  }
  ...
  function defaultFinalChapter() {
    return {
      version: FINAL_CHAPTER_FORMAT_VERSION,
      ...
      activatedAt: null,
      participantIds: [],
      directQuarterFinalistIds: [],
      withdrawnPlayerIds: [],
      legacyKnockoutBackup: null,
      settings: {
        playoffStars: 4,
      ...
      secondChanceSemi: { drawn: false, locked: false, drawSeed: null, drawMethod: null, entrantIds: [], drawnAt: null, lockedAt: null, series: [] },
  ...
  function mergeFinalChapter(raw) {
    const fresh = defaultFinalChapter();
    if (!raw || typeof raw !== "object") return fresh;
    return {
      ...fresh,
      ...raw,
      version: Number(raw.version) || fresh.version,
      formatVersion: Number(raw.formatVersion) || Number(raw.version) || 41,
      ...
      secondChanceSemi: { ...fresh.secondChanceSemi, ...(raw.secondChanceSemi || {}), series: Array.isArray(raw.secondChanceSemi?.series) ? raw.secondChanceSemi.series : [] },
      ...
      withdrawnPlayerIds: Array.isArray(raw.withdrawnPlayerIds) ? raw.withdrawnPlayerIds : [],
  ...
  function migrateFinalChapterFormat(fc) {
    if (!fc || Number(fc.formatVersion || fc.version || 0) >= FINAL_CHAPTER_FORMAT_VERSION) return fc;
    if (finalChapterRawHasCompletedGames(fc)) {
      fc.migrationBlocked = true;
      fc.migrationNote = "Eski Final Chapter formatında sonuç bulunduğu için otomatik dönüşüm yapılmadı.";
      return fc;
    }
    const fresh = defaultFinalChapter();
    const withdrawnIds = FINAL_CHAPTER_WITHDRAWN_NAMES.map(participantIdByName).filter(Boolean);
    const participantSource = Array.isArray(fc.participantIds) && fc.participantIds.length ? fc.participantIds : finalChapterEligibleIds();
    fc.version = FINAL_CHAPTER_FORMAT_VERSION;
    fc.formatVersion = FINAL_CHAPTER_FORMAT_VERSION;
    fc.withdrawnPlayerIds = withdrawnIds;
    fc.participantIds = participantSource.filter(id => !withdrawnIds.includes(id));
    fc.playoff = { ...fresh.playoff };
    fc.secondChanceSemi = { ...fresh.secondChanceSemi };
  ...
  function finalChapterIsEnabled() {
    return Boolean(state.current.finalChapter?.enabled);
  }

  function finalChapterEligibleIds() {
    return [...new Set([...(state.current.phase2?.goldIds || []), ...(state.current.phase2?.silverIds || [])])].filter(Boolean);
  }

  function finalChapterDirectIds() {
    const eligible = new Set(finalChapterEligibleIds());
    return FINAL_CHAPTER_DIRECT_NAMES.map(participantIdByName).filter(id => id && eligible.has(id));
  }

  function finalChapterWithdrawnIds() {
    const eligible = new Set(finalChapterEligibleIds());
    return FINAL_CHAPTER_WITHDRAWN_NAMES.map(participantIdByName).filter(id => id && eligible.has(id));
  }

  function finalChapterSeries(stage) {
    const fc = finalChapterState();
    return Array.isArray(fc[stage]?.series) ? fc[stage].series : [];
  }

  function allFinalChapterGames() {
    const fc = finalChapterState();
    return [
      ...finalChapterSeries("playoff").flatMap(series => series.games || []),
      ...finalChapterSeries("secondChanceSemi").flatMap(series => series.games || []),
  ...
  function finalChapterStageLabel(stage) {
    return ({
      playoff: "Ana Eleme",
      secondChanceSemi: "Son Bilet Play-off Yarı Finali",
  ...
  function finalChapterStars(stage) {
    const settings = finalChapterState().settings;
    return ({
      playoff: settings.playoffStars,
      secondChanceSemi: settings.playoffStars,
  ...
  function finalChapterHistory(action, detail = {}) {
    const fc = finalChapterState();
    fc.history.unshift({ action, at: new Date().toISOString(), ...detail });
    fc.history = fc.history.slice(0, 60);
  }

  function finalChapterHasCompletedGames(stage = null) {
    return allFinalChapterGames().some(match => matchComplete(match) && (!stage || match.finalChapterStage === stage));
  }

  function finalChapterStageWinners(stage) {
    return finalChapterSeries(stage).map(seriesWinner).filter(Boolean);
  }

  function finalChapterStageLosers(stage) {
    return finalChapterSeries(stage).map(series => {
      const winner = seriesWinner(series);
      if (!winner) return null;
      return winner === series.playerAId ? series.playerBId : series.playerAId;
    }).filter(Boolean);
  }

  function finalChapterStageComplete(stage) {
    const series = finalChapterSeries(stage);
    return series.length > 0 && series.every(item => Boolean(seriesWinner(item)));
  }


  const FINAL_CHAPTER_FLOW_STAGES = ["playoff", "secondChanceSemi", "secondChanceFinal", "quarterfinal", "semifinal"];

  function finalChapterDrawPolicy() {
    const value = String(finalChapterState().settings?.drawPolicy || "ask");
    return ["ask", "automatic", "manual"].includes(value) ? value : "ask";
  }

  function finalChapterStageEntrants(stage) {
    const fc = finalChapterState();
    if (stage === "playoff") return fc.participantIds.filter(id => !fc.directQuarterFinalistIds.includes(id));
    if (stage === "secondChanceSemi") return finalChapterStageLosers("playoff");
    if (stage === "secondChanceFinal") return finalChapterStageWinners("secondChanceSemi");
    if (stage === "quarterfinal") return [
      ...fc.directQuarterFinalistIds,
      ...finalChapterStageWinners("playoff"),
      ...finalChapterStageWinners("secondChanceFinal")
    ].filter(Boolean);
    if (stage === "semifinal") return finalChapterStageWinners("quarterfinal");
    return [];
  }

  function finalChapterStageReady(stage) {
    const fc = finalChapterState();
    if (!fc.enabled) return false;
    if (stage === "playoff") return finalChapterStageEntrants(stage).length === 8;
    if (stage === "secondChanceSemi") return fc.playoff.locked && finalChapterStageComplete("playoff") && finalChapterStageEntrants(stage).length === 4;
    if (stage === "secondChanceFinal") return fc.secondChanceSemi.locked && finalChapterStageComplete("secondChanceSemi") && finalChapterStageEntrants(stage).length === 2;
    if (stage === "quarterfinal") return fc.secondChanceFinal.locked && finalChapterStageComplete("secondChanceFinal") && finalChapterStageEntrants(stage).length === 8 && new Set(finalChapterStageEntrants(stage)).size === 8;
    if (stage === "semifinal") return fc.quarterfinal.locked && finalChapterStageComplete("quarterfinal") && finalChapterStageEntrants(stage).length === 4;
    return false;
  }

  function finalChapterStagePairCount(stage) {
    return ({ playoff: 4, secondChanceSemi: 2, secondChanceFinal: 1, quarterfinal: 4, semifinal: 2 })[stage] || 0;
  }

  function finalChapterDrawMethodLabel(method) {
    return ({ automatic: "SİTE KURASI", manual: "MANUEL KURA", legacy: "MEVCUT KURA" })[method] || "KURA";
  }

  function finalChapterDrawPolicyLabel(policy = finalChapterDrawPolicy()) {
    return ({ ask: "HER TURDA SOR", automatic: "SİTE OTOMATİK & KİLİTLİ", manual: "MANUEL EŞLEŞME" })[policy] || "HER TURDA SOR";
  }

  function validateFinalChapterPairings(stage, pairs) {
    const entrants = finalChapterStageEntrants(stage);
    const expectedCount = finalChapterStagePairCount(stage);
    if (!finalChapterStageReady(stage)) return `${finalChapterStageLabel(stage)} henüz kura için hazır değil.`;
    if (!Array.isArray(pairs) || pairs.length !== expectedCount) return `${expectedCount} eşleşme girilmelidir.`;
    const selected = pairs.flat();
    if (selected.some(id => !id)) return "Bütün eşleşme alanları doldurulmalıdır.";
    if (selected.some(id => !entrants.includes(id))) return "Kura havuzunda olmayan bir oyuncu seçildi.";
    if (new Set(selected).size !== entrants.length || selected.length !== entrants.length) return "Her oyuncu yalnızca bir kez kullanılmalıdır.";
    if (pairs.some(([a, b]) => a === b)) return "Bir oyuncu kendisiyle eşleşemez.";
    return "";
  }

  function commitFinalChapterStageDraw(stage, pairs, method, drawSeed = null) {
    const fc = finalChapterState();
    const target = fc[stage];
    if (!target || target.drawn || target.locked || finalChapterHasCompletedGames(stage)) return false;
    const error = validateFinalChapterPairings(stage, pairs);
    if (error) throw new Error(error);
    const nowIso = new Date().toISOString();
    target.drawSeed = drawSeed;
    target.drawMethod = method;
    target.entrantIds = finalChapterStageEntrants(stage);
    target.series = pairs.map((pair, index) => createFinalChapterSeries(stage, index + 1, pair[0], pair[1]));
    target.drawn = true;
    target.locked = true;
    target.drawnAt = nowIso;
    target.lockedAt = nowIso;
    finalChapterHistory(`${stage}-drawn-and-locked`, {
      playerIds: [...target.entrantIds],
      drawSeed,
      drawMethod: method,
      pairings: pairs.map(pair => [...pair])
    });
    return true;
  }

  function automaticFinalChapterDraw(stage, options = {}) {
    const target = finalChapterState()[stage];
    if (!target || target.drawn || target.locked) return false;
    const players = finalChapterStageEntrants(stage);
    const seed = randomSeed();
    const order = shuffled(players, seed);
    const pairs = Array.from({ length: finalChapterStagePairCount(stage) }, (_, index) => [order[index * 2], order[index * 2 + 1]]);
    try {
      const created = commitFinalChapterStageDraw(stage, pairs, "automatic", seed);
      if (created && !options.silent) toast(`${finalChapterStageLabel(stage)} site tarafından çekildi ve kilitlendi.`, "success");
      return created;
    } catch (error) {
  ...
  function processAutomaticFinalChapterProgression() {
    if (finalChapterDrawPolicy() !== "automatic") return false;
    for (const stage of FINAL_CHAPTER_FLOW_STAGES) {
      const target = finalChapterState()[stage];
      if (!target.drawn && !target.locked && finalChapterStageReady(stage)) {
        return automaticFinalChapterDraw(stage, { silent: true });
      }
    }
    return false;
  }

  function finalChapterNextDestination(stage, resultType) {
    const map = {
      playoff: { winner: "Çeyrek Final Havuzu", loser: "Son Bilet Yarı Final Havuzu" },
  ...
  function finalChapterPlayerChips(ids, emptyText = "Sonuç bekleniyor") {
    return ids.length
      ? ids.map(id => `<span class="fc-route-player">${escapeHTML(displayName(id))}</span>`).join("")
      : `<span class="fc-route-empty">${escapeHTML(emptyText)}</span>`;
  }

  function finalChapterProgressionBoard() {
    const fc = finalChapterState();
    const playoffWinners = finalChapterStageWinners("playoff");
    const playoffLosers = finalChapterStageLosers("playoff");
    const lastTicketSemiWinners = finalChapterStageWinners("secondChanceSemi");
    const lastTicketSemiLosers = finalChapterStageLosers("secondChanceSemi");
    const lastTicketWinner = finalChapterStageWinners("secondChanceFinal");
    const lastTicketLoser = finalChapterStageLosers("secondChanceFinal");
    const qfWinners = finalChapterStageWinners("quarterfinal");
    ...
    return `<section class="fc-progression-centre">
      ...
      <div class="fc-route-grid">
        <article><header><b>ANA ELEME KAZANANLARI</b><span>→ ÇEYREK FİNAL</span></header><div>${finalChapterPlayerChips(playoffWinners, "4 tur atlayan bekleniyor")}</div></article>
        <article><header><b>ANA ELEME KAYBEDENLERİ</b><span>→ SON BİLET YF</span></header><div>${finalChapterPlayerChips(playoffLosers, "4 kaybeden bekleniyor")}</div></article>
        <article><header><b>SON BİLET YF GALİPLERİ</b><span>→ SON BİLET FİNALİ</span></header><div>${finalChapterPlayerChips(lastTicketSemiWinners, "2 finalist bekleniyor")}</div></article>
  ...
  function finalChapterDrawPolicyPanel() {
    ...
    const choices = [
      ["ask", "Her Turda Sor", "Hazır olan her turda site veya manuel kura seçimi gösterilir."],
      ["automatic", "Site Otomatik & Kilitli", "Sonuçlar tamamlanınca site kurayı çeker ve anında değiştirilemez şekilde kilitler."],
      ["manual", "Kuraları Biz Çekeceğiz", "Oyuncular doğru havuza aktarılır; eşleşmeleri siz girer ve tek onayla kilitlersiniz."]
    ];
    return `<section class="fc-draw-policy-panel"><div><span>KURA YÖNETİMİ</span><h3>Final Chapter kura politikası</h3><p>Bu seçim yalnızca henüz oluşturulmamış gelecek turları etkiler. Kilitli eşleşmeler değişmez.</p></div><div class="fc-policy-options">${choices.map(([value,label,desc]) => `<button class="${policy === value ? "active" : ""}" data-action="fc-set-draw-policy" data-policy="${value}"><strong>${label}</strong><small>${desc}</small></button>`).join("")}</div></section>`;
  }

  function openFinalChapterManualDraw(stage) {
    if (!canEdit()) return;
    const target = finalChapterState()[stage];
    if (!target || target.drawn || target.locked) { toast("Bu turun kurası zaten oluşturulmuş veya kilitlenmiş.", "error"); return; }
    if (!finalChapterStageReady(stage)) { toast(`${finalChapterStageLabel(stage)} henüz kura için hazır değil.`, "error"); return; }
    const entrants = finalChapterStageEntrants(stage);
    const optionRows = `<option value="">Oyuncu seç</option>${entrants.map(id => `<option value="${id}">${escapeHTML(displayName(id))}</option>`).join("")}`;
    const pairRows = Array.from({ length: finalChapterStagePairCount(stage) }, (_, index) => `<article class="fc-manual-pair"><span>EŞLEŞME ${index + 1}</span><select name="pair-${index}-a" required>${optionRows}</select><b>VS</b><select name="pair-${index}-b" required>${optionRows}</select></article>`).join("");
    openModal(`${finalChapterStageLabel(stage)} · Manuel Kura`, `<form id="finalChapterManualDrawForm" data-stage="${stage}">
      <section class="fc-manual-draw-intro"><span>KURA HAVUZU · ${entrants.length} OYUNCU</span><div>${finalChapterPlayerChips(entrants)}</div><p>Eşleşmeleri kendi kuranızın sonucuna göre girin. Onaylandığında kura anında kilitlenir ve daha sonra değiştirilemez.</p></section>
      <div class="fc-manual-pair-grid">${pairRows}</div>
      <label class="fc-lock-confirm"><input type="checkbox" name="confirmLock" value="yes" required><span>Eşleşmelerin doğru olduğunu ve onaydan sonra değiştirilemeyeceğini kabul ediyorum.</span></label>
      <div class="modal-actions"><button type="button" class="btn btn-ghost" data-action="close-modal">Vazgeç</button><button type="submit" class="btn btn-gold">Eşleşmeleri Onayla ve Kilitle</button></div>
    </form>`, "MANUAL LOCKED DRAW");
  }

  function saveFinalChapterManualDraw(form) {
    if (!canEdit()) return;
    const stage = String(form.dataset.stage || "");
    if (!FINAL_CHAPTER_FLOW_STAGES.includes(stage)) return;
    const data = new FormData(form);
    if (data.get("confirmLock") !== "yes") { toast("Kilit onayını işaretle.", "error"); return; }
    const pairs = Array.from({ length: finalChapterStagePairCount(stage) }, (_, index) => [String(data.get(`pair-${index}-a`) || ""), String(data.get(`pair-${index}-b`) || "")]);
    const error = validateFinalChapterPairings(stage, pairs);
    if (error) { toast(error, "error"); return; }
    if (!window.confirm("Bu eşleşmeler kilitlenecek ve daha sonra değiştirilemeyecek. Devam edilsin mi?")) return;
    try {
      commitFinalChapterStageDraw(stage, pairs, "manual", null);
      refreshFinalChapter();
      closeModal();
      saveState(true, true);
      toast(`${finalChapterStageLabel(stage)} manuel kura ile oluşturuldu ve kilitlendi.`, "success");
      renderFinalChapterEngine();
    } catch (error) {
      toast(error.message, "error");
    }
  }

  function setFinalChapterDrawPolicy(policy) {
    if (!canEdit()) return;
    if (!["ask", "automatic", "manual"].includes(policy)) return;
    const fc = finalChapterState();
    fc.settings.drawPolicy = policy;
    finalChapterHistory("draw-policy-changed", { policy });
    const autoCreated = policy === "automatic" ? processAutomaticFinalChapterProgression() : false;
    refreshFinalChapter();
    saveState(true, true);
    toast(autoCreated ? "Kura politikası kaydedildi; hazır tur site tarafından çekilip kilitlendi." : `Kura politikası: ${finalChapterDrawPolicyLabel(policy)}.`, "success");
    renderFinalChapterEngine();
  }

  function finalChapterEditLockReason(match) {
    if (!match?.finalChapterStage) return "";
    const fc = finalChapterState();
    if (match.finalChapterStage === "playoff" && fc.secondChanceSemi.drawn) return "Son Bilet play-off kurası oluşturulduğu için ana eleme sonuçları artık değiştirilemez.";
  ...
  function refreshFinalChapter() {
    ...
    else if (fc.secondChanceFinal.drawn) fc.status = "last-ticket-final";
    else if (fc.secondChanceSemi.drawn) fc.status = "last-ticket-playoff";
    else if (fc.playoff.drawn) fc.status = "qualifier";
    else fc.status = "setup";
  }

  function activateFinalChapter() {
    if (!canEdit()) return;
    const eligible = finalChapterEligibleIds();
    const direct = finalChapterDirectIds();
    const withdrawn = finalChapterWithdrawnIds();
    if (eligible.length !== 12) {
      toast(`Final Chapter için Altın ve Gümüş gruplarında toplam 12 oyuncu bulunmalıdır. Mevcut: ${eligible.length}.`, "error");
      return;
    }
    if (direct.length !== 3) {
      toast("Doğrudan çeyrek finalistlerden Oğuzhan Dindar, Aziz Sarıoğlu veya Ercan Köseoğlu oyuncu listesinde bulunamadı.", "error");
      return;
    }
    if (withdrawn.length !== 1) {
      toast("Sultan Atasaral oyuncu listesinde bulunamadı. Final Chapter çekilme kaydı uygulanamadı.", "error");
      return;
    }
    if (finalChapterIsEnabled()) { navTo("knockout"); return; }
        const fc = defaultFinalChapter();
    fc.enabled = true;
    fc.status = "setup";
    fc.activatedAt = new Date().toISOString();
    fc.withdrawnPlayerIds = withdrawn;
    fc.participantIds = eligible.filter(id => !withdrawn.includes(id));
    fc.directQuarterFinalistIds = direct;
    fc.legacyKnockoutBackup = JSON.parse(JSON.stringify(state.current.knockout || defaultState().current.knockout));
    fc.history = [{ action: "activated-last-ticket-format", at: fc.activatedAt, playerIds: fc.participantIds, directIds: direct, withdrawnIds: withdrawn }];
    state.current.finalChapter = fc;
    state.current.knockout = defaultState().current.knockout;
    saveState(true, true);
    toast("FIFA09 Final Chapter · Son Bilet formatı etkinleştirildi.", "success");
    navTo("knockout");
  }

  function cancelFinalChapter() {
    if (!canEdit()) return;
    const fc = finalChapterState();
    if (!fc.enabled) return;
    if (finalChapterHasCompletedGames()) {
      toast("Sonuç girilmiş Final Chapter maçları bulunduğu için format iptal edilemez. Önce tüm sonuçları temizle.", "error");
      return;
    }
    if (!window.confirm("Final Chapter hazırlığı iptal edilsin ve varsa önceki eleme şablonu geri yüklensin mi?")) return;
    const legacy = fc.legacyKnockoutBackup;
    state.current.finalChapter = defaultFinalChapter();
    if (legacy) state.current.knockout = legacy;
    saveState(true, true);
    toast("Final Chapter hazırlığı iptal edildi.");
    render();
  }

  function drawFinalChapterStage(stage) {
    if (!canEdit()) return;
    const fc = finalChapterState();
    if (!fc.enabled) { toast("Önce Final Chapter formatını etkinleştir.", "error"); return; }
    const target = fc[stage];
    if (!target || target.drawn || target.locked || finalChapterHasCompletedGames(stage)) {
      toast("Bu turun kurası oluşturulmuş, kilitlenmiş veya sonuç girişi başlamış.", "error");
      return;
    }
    if (!finalChapterStageReady(stage)) { toast(`${finalChapterStageLabel(stage)} henüz kura için hazır değil.`, "error"); return; }
    if (automaticFinalChapterDraw(stage)) {
      refreshFinalChapter();
      saveState(true, true);
      renderFinalChapterEngine();
    }
  ...
  function lockFinalChapterStage(stage) {
    if (!canEdit()) return;
    const target = finalChapterState()[stage];
    if (!target?.drawn) { toast("Önce kurayı oluştur.", "error"); return; }
    if (target.locked) return;
    target.locked = true;
    target.drawMethod = target.drawMethod || "legacy";
    target.entrantIds = target.entrantIds?.length ? target.entrantIds : finalChapterStageEntrants(stage);
    target.lockedAt = target.lockedAt || new Date().toISOString();
    finalChapterHistory(`${stage}-locked`, { drawSeed: target.drawSeed, drawMethod: target.drawMethod });
    saveState(true, true);
    toast(`${finalChapterStageLabel(stage)} kurası kilitlendi.`, "success");
    renderFinalChapterEngine();
  }

  function parseTeamPool(value) {
    return [...new Set(String(value || "").split(/[\n,;]+/).map(item => item.trim()).filter(Boolean))];
  }

  function openFinalChapterTeamPools() {
    if (!canEdit()) return;
    const pools = finalChapterState().teamPools;
    openModal("Final Chapter Takım Havuzları", `<form id="finalChapterPoolsForm">
      <div class="fc-pool-form-grid">
  ...
  function saveFinalChapterTeamPools(form) {
    if (!canEdit()) return;
    const data = new FormData(form);
    const fc = finalChapterState();
    ...
    saveState(true, true);
    closeModal();
    toast("Final Chapter takım havuzları kaydedildi.", "success");
    renderFinalChapterEngine();
  }
  ...
  function finalChapterUsedTeams(playerId, stage, excludeMatchId = "") {
    const used = new Set();
    allFinalChapterGames().forEach(match => {
      if (match.id === excludeMatchId || match.finalChapterStage !== stage) return;
      if (match.homeId === playerId && match.homeTeam) used.add(normalizeTeamKey(match.homeTeam));
      if (match.awayId === playerId && match.awayTeam) used.add(normalizeTeamKey(match.awayTeam));
    });
    return used;
  }
  ...
  function validateFinalChapterTeams(match, homeTeam, awayTeam) {
    ...
    for (const [playerId, team] of checks) {
      if (!team) continue;
      if (finalChapterUsedTeams(playerId, stage, match.id).has(normalizeTeamKey(team))) return `${playerName(playerId)} bu turda ${team} takımını daha önce kullandı.`;
    }
    return "";
  }

  function drawFinalChapterGameTeams(matchId) {
    if (!canEdit()) return;
    ...
    if (["thirdPlace", "final"].includes(stage)) {
      ...
      const enforceRepeatRule = ["quarterfinal", "semifinal"].includes(stage) && finalChapterState().settings.preventTeamRepeat;
      const homeUsed = enforceRepeatRule ? finalChapterUsedTeams(match.homeId, stage, match.id) : new Set();
      const awayUsed = enforceRepeatRule ? finalChapterUsedTeams(match.awayId, stage, match.id) : new Set();
      const homeOptions = pool.filter(team => !homeUsed.has(normalizeTeamKey(team)));
      const awayOptions = pool.filter(team => !awayUsed.has(normalizeTeamKey(team)));
      if (!homeOptions.length || !awayOptions.length) { toast("Bu oyuncu için tekrar etmeyen takım kalmadı. Havuzu genişlet.", "error"); return; }
      match.homeTeam = shuffled(homeOptions, randomSeed())[0];
      const distinctAway = awayOptions.filter(team => normalizeTeamKey(team) !== normalizeTeamKey(match.homeTeam));
      match.awayTeam = shuffled(distinctAway.length ? distinctAway : awayOptions, randomSeed())[0];
    }
    match.updatedAt = new Date().toISOString();
    finalChapterHistory("team-draw", { matchId, stage, homeTeam: match.homeTeam, awayTeam: match.awayTeam });
    saveState(true, true);
    toast("Takım kurası tamamlandı.", "success");
    renderFinalChapterEngine();
  }

  function finalChapterStageControls(stage) {
    const target = finalChapterState()[stage];
    if (!canEdit()) return target?.locked ? `<span class="badge badge-green">${finalChapterDrawMethodLabel(target.drawMethod || "legacy")} · KİLİTLİ</span>` : "";
    if (target.drawn && target.locked) return `<span class="badge badge-green">${finalChapterDrawMethodLabel(target.drawMethod || "legacy")} · KİLİTLİ</span>`;
    if (target.drawn && !target.locked && !finalChapterHasCompletedGames(stage)) return `<button class="btn btn-gold" data-action="fc-lock-stage" data-stage="${stage}">Mevcut Kurayı Kilitle</button>`;
    if (!finalChapterStageReady(stage)) return "";
    const policy = finalChapterDrawPolicy();
    if (policy === "automatic") return `<button class="btn btn-gold" data-action="fc-auto-draw-stage" data-stage="${stage}">Site Kurasını Çek ve Kilitle</button>`;
    if (policy === "manual") return `<button class="btn btn-blue" data-action="fc-open-manual-draw" data-stage="${stage}">Manuel Eşleşmeleri Gir</button>`;
    return `<button class="btn btn-gold" data-action="fc-auto-draw-stage" data-stage="${stage}">Site Çeksin · Kilitle</button><button class="btn btn-blue" data-action="fc-open-manual-draw" data-stage="${stage}">Kendi Kuramızı Gir</button>`;
  }

  function finalChapterEntrantPool(stage) {
    const entrants = finalChapterStageEntrants(stage);
    if (!entrants.length) return "";
    return `<div class="fc-stage-entrant-pool"><span>KURA HAVUZU · ${entrants.length} OYUNCU</span><div>${finalChapterPlayerChips(entrants)}</div></div>`;
  }

  function finalChapterGameRow(game, series, stageLocked, winner, index) {
    const previousComplete = index === 0 || matchComplete(series.games[index - 1]);
    const resultLocked = !stageLocked || !previousComplete || (winner && !matchComplete(game));
    const score = matchComplete(game) ? `${game.homeScore}–${game.awayScore}${game.tiebreakWinnerId ? " P" : ""}` : "Sonuç bekleniyor";
    return `<div class="fc-engine-game ${matchComplete(game) ? "complete" : ""}">
      <div class="fc-game-number">M${index + 1}</div>
      <div class="fc-game-side"><strong>${displayName(game.homeId)}</strong><span>${escapeHTML(game.homeTeam || "Takım bekliyor")}</span></div>
      <div class="fc-game-score">${score}</div>
      <div class="fc-game-side right"><strong>${displayName(game.awayId)}</strong><span>${escapeHTML(game.awayTeam || "Takım bekliyor")}</span></div>
      ${canEdit() ? `<div class="fc-game-actions"><button class="btn btn-ghost btn-small" data-action="fc-draw-teams" data-match-id="${game.id}" ${resultLocked || matchComplete(game) ? "disabled" : ""}>Takım Kurası</button><button class="btn btn-blue btn-small" data-action="edit-match" data-match-id="${game.id}" ${resultLocked ? "disabled" : ""}>${matchComplete(game) ? "Düzelt" : "Sonuç"}</button></div>` : ""}
    </div>`;
  }

  function finalChapterSeriesCard(series, stageLocked) {
    const wins = seriesWins(series);
    const winner = seriesWinner(series);
    return `<article class="fc-engine-series ${winner ? "complete" : ""}">
      <div class="fc-engine-series-head"><div><small>${escapeHTML(finalChapterStageLabel(series.finalChapterStage))} · ${escapeHTML(String(series.stars))}★</small><h4>${displayName(series.playerAId)} <span>vs</span> ${displayName(series.playerBId)}</h4></div><div class="fc-series-score"><b>${wins.a}</b><span>–</span><b>${wins.b}</b></div></div>
      <div class="fc-engine-games">${series.games.map((game, index) => finalChapterGameRow(game, series, stageLocked, winner, index)).join("")}</div>
      ${winner ? `<div class="fc-series-winner fc-series-routing"><span>✓ <strong>${displayName(winner)}</strong></span><em>→ ${escapeHTML(finalChapterNextDestination(series.finalChapterStage, "winner"))}</em><small>${escapeHTML(displayName(winner === series.playerAId ? series.playerBId : series.playerAId))} → ${escapeHTML(finalChapterNextDestination(series.finalChapterStage, "loser"))}</small></div>` : ""}
    </article>`;
  }

  function finalChapterStageSection(stage, title, subtitle, ready, waitingText) {
    const fc = finalChapterState();
    const target = fc[stage];
    if (!ready && !target.drawn) return `<section class="panel fc-engine-stage locked"><div class="fc-engine-stage-head"><div><div class="eyebrow">${escapeHTML(title)}</div><h3>${escapeHTML(waitingText)}</h3><p>${escapeHTML(subtitle)}</p></div><span class="fc-stage-lock">⌛</span></div></section>`;
    const method = target.drawn ? finalChapterDrawMethodLabel(target.drawMethod || "legacy") : "KURA YÖNTEMİ BEKLENİYOR";
    return `<section class="panel fc-engine-stage ${target.drawn ? "drawn" : "ready"}">
      <div class="fc-engine-stage-head"><div><div class="eyebrow">${escapeHTML(title)}</div><h3>${target.drawn ? `${target.series.length} eşleşme hazır · ${method}` : "Oyuncular doğru havuza aktarıldı"}</h3><p>${escapeHTML(subtitle)}</p></div><div class="fc-engine-stage-actions">${finalChapterStageControls(stage)}</div></div>
      ${!target.drawn ? finalChapterEntrantPool(stage) : ""}
      ${target.drawn ? `<div class="fc-engine-series-grid">${target.series.map(series => finalChapterSeriesCard(series, target.locked)).join("")}</div>` : ""}
    </section>`;
  }

  function renderFinalChapterEngine() {
    const fc = finalChapterState();
    if (!fc.enabled) {
      const eligible = finalChapterEligibleIds();
      view.innerHTML = `<div class="fc-engine-page"><section class="fc-engine-hero setup"><div><div class="eyebrow">FIFA09 · FINAL CHAPTER ENGINE</div><h2>Son Bilet formatını hazırla</h2><p>Altın ve Gümüş Grup sonuçları korunur. Sultan Atasaral turnuvada yer almaz; şanslı çeyrek finalist kaldırılır. Üç doğrudan çeyrek finalist dışında kalan sekiz oyuncu dört Best of 3 seri oynar. Elenen dört oyuncu, sekizinci çeyrek final bileti için kendi arasında yeniden Best of 3 play-off oynar.</p><div class="fc-direct-mini">${FINAL_CHAPTER_DIRECT_NAMES.map(name => `<span>QF · ${escapeHTML(name)}</span>`).join("")}<span class="withdrawn">ÇEKİLDİ · Sultan Atasaral</span></div></div><div class="fc-engine-mark"><strong>11</strong><span>AKTİF<br>OYUNCU</span></div></section>
        <section class="panel fc-activation-panel"><div><h3>Onaylanan Final Chapter formatı hazır</h3><p>Onaylanan yapı; 8 kişilik ana eleme, dört elenen oyuncu için Son Bilet play-off'u ve sekiz kişilik çeyrek finalden oluşur.</p></div>${canEdit() ? `<button class="btn btn-gold" data-action="activate-final-chapter">Onaylanan Formatı Başlat</button>` : `<span class="badge badge-gold">YÖNETİCİ BAŞLATMASI BEKLENİYOR</span>`}</section></div>`;
      return;
    }
    refreshFinalChapter();
    const directNames = fc.directQuarterFinalistIds.map(displayName);
    const withdrawnNames = fc.withdrawnPlayerIds.map(displayName);
    const playoffReady = true;
    const secondChanceSemiReady = fc.playoff.locked && finalChapterStageComplete("playoff");
  ...
  function refreshKnockout() {
    if (finalChapterIsEnabled()) { refreshFinalChapter(); return; }
  ...
  function allKnockoutGames() {
    if (finalChapterIsEnabled()) return allFinalChapterGames();
    const ko = state.current.knockout;
  ...
  function findMatch(id) {
    return allCurrentMatches().find(match => match.id === id)
      || fifa9CurrentMatches().find(match => match.id === id)
      || null;
  }
  ...
  function progressPercent() {
    ...
    const estimated = 48 + 30 + (finalChapterIsEnabled() ? 40 : 11);
  ...
  const museumCompetitions = [
    { id: "premier", name: "Premier League", short: "Premier", trophy: "assets/trophies/premier-league.svg", activeFrom: 10, prestige: "Ana Lig Kupası" },
    { id: "championship", name: "Championship", short: "Championship", trophy: "assets/trophies/championship.svg", activeFrom: 10, prestige: "Yükselme Ligi" },
    { id: "oruc", name: "Oruç Reis Kupası", short: "Oruç Reis", trophy: "assets/trophies/oruc-reis-cup.svg", activeFrom: 1, prestige: "Ana Kupa" },
    { id: "super", name: "Süper Kupa", short: "Süper Kupa", trophy: "assets/trophies/super-cup.svg", activeFrom: 10, prestige: "Sezonlar Arası Kupa" }
  ];

  function seasonSystem() {
    if (!state.seasonSystem) state.seasonSystem = defaultSeasonSystem();
    if (!state.seasonSystem.fifa10Draft) state.seasonSystem.fifa10Draft = defaultSeasonSystem().fifa10Draft;
    if (!Array.isArray(state.seasonSystem.customHonours)) state.seasonSystem.customHonours = [];
    return state.seasonSystem;
  }

  function seasonLabel(edition) {
    return `FIFA${String(Number(edition) || 0).padStart(2, "0")}`;
  }

  function normalizeCompetition(value) {
    return museumCompetitions.some(item => item.id === value) ? value : "oruc";
  }

  function historicalHonours() {
    return (historical.editions || []).map(item => ({
      id: `historical-oruc-${item.edition}`,
      edition: Number(item.edition),
      competition: "oruc",
  ...
  function allMuseumHonours() {
    const records = [...historicalHonours()];
    const current = currentFifa9Honour();
    if (current) records.push(current);
    for (const custom of seasonSystem().customHonours) {
      const normalized = {
        id: custom.id || `custom-${Date.now()}-${Math.random().toString(16).slice(2)}`,
        edition: Number(custom.edition),
        competition: normalizeCompetition(custom.competition),
        winner: String(custom.winner || "").trim(),
        runnerUp: String(custom.runnerUp || "").trim(),
        third: String(custom.third || "").trim(),
        source: "custom"
      };
      const index = records.findIndex(item => item.edition === normalized.edition && item.competition === normalized.competition);
  ...
  function fifa10FixtureIdentity(group, leg, homeName, awayName) {
    const names = [normalizeAdministrativeName(homeName), normalizeAdministrativeName(awayName)].sort();
    return `${group}|${Number(leg)}|${names.join("|")}`;
  }

  function fifa10IntegratedSignature(draw) {
    if (!draw) return "";
    return JSON.stringify({
      drawId: draw.drawId,
      status: draw.status,
      participants: (draw.participants || []).map(player => [player.id, player.name, player.elo, player.pot]),
      groups: draw.groups,
      fixtures: (draw.fixtures || []).map(match => [
        match.id, match.group, match.leg, match.homeId, match.awayId,
        match.completed ? Number(match.homeScore) : null,
        match.completed ? Number(match.awayScore) : null,
        match.homeTeam || "", match.awayTeam || "", Boolean(match.completed)
      ]),
      importedStandaloneUpdatedAt: draw.importedStandaloneUpdatedAt || ""
    });
  }

  function ensureFifa10IntegratedTournament() {
    const system = seasonSystem();
    const draft = system.fifa10Draft;
    const existingDraw = draft.draw && typeof draft.draw === "object" ? draft.draw : null;
    const standalone = state.fifa10StandaloneOperations && typeof state.fifa10StandaloneOperations === "object"
      ? state.fifa10StandaloneOperations
      : null;
    const standaloneResults = standalone?.results && typeof standalone.results === "object" ? standalone.results : {};
    const standaloneUpdatedAt = String(standalone?.updatedAt || "");
    const shouldImportStandalone = Boolean(
      standaloneUpdatedAt
      && (!existingDraw || String(existingDraw.importedStandaloneUpdatedAt || "") !== standaloneUpdatedAt)
    );
    const sourcePlayers = [
      ...(Array.isArray(existingDraw?.participants) ? existingDraw.participants : []),
      ...(Array.isArray(draft.players) ? draft.players : [])
    ];
    const sourceByName = new Map(sourcePlayers.map((player, index) => {
      const name = String(player?.name || player?.playerName || player?.player_name || "").trim();
      return [normalizeAdministrativeName(name), { ...player, name, sourceIndex: index }];
    }).filter(([key]) => key));
    const participants = [];
    const participantByName = new Map();
    Object.entries(FIFA10_FIXED_GROUPS).forEach(([group, names]) => {
      names.forEach((name, index) => {
        const source = sourceByName.get(normalizeAdministrativeName(name)) || {};
        const player = {
          id: String(source.id || `F10-${group}-${index + 1}`),
          name,
          elo: Number(source.elo) || (source.source === "new" ? 1350 : 1500),
          source: source.source || "existing",
          registeredAt: source.registeredAt || source.registered_at || null,
          group,
          pot: index + 1,
          tieBreakOrder: Number(source.tieBreakOrder) || participants.length + 1
        };
        participants.push(player);
        participantByName.set(normalizeAdministrativeName(name), player);
      });
    });
    const groups = Object.fromEntries(Object.entries(FIFA10_FIXED_GROUPS).map(([group, names]) => [
      group,
      names.map(name => participantByName.get(normalizeAdministrativeName(name))?.id).filter(Boolean)
    ]));
    const existingByIdentity = new Map((existingDraw?.fixtures || []).map(match => {
      const homeName = (existingDraw?.participants || []).find(player => player.id === match.homeId)?.name || playerName(match.homeId);
      const awayName = (existingDraw?.participants || []).find(player => player.id === match.awayId)?.name || playerName(match.awayId);
      return [fifa10FixtureIdentity(match.group, match.leg, homeName, awayName), match];
    }));
    const fixtures = [];
    let sequence = 0;
    Object.entries(FIFA10_FIXED_GROUPS).forEach(([group, names]) => {
      FIFA10_LEG_STARS.forEach((stars, legIndex) => {
        let pairIndex = 0;
        for (let first = 0; first < names.length; first += 1) {
          for (let second = first + 1; second < names.length; second += 1) {
            pairIndex += 1;
            sequence += 1;
            const reverse = legIndex === 1;
            const homeName = reverse ? names[second] : names[first];
            const awayName = reverse ? names[first] : names[second];
            const identity = fifa10FixtureIdentity(group, legIndex + 1, homeName, awayName);
            const previous = existingByIdentity.get(identity) || {};
            const standaloneResult = standaloneResults[`${group}-${legIndex + 1}-${first}-${second}`] || null;
            const hasStandaloneScore = shouldImportStandalone && standaloneResult
              && standaloneResult.homeScore !== "" && standaloneResult.homeScore != null
              && standaloneResult.awayScore !== "" && standaloneResult.awayScore != null;
            const hasPreviousScore = Boolean(previous.completed)
              && Number.isFinite(Number(previous.homeScore)) && Number.isFinite(Number(previous.awayScore));
            const completed = Boolean(hasStandaloneScore || hasPreviousScore);
            fixtures.push({
              ...previous,
              id: `F10-${group}-${legIndex + 1}-${first}-${second}`,
              edition: 10,
              phase: "fifa10-group",
              group,
              leg: legIndex + 1,
              ...
              matchday: pairIndex,
              sequence,
              homeId: participantByName.get(normalizeAdministrativeName(homeName)).id,
              awayId: participantByName.get(normalizeAdministrativeName(awayName)).id,
              homeScore: completed ? Number(hasStandaloneScore ? standaloneResult.homeScore : previous.homeScore) : null,
              awayScore: completed ? Number(hasStandaloneScore ? standaloneResult.awayScore : previous.awayScore) : null,
              homeTeam: String(hasStandaloneScore ? standaloneResult.homeTeam || "" : previous.homeTeam || ""),
              awayTeam: String(hasStandaloneScore ? standaloneResult.awayTeam || "" : previous.awayTeam || ""),
              tiebreakWinnerId: null,
              allowDraw: true,
              completed,
              updatedAt: completed ? (standalone?.updatedAt || previous.updatedAt || new Date().toISOString()) : null
            });
          }
        }
      });
    });
    const assignments = participants.map((player, index) => ({
      sequence: index + 1,
      pot: player.pot,
      playerId: player.id,
      ...
      elo: player.elo,
      group: player.group,
      revealedAt: existingDraw?.completedAt || standalone?.updatedAt || null
    }));
    const nextDraw = {
      ...(existingDraw || {}),
      version: 2,
      drawId: existingDraw?.drawId || "F10-OFFICIAL-FIXED-GROUPS",
      status: "completed",
      entryMode: "official-fixed-groups",
      createdAt: existingDraw?.createdAt || standalone?.updatedAt || new Date().toISOString(),
      completedAt: existingDraw?.completedAt || standalone?.updatedAt || new Date().toISOString(),
      participants,
      groups,
      assignments,
      fixtures,
      fivePlayerGroups: ["A", "C"],
      fivePlayerGroup: "A",
      rankingRule: {
        primary: "PPG",
      ...
      importedStandaloneUpdatedAt: shouldImportStandalone
        ? standaloneUpdatedAt
        : String(existingDraw?.importedStandaloneUpdatedAt || standaloneUpdatedAt)
    };
    const changed = fifa10IntegratedSignature(existingDraw) !== fifa10IntegratedSignature(nextDraw);
    if (!changed) return { changed: false, draw: existingDraw };
    const updatedAt = standalone?.updatedAt || new Date().toISOString();
    nextDraw.updatedAt = updatedAt;
    draft.draw = nextDraw;
    draft.players = participants.map(player => ({
      id: player.id,
      name: player.name,
      elo: player.elo,
    ...
    draft.status = fixtures.some(match => match.completed) ? "group-stage-active" : "groups-ready";
    draft.settings.registrationOpen = false;
    draft.settings.potsLocked = true;
    draft.updatedAt = updatedAt;
    system.activeEdition = 10;
    return { changed: true, draw: nextDraw };
  }

  function syncFifa10ResultCompatibility(match, clear = false) {
    if (match?.phase !== "fifa10-group" || Number(match.edition || 10) !== 10) return;
    const draft = seasonSystem().fifa10Draft;
    const now = new Date().toISOString();
    if (draft.draw) {
      draft.draw.updatedAt = now;
      draft.status = (draft.draw.fixtures || []).every(item => matchComplete(item))
        ? "group-stage-completed"
        : "group-stage-active";
      draft.updatedAt = now;
    }
    const key = String(match.id || "").replace(/^F10-/, "");
    const previous = state.fifa10StandaloneOperations && typeof state.fifa10StandaloneOperations === "object"
      ? state.fifa10StandaloneOperations
      : {};
    const results = { ...(previous.results || {}) };
    if (clear) delete results[key];
    else results[key] = {
    ...
    if (draft.draw) draft.draw.importedStandaloneUpdatedAt = now;
  }
  ...
  function validateFifa10MatchTeams(match, homeTeam, awayTeam) {
    ...
    for (const check of checks) {
      const key = normalizeTeamKey(check.team);
      if (!key) continue;
      const repeated = fifa10CurrentMatches().some(item => {
        if (item.id === match.id || !matchComplete(item)) return false;
        if (item.homeId === check.playerId && normalizeTeamKey(item.homeTeam) === key) return true;
        return item.awayId === check.playerId && normalizeTeamKey(item.awayTeam) === key;
      });
      if (repeated) return `${playerName(check.playerId)} FIFA 10 boyunca ${check.team} takımını daha önce kullandı.`;
    }
    return "";
  }
  ...
  function completedFifa9SeasonSnapshot() {
    ...
    const matches = fifa9CurrentMatches().filter(matchComplete).map(match => ({
      id: match.id,
      edition: 9,
  ...
  function competitionMedalCounts(id) {
    const records = honoursForCompetition(id);
    return {
      titles: records.filter(item => item.winner).length,
      runners: records.filter(item => item.runnerUp).length,
      thirds: records.filter(item => item.third).length
    };
  }

  function museumPlayerNames() {
    const names = new Set();
    (historical.allTime || []).forEach(item => item.name && names.add(item.name));
    state.current.participants.forEach(item => item.name?.trim() && names.add(item.name.trim()));
    (seasonSystem().fifa10Draft?.players || []).forEach(item => item.name?.trim() && names.add(item.name.trim()));
    allMuseumHonours().forEach(item => [item.winner,item.runnerUp,item.third].forEach(name => name && names.add(name)));
    return [...names].sort((a,b)=>a.localeCompare(b,"tr"));
  }

  function playerMuseumRecord(name) {
    const records = allMuseumHonours();
    const byCompetition = Object.fromEntries(museumCompetitions.map(comp => [comp.id, {
      titles: records.filter(r => r.competition === comp.id && r.winner === name).map(r=>r.edition),
      runners: records.filter(r => r.competition === comp.id && r.runnerUp === name).map(r=>r.edition),
      thirds: records.filter(r => r.competition === comp.id && r.third === name).map(r=>r.edition)
    }]));
    const totals = Object.values(byCompetition).reduce((acc,item)=>{
      acc.titles += item.titles.length;
      acc.runners += item.runners.length;
      acc.thirds += item.thirds.length;
      return acc;
    },{titles:0,runners:0,thirds:0});
    return { byCompetition, totals };
  }

  function honourMedalPill(record, type) {
    const name = type === "winner" ? record.winner : type === "runner" ? record.runnerUp : record.third;
    const className = type === "winner" ? "gold" : type === "runner" ? "silver" : "bronze";
    const label = type === "winner" ? "Winner" : type === "runner" ? "Runners-up" : "Third place";
    return `<div class="museum-medal-pill ${className}"><span>${seasonLabel(record.edition)}</span><strong>${escapeHTML(name || "—")}</strong><small>${label}</small></div>`;
  }

  function renderMuseumTrophyCard(comp) {
    const counts = competitionMedalCounts(comp.id);
    const available = museumSelectedEdition >= comp.activeFrom;
    const record = allMuseumHonours().find(item => item.competition === comp.id && item.edition === museumSelectedEdition);
    return `<article class="museum-trophy-card ${comp.id === "oruc" ? "featured" : ""} ${available ? "" : "future"}">
      <div class="museum-trophy-head"><span>${escapeHTML(comp.name)}</span><small>${escapeHTML(comp.prestige)}</small></div>
      <div class="museum-trophy-stage"><img src="${comp.trophy}" alt="${escapeHTML(comp.name)} kupası" /></div>
      <div class="museum-trophy-season">${available ? (record ? `${seasonLabel(museumSelectedEdition)} KAYDI` : museumSelectedEdition >= 10 ? `${seasonLabel(museumSelectedEdition)} İLE BAŞLAYACAK` : "TARİHİ KOLEKSİYON") : `${seasonLabel(comp.activeFrom)} İLE BAŞLAYACAK`}</div>
      <div class="museum-trophy-counts">
        <div><strong>${counts.titles}</strong><span>Şampiyonluk</span></div>
        <div><strong>${counts.runners}</strong><span>İkincilik</span></div>
        <div><strong>${counts.thirds}</strong><span>Üçüncülük</span></div>
      </div>
    </article>`;
  }

  function renderHonourBoard() {
    const records = allMuseumHonours().filter(item => item.competition === "oruc");
    const editions = Array.from({length:10},(_,i)=>i+1);
    const rows = [
      ["winner","🏆","Winner","Şampiyon"],
      ["runner","◈","Runners-up","İkincilik"],
      ["third","◆","Third place","Üçüncülük"]
    ];
    return `<section class="panel museum-honour-board">
      <div class="panel-header"><div><h3 class="panel-title">Onur Tablosu</h3><div class="panel-subtitle">Oruç Reis Kupası tarihinin sezon bazlı madalya kayıtları.</div></div>${canEdit()?`<button class="btn btn-ghost btn-small" data-action="open-honour-editor">Kupa Kaydı Düzenle</button>`:""}</div>
      <div class="honour-season-strip">${editions.map(e=>`<button class="${museumSelectedEdition===e?"active":""}" data-action="set-museum-edition" data-edition="${e}">${seasonLabel(e)}</button>`).join("")}</div>
      <div class="honour-board-grid">${rows.map(([type,icon,en,tr])=>{
        const key = type === "winner" ? "winner" : type === "runner" ? "runnerUp" : "third";
        return `<div class="honour-board-row ${type}"><div class="honour-board-label"><span>${icon}</span><div><strong>${en}</strong><small>${tr}</small></div></div><div class="honour-board-seasons">${editions.map(e=>{
          const rec=records.find(r=>r.edition===e); const value=rec?.[key]||"";
          return `<div class="honour-mini-card ${museumSelectedEdition===e?"selected":""} ${value?"filled":"pending"}"><span>${seasonLabel(e)}</span><strong>${escapeHTML(value || (e===9&&!currentFifa9Honour()?"Devam ediyor":e===10?"Yeni format":"—"))}</strong></div>`;
        }).join("")}</div></div>`;
      }).join("")}</div>
    </section>`;
  }

  function renderPlayerMuseum() {
    const names = museumPlayerNames();
    if (!museumSelectedPlayerName || !names.includes(museumSelectedPlayerName)) museumSelectedPlayerName = names[0] || "";
    const record = playerMuseumRecord(museumSelectedPlayerName);
    const wonSeasons = Object.values(record.byCompetition).flatMap(item=>item.titles).sort((a,b)=>a-b);
    return `<section class="panel museum-player-history">
      <div class="panel-header"><div><h3 class="panel-title">Oyuncu Kupa Geçmişi</h3><div class="panel-subtitle">Şampiyonluk, finalistlik ve üçüncülük koleksiyonları.</div></div><select id="museumPlayerSelect" class="select-control">${names.map(name=>`<option value="${escapeHTML(name)}" ${name===museumSelectedPlayerName?"selected":""}>${escapeHTML(name)}</option>`).join("")}</select></div>
      <div class="museum-player-layout">
        <div class="museum-player-card"><div class="museum-player-avatar">${escapeHTML(museumSelectedPlayerName.split(/\s+/).map(v=>v[0]).slice(0,2).join(""))}</div><h4>${escapeHTML(museumSelectedPlayerName || "Oyuncu seç")}</h4><span>Kupa Müzesi Profili</span><div class="museum-player-total"><strong>${record.totals.titles}</strong><small>Toplam Şampiyonluk</small></div></div>
        <div class="museum-player-cabinets">${museumCompetitions.map(comp=>{
          const item=record.byCompetition[comp.id];
          return `<div class="museum-mini-cabinet"><img src="${comp.trophy}" alt=""/><strong>${escapeHTML(comp.name)}</strong><div><b>${item.titles.length}</b> şampiyonluk</div><small>${item.titles.length?item.titles.map(seasonLabel).join(" · "):"Henüz yok"}</small></div>`;
        }).join("")}</div>
        <div class="museum-player-summary"><div><span>🏆</span><strong>${record.totals.titles}</strong><small>Şampiyonluk</small></div><div><span>◈</span><strong>${record.totals.runners}</strong><small>İkincilik</small></div><div><span>◆</span><strong>${record.totals.thirds}</strong><small>Üçüncülük</small></div></div>
      </div>
      <div class="museum-won-seasons"><strong>Kazanılan Sezonlar</strong><div>${wonSeasons.length?wonSeasons.map(e=>`<span>${seasonLabel(e)}</span>`).join(""):`<span class="empty">Henüz şampiyonluk kaydı yok</span>`}</div></div>
    </section>`;
  }
  ...
  function autoAssignFifa10Players(showToast = true) {
    const draft = seasonSystem().fifa10Draft;
    const filled = draft.players.filter(item=>item.name?.trim());
    const sorted = [...filled].sort((a,b)=>rankForDraftPlayer(a.name)-rankForDraftPlayer(b.name) || a.name.localeCompare(b.name,"tr"));
    const premierIds = new Set(sorted.slice(0, Number(draft.settings?.premierSize)||7).map(item=>item.id));
    draft.players.forEach(item => item.league = item.name?.trim() && premierIds.has(item.id) ? "premier" : "championship");
    draft.updatedAt = new Date().toISOString();
    saveState(false,true);
    if (showToast) toast("FIFA 10 oyuncuları Tüm Zamanlar sıralamasına göre dağıtıldı.","success");
  }

  function createFifa10Draft() {
    if (!canEdit()) return;
    const system = seasonSystem();
    if (system.fifa10Draft.status === "draft") { toast("FIFA 10 taslağı zaten aktif.","error"); return; }
    const currentNames = state.current.participants.map(item=>item.name?.trim()).filter(Boolean);
    const fallback = (historical.allTime || []).slice(0,16).map(item=>item.name);
    const names = currentNames.length ? currentNames : fallback;
    system.fifa10Draft = {
      ...defaultSeasonSystem().fifa10Draft,
      status: "draft",
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
      players: names.map((name,index)=>({ id:`F10P${String(index+1).padStart(2,"0")}`, name, league:"championship" }))
    };
  ...
  function openCancelFifa10Draft() {
    if (!canEdit()) return;
    openModal("FIFA 10 Taslağını İptal Et",`<div class="info-box warning-box">Bu işlem yalnızca FIFA 10 test taslağını, oyuncu dağılımını ve taslak ayarlarını siler. Devam eden FIFA 09 turnuvası ile tarihî kupa kayıtları etkilenmez.</div><div class="modal-actions"><button class="btn btn-ghost" data-action="close-modal">Vazgeç</button><button class="btn btn-danger" data-action="confirm-cancel-fifa10-draft">Taslağı Kalıcı Olarak İptal Et</button></div>`,"DRAFT CONTROL");
  }
  ...
  function openHonourEditor(recordId="") {
    if (!canEdit()) return;
    const records=allMuseumHonours();
    const record=records.find(item=>item.id===recordId)||records.find(item=>item.edition===museumSelectedEdition&&item.competition==="oruc")||{edition:museumSelectedEdition,competition:"oruc",winner:"",runnerUp:"",third:""};
    const names=museumPlayerNames();
    const options=(selected)=>`<option value="">— Seçilmedi —</option>${names.map(name=>`<option value="${escapeHTML(name)}" ${name===selected?"selected":""}>${escapeHTML(name)}</option>`).join("")}`;
    openModal("Kupa Kaydı Düzenle",`<form id="honourRecordForm" class="form-stack"><input type="hidden" name="recordId" value="${escapeHTML(record.source==="custom"?record.id:"")}"><div class="form-grid"><label>Sezon<input class="text-input" name="edition" type="number" min="1" max="99" value="${Number(record.edition)||museumSelectedEdition}" required></label><label>Kupa<select class="select-control" name="competition">${museumCompetitions.map(comp=>`<option value="${comp.id}" ${comp.id===record.competition?"selected":""}>${escapeHTML(comp.name)}</option>`).join("")}</select></label></div><label>Winner / Şampiyon<select class="select-control" name="winner">${options(record.winner)}</select></label><label>Runners-up / İkinci<select class="select-control" name="runnerUp">${options(record.runnerUp)}</select></label><label>Third place / Üçüncü<select class="select-control" name="third">${options(record.third)}</select></label><div class="modal-actions"><button type="button" class="btn btn-ghost" data-action="close-modal">Vazgeç</button><button class="btn btn-gold" type="submit">Kupa Kaydını Kaydet</button></div></form>`,"ADMIN · TROPHY HISTORY");
  }

  function saveHonourRecord(form) {
    if (!canEdit()) return;
    const data=new FormData(form); const system=seasonSystem();
    const record={id:String(data.get("recordId")||`honour-${Date.now()}`),edition:Number(data.get("edition")),competition:normalizeCompetition(String(data.get("competition")||"oruc")),winner:String(data.get("winner")||"").trim(),runnerUp:String(data.get("runnerUp")||"").trim(),third:String(data.get("third")||"").trim()};
    if (!record.edition || record.edition<1) { toast("Geçerli bir sezon numarası gir.","error"); return; }
    const existing=system.customHonours.findIndex(item=>item.id===record.id || (Number(item.edition)===record.edition&&item.competition===record.competition));
    if (existing>=0) system.customHonours[existing]=record; else system.customHonours.push(record);
    saveState(true,true); closeModal(); museumSelectedEdition=record.edition; renderSeasonMuseum(); toast("Kupa müzesi kaydı güncellendi.","success");
  }
  ...
  function liveEventThreatValue(event) {
    const weights = { goal: .9, penalty: .7, penaltymiss: .6, bigchance: .55, hugemiss: .62, ontarget: .3, dangerous: .22, attack: .14, note: .07, yellow: .03, red: .09 };
  ...
  function buildLiveStudioDataFromLive(live, match) {
    ...
    const eventProfile = type => ({
      ...
      penalty: { swing: 27, home: 46, away: 46, minute: 88, label: intelligenceCopy('Penaltı', 'Penalty'), marker: '●' },
      penaltymiss: { swing: 20, home: 39, away: 39, minute: 76, label: intelligenceCopy('Penaltı Kaçtı', 'Penalty Missed'), marker: '⊘' },
  ...
  function renderLiveStudioMomentumPanel(data) {
    ...
    const iconMap = { goal: '⚽', penalty: '●', penaltymiss: '⊘', bigchance: '◎', hugemiss: '✖', ontarget: '◉', dangerous: '▲', attack: '↗', yellow: '🟨', red: '🟥', note: '•' };
    return `<div class="live-studio-panel-stack">
  ...
  function renderLiveStudioPressurePanel(data) {
    ...
    const labelMap = { goal: intelligenceCopy('Gol', 'Goal'), penalty: intelligenceCopy('Penaltı', 'Penalty'), penaltymiss: intelligenceCopy('Penaltı Kaçtı', 'Penalty Missed'), bigchance: intelligenceCopy('Büyük Şans', 'Big Chance'), hugemiss: intelligenceCopy('Mutlak Gol Kaçtı', 'Huge Miss'), ontarget: intelligenceCopy('İsabetli Şut', 'Shot on Target'), dangerous: intelligenceCopy('Tehlikeli Baskı', 'Dangerous Pressure'), attack: intelligenceCopy('Atak', 'Attack'), yellow: intelligenceCopy('Sarı Kart', 'Yellow Card'), red: intelligenceCopy('Kırmızı Kart', 'Red Card'), note: intelligenceCopy('Not', 'Note') };
  ...
  function renderLiveMatchCentre() {
    ...
    view.innerHTML = `
      <div class="group-banner live-match-banner active-live-banner studio-banner">
        <div><div class="eyebrow"><span class="live-pulse-dot"></span> LIVE MATCH STUDIO v35</div><h2>${displayName(match.homeId)} vs ${displayName(match.awayId)}</h2><p>${escapeHTML(liveStageLabel(match))} · Son güncelleme ${escapeHTML(formatLiveUpdatedAt(live.updatedAt))} · ${escapeHTML(studio.importance.note || '')}</p></div>
      ...
      <section class="live-studio-scoreboard">
        ...
        <div class="studio-team-card away"><span class="studio-side-tag">DEPLASMAN</span><strong>${displayName(match.awayId)}</strong><small>${escapeHTML(live.awayTeam || match.awayTeam || 'Takım bekleniyor')}</small>${renderLiveHeatBadge(studio.awayHeat, studio.awayName)}<div class="live-view-score">${live.awayScore}</div></div>
      </section>
      ...
      ${canEdit() ? `<section class="panel master-live-control-panel mt-24">
        ...
        <div class="master-live-control-grid">
          ...
          <section class="master-match-console">
            <div class="master-clock-label">${intelligenceCopy("MAÇ SÜRESİ","MATCH CLOCK")}</div>
            <div class="master-clock-row"><strong>${Number(live.minute)||0}'</strong><div><span>+${Number(live.addedTime)||0}</span><small>${intelligenceCopy("EK SÜRE","ADDED")}</small></div></div>
            <div class="master-minute-buttons"><button data-action="live-minute-change" data-delta="-5">−5</button><button data-action="live-minute-change" data-delta="-1">−1</button><button data-action="live-minute-change" data-delta="1">+1</button><button data-action="live-minute-change" data-delta="5">+5</button></div>
            <div class="master-minute-inputs"><label><span>${intelligenceCopy("Dakika","Minute")}</span><input type="number" min="0" max="130" value="${Number(live.minute)||0}" data-live-number="minute"></label><label><span>${intelligenceCopy("Ek","Added")}</span><input type="number" min="0" max="20" value="${Number(live.addedTime)||0}" data-live-number="addedTime"></label></div>
            <div class="master-score-centre"><span>${intelligenceCopy("SKOR","SCORE")}</span><strong>${live.awayScore}<i>–</i>${live.homeScore}</strong><small>${displayName(match.awayId)} · ${displayName(match.homeId)}</small></div>
            <div class="master-status-title">${intelligenceCopy("MAÇ DURUMU","MATCH STATE")}</div>
            <div class="master-status-grid"><button class="${live.status==='live'?'active is-live':''}" data-action="live-set-status" data-status="live">● ${intelligenceCopy("Canlı","Live")}</button><button class="${live.status==='paused'?'active':''}" data-action="live-set-status" data-status="paused">Ⅱ ${intelligenceCopy("Duraklat","Pause")}</button><button class="${live.status==='halftime'?'active':''}" data-action="live-set-status" data-status="halftime">◫ ${intelligenceCopy("Devre Arası","Half-Time")}</button><button class="${live.status==='secondhalf'?'active':''}" data-action="live-set-status" data-status="secondhalf">↻ ${intelligenceCopy("2. Yarı","2nd Half")}</button><button class="${live.status==='extra'?'active':''}" data-action="live-set-status" data-status="extra">◷ ${intelligenceCopy("Uzatma","Extra Time")}</button><button class="${live.status==='penalties'?'active':''}" data-action="live-set-status" data-status="penalties">▦ ${intelligenceCopy("Penaltılar","Penalties")}</button></div>
            <div class="master-control-actions"><button data-action="share-live-match">${intelligenceCopy("Skoru Paylaş","Share")}</button><button data-action="open-live-pulse">${intelligenceCopy("Canlı Nabız","Live Pulse")}</button><button class="danger" data-action="open-cancel-live">${intelligenceCopy("İptal","Cancel")}</button><button class="finish" data-action="open-finish-live">${intelligenceCopy("Bitir ve Kaydet","Finish & Save")}</button></div>
          </section>
  ...
  function renderLiveControlEventButtons(side) {
    const events = [
      { type:"attack", icon:"↗", tr:"Atak", en:"Attack" },
      { type:"dangerous", icon:"▲", tr:"Tehlikeli Atak", en:"Dangerous Attack" },
      { type:"bigchance", icon:"◎", tr:"Büyük Şans", en:"Big Chance" },
      { type:"hugemiss", icon:"✖", tr:"Mutlak Gol Kaçtı", en:"Huge Chance Missed" },
      { type:"ontarget", icon:"◉", tr:"İsabetli Şut", en:"Shot on Target" },
      { type:"goal", icon:"⚽", tr:"Gol", en:"Goal", goal:true },
      { type:"yellow", icon:"▰", tr:"Sarı Kart", en:"Yellow Card" },
      { type:"red", icon:"■", tr:"Kırmızı Kart", en:"Red Card" },
      { type:"penalty", icon:"●", tr:"Penaltı", en:"Penalty" },
      { type:"penaltymiss", icon:"⊘", tr:"Penaltı Kaçtı", en:"Penalty Missed" }
    ];
    return events.map(event=>`<button type="button" class="master-event-btn ${event.goal?'is-goal':''}" data-action="live-quick-event" data-event-type="${event.type}" data-side="${side}"><span>${event.icon}</span><strong>${intelligenceCopy(event.tr,event.en)}</strong></button>`).join("");
  }

  function renderLiveEvent(event, match) {
    const iconMap = { goal: "⚽", penalty: "●", penaltymiss: "⊘", bigchance: "◎", hugemiss: "✖", ontarget: "◉", dangerous: "▲", attack: "↗", yellow: "▰", red: "■", note: "◆" };
    const labelMap = { goal: "GOL", penalty: "PENALTI", penaltymiss: "PENALTI KAÇTI", bigchance: "BÜYÜK ŞANS", hugemiss: "MUTLAK KAÇTI", ontarget: "İSABETLİ ŞUT", dangerous: "TEHLİKELİ BASKI", attack: "ATAK", yellow: "SARI KART", red: "KIRMIZI KART", note: "MAÇ NOTU" };
  ...
  function addLiveEvent(form) {
    ...
    active.events = Array.isArray(active.events) ? active.events : [];
    const defaultText = type === "goal" ? "Gol" : type === "penalty" ? "Penaltı kazanıldı" : type === "penaltymiss" ? "Penaltı kaçtı" : type === "yellow" ? "Sarı kart" : type === "red" ? "Kırmızı kart" : type === "bigchance" ? "Büyük gol fırsatı" : type === "hugemiss" ? "Mutlak gol kaçtı" : type === "ontarget" ? "İsabetli şut" : type === "dangerous" ? "Tehlikeli baskı" : type === "attack" ? "Atak" : "Canlı maç olayı";
  ...
  function addLiveQuickEvent(type, side) {
    if (!canEdit()) return;
    const active = getLiveState().active;
    const allowed = ["attack", "dangerous", "bigchance", "hugemiss", "ontarget", "goal", "yellow", "red", "penalty", "penaltymiss"];
  ...
  function finishLiveMatch(form) {
    ...
    const nextHomeTeam = String(live.homeTeam || "").trim();
    const nextAwayTeam = String(live.awayTeam || "").trim();
    const teamError = validateFifa10MatchTeams(match, nextHomeTeam, nextAwayTeam);
    ...
    match.updatedAt = new Date().toISOString();
    if (match.phase === "fifa10-group") match.completed = true;
    syncFifa10ResultCompatibility(match);
    const liveState = getLiveState();
  ...
  function renderGroup(group) {
    ...
    view.innerHTML = `
      ...
      <div class="grid-2">
        <section class="panel">
          <div class="panel-header"><div><h3 class="panel-title">Kümülatif Puan Durumu</h3><div class="panel-subtitle">League Phase + ${isGold ? "Altın" : "Gümüş"} Grup sonuçları birlikte hesaplanır.</div></div>${phase2Finished() && !finalChapterIsEnabled() && canEdit() ? `<button class="btn btn-gold" data-action="activate-final-chapter">Final Chapter'ı Hazırla</button>` : finalChapterIsEnabled() ? `<button class="btn btn-blue" data-nav="knockout">Final Chapter'a Git</button>` : ""}</div>
  ...
  function renderKnockout() {
    if (finalChapterIsEnabled()) { renderFinalChapterEngine(); return; }
    const ko = state.current.knockout;
    if (!ko.generated) {
      renderFinalChapterEngine();
      return;
  ...
  function benchmarkClamp(value, min = 0, max = 100) {
    return Math.max(min, Math.min(max, Number(value) || 0));
  }

  function benchmarkNumber(value, digits = 2) {
    const number = Number(value || 0);
    return Number.isFinite(number) ? number.toFixed(digits) : (0).toFixed(digits);
  }

  function benchmarkStdDev(values) {
  ...
  function benchmarkNameKey(name) {
    return normalizeAdministrativeName(String(name || "").trim());
  }

  function benchmarkChampionSet() {
    const champions = new Set();
    const add = name => {
      const key = benchmarkNameKey(name);
      if (key) champions.add(key);
    };
    (historical.champions || []).forEach(row => {
      if (Number(row.titles || 0) > 0) add(row.name);
    });
    (historical.editions || []).forEach(edition => add(edition.champion));
    const finalChapterChampion = state.current.finalChapter?.final?.championId;
    const legacyChampion = state.current.knockout?.championId;
    if (finalChapterChampion) add(playerName(finalChapterChampion));
    if (legacyChampion) add(playerName(legacyChampion));
    return champions;
  }

  function benchmarkHonoursMap() {
    const map = new Map();
    const ensure = name => {
      const key = benchmarkNameKey(name);
      if (!key) return null;
      if (!map.has(key)) map.set(key, { name: String(name || "").trim(), titles: 0, finals: 0, podiums: 0 });
      return map.get(key);
    };
    (historical.champions || []).forEach(row => {
      const record = ensure(row.name);
      if (!record) return;
      record.titles = Math.max(record.titles, Number(row.titles || 0));
      record.finals = Math.max(record.finals, Number(row.finals || 0));
      record.podiums = Math.max(record.podiums, Number(row.podiums || 0));
    });
    const derived = new Map();
    const countDerived = (name, field) => {
      const key = benchmarkNameKey(name);
      if (!key) return;
      if (!derived.has(key)) derived.set(key, { name: String(name || "").trim(), titles: 0, finals: 0, podiums: 0 });
      derived.get(key)[field] += 1;
    };
    (historical.editions || []).forEach(edition => {
      countDerived(edition.champion, "titles"); countDerived(edition.champion, "finals"); countDerived(edition.champion, "podiums");
      countDerived(edition.runnerUp, "finals"); countDerived(edition.runnerUp, "podiums");
      countDerived(edition.third, "podiums");
    });
    derived.forEach(row => {
      const record = ensure(row.name);
      record.titles = Math.max(record.titles, row.titles);
      record.finals = Math.max(record.finals, row.finals);
      record.podiums = Math.max(record.podiums, row.podiums);
    });
    const fc = state.current.finalChapter;
    const finalMatch = fc?.final?.match;
    const championId = fc?.final?.championId;
    if (finalMatch && championId) {
      const winner = ensure(playerName(championId));
      const runnerId = championId === finalMatch.homeId ? finalMatch.awayId : finalMatch.homeId;
      const runner = ensure(playerName(runnerId));
      if (winner) { winner.titles += 1; winner.finals += 1; winner.podiums += 1; }
      if (runner) { runner.finals += 1; runner.podiums += 1; }
    }
    const thirdId = fc?.thirdPlace?.winnerId;
    if (thirdId) {
      const third = ensure(playerName(thirdId));
      if (third) third.podiums += 1;
    }
    if (!championId && state.current.knockout?.championId && state.current.knockout?.final) {
      const legacyFinal = state.current.knockout.final;
      const legacyChampionId = state.current.knockout.championId;
      const winner = ensure(playerName(legacyChampionId));
      const runnerId = legacyChampionId === legacyFinal.homeId ? legacyFinal.awayId : legacyFinal.homeId;
      const runner = ensure(playerName(runnerId));
      if (winner) { winner.titles += 1; winner.finals += 1; winner.podiums += 1; }
      if (runner) { runner.finals += 1; runner.podiums += 1; }
    }
  ...
  function benchmarkStageWeight(stage) {
    const value = String(stage || "").toLocaleLowerCase("en-US");
    if (/third|3rd|üçün/.test(value)) return 1.25;
    if (/semi|yarı/.test(value)) return 1.35;
    if (/quarter|çeyrek/.test(value)) return 1.20;
    if (/grand final|büyük final/.test(value)) return 1.50;
    if (/final/.test(value) && !/chapter/.test(value)) return 1.50;
    if (/knockout|play.?off|son bilet|eleme/.test(value)) return 1.15;
    if (/gold|silver|altın|gümüş/.test(value)) return 1.05;
    return 1;
  }

  function benchmarkMatchResult(match) {
    const homeScore = Number(match.homeScore);
    const awayScore = Number(match.awayScore);
    const homeName = String(match.homeName || "").trim();
    const awayName = String(match.awayName || "").trim();
    if (homeScore > awayScore) return { home: 3, away: 0, winner: homeName, draw: false };
    if (awayScore > homeScore) return { home: 0, away: 3, winner: awayName, draw: false };
    const winnerKey = benchmarkNameKey(match.winnerName);
    if (winnerKey) {
      const homeWins = winnerKey === benchmarkNameKey(homeName);
      return { home: homeWins ? 3 : 0, away: homeWins ? 0 : 3, winner: homeWins ? homeName : awayName, draw: false };
    }
    return { home: 1, away: 1, winner: "", draw: true };
  }

  function benchmarkEmptyRow(name, edition = null) {
    return {
      name, edition, games: 0, wins: 0, draws: 0, losses: 0, points: 0,
      gf: 0, ga: 0, weightedPoints: 0, weightedGames: 0,
      clutchGames: 0, clutchPoints: 0, closeGames: 0, editions: new Map()
    };
  }

  function benchmarkApplySide(row, editionRow, side) {
    for (const target of [row, editionRow]) {
      target.games += 1;
      target.gf += side.gf;
      target.ga += side.ga;
      target.points += side.points;
      target.weightedPoints += side.points * side.weight;
      target.weightedGames += side.weight;
      if (side.points === 3) target.wins += 1;
      else if (side.points === 1) target.draws += 1;
      else target.losses += 1;
      if (side.weight >= 1.20) {
        target.clutchGames += 1;
        target.clutchPoints += side.points;
      }
      if (side.margin <= 1) target.closeGames += 1;
    }
  }

  function buildTournamentBenchmarkAnalytics() {
    const championSet = benchmarkChampionSet();
    const honours = benchmarkHonoursMap();
    const matches = buildUnifiedAllTimeMatches().filter(match => {
      const home = String(match.homeName || "").trim();
      const away = String(match.awayName || "").trim();
      return home && away && !/^P\d+$/i.test(home) && !/^P\d+$/i.test(away) && Number.isFinite(Number(match.homeScore)) && Number.isFinite(Number(match.awayScore));
    });
    ...
    const ensurePlayer = name => {
      const key = benchmarkNameKey(name);
      if (!playerMap.has(key)) playerMap.set(key, benchmarkEmptyRow(String(name || "").trim()));
      return playerMap.get(key);
    };
    const ensureEdition = edition => {
      const number = Number(edition) || 0;
      if (!editionMap.has(number)) editionMap.set(number, { edition: number, label: `FIFA ${number}`, matches: [], players: new Map(), champion: "" });
      return editionMap.get(number);
    };
    const ensureEditionPlayer = (editionRecord, name) => {
      const key = benchmarkNameKey(name);
      if (!editionRecord.players.has(key)) editionRecord.players.set(key, benchmarkEmptyRow(String(name || "").trim(), editionRecord.edition));
      return editionRecord.players.get(key);
    };

    matches.forEach(match => {
      const edition = Number(match.edition) || 0;
      const editionRecord = ensureEdition(edition);
      editionRecord.matches.push(match);
      const result = benchmarkMatchResult(match);
      const weight = benchmarkStageWeight(match.stage);
      const margin = Math.abs(Number(match.homeScore) - Number(match.awayScore));
      const home = ensurePlayer(match.homeName);
      const away = ensurePlayer(match.awayName);
      const homeEdition = ensureEditionPlayer(editionRecord, match.homeName);
      const awayEdition = ensureEditionPlayer(editionRecord, match.awayName);
      benchmarkApplySide(home, homeEdition, { gf: Number(match.homeScore), ga: Number(match.awayScore), points: result.home, weight, margin });
      benchmarkApplySide(away, awayEdition, { gf: Number(match.awayScore), ga: Number(match.homeScore), points: result.away, weight, margin });
      home.editions.set(edition, homeEdition);
      away.editions.set(edition, awayEdition);
    });

    (historical.editions || []).forEach(edition => {
      const record = ensureEdition(edition.edition);
      record.champion = edition.champion || record.champion;
    });
    const currentChampion = state.current.finalChapter?.final?.championId || state.current.knockout?.championId;
    if (currentChampion) ensureEdition(state.current.edition || 9).champion = playerName(currentChampion);

    const finalizeBasic = row => {
      row.gd = row.gf - row.ga;
      row.ppg = row.games ? row.points / row.games : 0;
      row.winRate = row.games ? row.wins / row.games * 100 : 0;
      row.unbeatenRate = row.games ? (row.wins + row.draws) / row.games * 100 : 0;
      row.gfPerGame = row.games ? row.gf / row.games : 0;
      row.gaPerGame = row.games ? row.ga / row.games : 0;
      row.gdPerGame = row.games ? row.gd / row.games : 0;
      row.weightedPPG = row.weightedGames ? row.weightedPoints / row.weightedGames : row.ppg;
      row.clutchPPG = row.clutchGames ? row.clutchPoints / row.clutchGames : row.ppg;
      row.closeRate = row.games ? row.closeGames / row.games * 100 : 0;
      return row;
    };

    const editionRows = [...editionMap.values()].filter(edition => edition.matches.length).sort((a, b) => a.edition - b.edition).map(edition => {
      const players = [...edition.players.values()].map(finalizeBasic);
      const ppgValues = players.map(row => row.ppg);
      const avgPPG = players.length ? players.reduce((sum, row) => sum + row.ppg, 0) / players.length : 0;
      const avgGF = edition.matches.length ? edition.matches.reduce((sum, match) => sum + Number(match.homeScore) + Number(match.awayScore), 0) / edition.matches.length : 0;
      const drawCount = edition.matches.filter(match => benchmarkMatchResult(match).draw).length;
      const avgMargin = edition.matches.length ? edition.matches.reduce((sum, match) => sum + Math.abs(Number(match.homeScore) - Number(match.awayScore)), 0) / edition.matches.length : 0;
      const closeRate = edition.matches.length ? edition.matches.filter(match => Math.abs(Number(match.homeScore) - Number(match.awayScore)) <= 1).length / edition.matches.length * 100 : 0;
      const balanceIndex = Math.round(benchmarkClamp(100 - benchmarkStdDev(ppgValues) / 1.25 * 100));
      const medianPPG = benchmarkMedian(ppgValues);
      players.forEach(row => {
        row.performanceRaw = row.ppg / 3 * 55 + row.winRate / 100 * 25 + benchmarkClamp(50 + row.gdPerGame * 18) / 100 * 20;
      });
      const ranked = [...players].sort((a, b) => b.performanceRaw - a.performanceRaw || b.games - a.games || a.name.localeCompare(b.name, "tr"));
      ranked.forEach((row, index) => {
        row.percentile = ranked.length <= 1 ? 50 : (ranked.length - 1 - index) / (ranked.length - 1) * 100;
        row.relativePPG = row.ppg - avgPPG;
        row.editionAveragePPG = avgPPG;
      });
      const eligible = ranked.filter(row => !championSet.has(benchmarkNameKey(row.name)));
      return {
        ...edition,
        players: ranked,
        playerMap: new Map(ranked.map(row => [benchmarkNameKey(row.name), row])),
        avgPPG, medianPPG, goalsPerMatch: avgGF, drawRate: edition.matches.length ? drawCount / edition.matches.length * 100 : 0,
        avgMargin, closeRate, balanceIndex, nonChampionLeader: eligible[0] || null
      };
    });
    const editionAnalyticsMap = new Map(editionRows.map(row => [row.edition, row]));

    const allRows = [...playerMap.values()].map(row => {
      finalizeBasic(row);
      const editions = [...row.editions.entries()].sort((a, b) => a[0] - b[0]).map(([edition, raw]) => {
        const finalized = finalizeBasic(raw);
        const environment = editionAnalyticsMap.get(edition);
        const official = environment?.playerMap.get(benchmarkNameKey(row.name));
        return {
          ...finalized,
          percentile: official?.percentile ?? 50,
          relativePPG: official?.relativePPG ?? 0,
          editionAveragePPG: official?.editionAveragePPG ?? 0,
          balanceIndex: environment?.balanceIndex ?? 0
        };
      });
      const percentiles = editions.map(item => item.percentile);
      const eraIndex = percentiles.length ? percentiles.reduce((sum, value) => sum + value, 0) / percentiles.length : 0;
      const rawConsistency = benchmarkClamp(100 - benchmarkStdDev(percentiles) * 1.65);
      const consistencySample = editions.length <= 1 ? 0 : Math.min(1, (editions.length - 1) / 3);
      const consistency = editions.length <= 1 ? 50 : 50 + (rawConsistency - 50) * consistencySample;
      const peakEdition = editions.length ? [...editions].sort((a, b) => b.percentile - a.percentile || b.ppg - a.ppg)[0] : null;
      const aboveAverage = editions.filter(item => item.relativePPG >= 0).length;
      const globalAveragePPG = matches.length ? [...playerMap.values()].reduce((sum, item) => sum + item.points, 0) / Math.max(1, matches.length * 2) : 1.5;
      const adjustedPPG = (row.points + globalAveragePPG * 5) / (row.games + 5);
      const reliability = 1 - Math.exp(-row.games / 18);
      const gdIndex = benchmarkClamp(50 + row.gdPerGame * 20);
      const rawScore = adjustedPPG / 3 * 100 * .27 + row.winRate * .18 + gdIndex * .16 + row.clutchPPG / 3 * 100 * .16 + eraIndex * .13 + consistency * .10;
      const benchmarkScore = rawScore * (.78 + reliability * .22);
      const versatility = editions.length ? benchmarkClamp((editions.length / Math.max(1, editionRows.length) * 100) * .58 + (aboveAverage / editions.length * 100) * .42) : 0;
      const firstEdition = editions[0] || null;
      const lastEdition = editions[editions.length - 1] || null;
      const momentum = editions.length > 1 ? lastEdition.ppg - firstEdition.ppg : 0;
      const honour = honours.get(benchmarkNameKey(row.name)) || { titles: 0, finals: 0, podiums: 0 };
      return {
        ...row, editions, editionCount: editions.length, eraIndex, consistency, peakEdition, aboveAverage,
        adjustedPPG, reliability, benchmarkScore, versatility, momentum,
        finals: Number(honour.finals || 0), podiums: Number(honour.podiums || 0), titles: Number(honour.titles || 0)
      };
    });

    const players = allRows.filter(row => !championSet.has(benchmarkNameKey(row.name))).sort((a, b) => b.benchmarkScore - a.benchmarkScore || b.games - a.games || a.name.localeCompare(b.name, "tr"));
    players.forEach((row, index) => row.benchmarkRank = index + 1);
    const metricDefinitions = {
      benchmark: { label: intelligenceCopy("Benchmark", "Benchmark"), short: "BMX", description: intelligenceCopy("Maç başı üretim, büyük maç, dönem uyumu ve istikrar", "Per-match production, big matches, era fit and consistency"), value: row => row.benchmarkScore, format: row => benchmarkNumber(row.benchmarkScore, 1) },
      ppg: { label: intelligenceCopy("Puan / Maç", "Points / Match"), short: "PPG", description: intelligenceCopy("Bütün sonuçların standart 3-1-0 ortalaması", "Standard 3-1-0 average across every result"), value: row => row.ppg, format: row => benchmarkNumber(row.ppg, 2) },
      clutch: { label: intelligenceCopy("Büyük Maç", "Big Match"), short: "CLUTCH", description: intelligenceCopy("Çeyrek final ve sonrası maç başı puan", "Points per match from quarter-finals onward"), value: row => row.clutchPPG, format: row => benchmarkNumber(row.clutchPPG, 2) },
      consistency: { label: intelligenceCopy("İstikrar", "Consistency"), short: "CONS", description: intelligenceCopy("Turnuvalar arası performans sapmasının düşüklüğü", "Low performance variance between tournaments"), value: row => row.consistency, format: row => benchmarkNumber(row.consistency, 0) },
      era: { label: intelligenceCopy("Dönem Üstünlüğü", "Era Dominance"), short: "ERA", description: intelligenceCopy("Her turnuvanın kendi ortamındaki yüzdelik dilim", "Percentile rank inside each tournament environment"), value: row => row.eraIndex, format: row => benchmarkNumber(row.eraIndex, 0) }
    };
    const selectedMetric = metricDefinitions[tournamentBenchmarkMetric] || metricDefinitions.benchmark;
    const sortedPlayers = [...players].sort((a, b) => selectedMetric.value(b) - selectedMetric.value(a) || b.games - a.games || a.name.localeCompare(b.name, "tr"));
    sortedPlayers.forEach((row, index) => row.metricRank = index + 1);

    const mostImproved = players.filter(row => row.editionCount >= 2).sort((a, b) => b.momentum - a.momentum || b.games - a.games)[0] || null;
    const almostChampion = [...players].sort((a, b) => b.finals - a.finals || b.podiums - a.podiums || b.benchmarkScore - a.benchmarkScore)[0] || null;
    const mostBalancedEdition = [...editionRows].sort((a, b) => b.balanceIndex - a.balanceIndex || b.closeRate - a.closeRate)[0] || null;
    const highestTempoEdition = [...editionRows].sort((a, b) => b.goalsPerMatch - a.goalsPerMatch)[0] || null;

    return {
      players, sortedPlayers, allRows, editionRows, championSet, metricDefinitions, selectedMetric,
      records: {
        benchmark: players[0] || null,
        ppg: [...players].sort((a, b) => b.ppg - a.ppg || b.games - a.games)[0] || null,
        clutch: [...players].filter(row => row.clutchGames > 0).sort((a, b) => b.clutchPPG - a.clutchPPG || b.clutchGames - a.clutchGames)[0] || null,
        consistency: [...players].filter(row => row.editionCount >= 2).sort((a, b) => b.consistency - a.consistency || b.editionCount - a.editionCount)[0] || null,
        mostImproved, almostChampion, mostBalancedEdition, highestTempoEdition
      },
      summary: {
        matches: matches.length,
        players: players.length,
        championsExcluded: championSet.size,
        editions: editionRows.length,
        avgGames: players.length ? players.reduce((sum, row) => sum + row.games, 0) / players.length : 0
      }
    };
  }

  function tournamentBenchmarkIdentity(row) {
    if (!row) return [];
    const tags = [];
    if (row.clutchGames >= 2 && row.clutchPPG >= 2.1) tags.push(intelligenceCopy("Büyük Maç Oyuncusu", "Big-Game Player"));
    if (row.consistency >= 78 && row.editionCount >= 2) tags.push(intelligenceCopy("İstikrar Ustası", "Consistency Master"));
    if (row.momentum >= .45) tags.push(intelligenceCopy("Yükselişte", "On the Rise"));
    if (row.gfPerGame >= 3.25) tags.push(intelligenceCopy("Yüksek Tempo", "High Tempo"));
    if (row.gaPerGame <= 2.15) tags.push(intelligenceCopy("Kontrollü Oyun", "Controlled Game"));
    if (row.versatility >= 70) tags.push(intelligenceCopy("Dönemler Üstü", "Cross-Era"));
    if (!tags.length) tags.push(intelligenceCopy("Dengeli Rakip", "Balanced Contender"));
    return tags.slice(0, 3);
  }

  function tournamentBenchmarkLeaderCard(label, row, value, note, code) {
    if (!row) return `<article class="tb-leader-card empty"><span>${escapeHTML(code)}</span><h3>–</h3><p>${escapeHTML(note)}</p></article>`;
    return `<article class="tb-leader-card"><span>${escapeHTML(code)}</span><small>${escapeHTML(label)}</small><h3>${escapeHTML(row.name)}</h3><strong>${escapeHTML(value)}</strong><p>${escapeHTML(note)}</p></article>`;
  }

  function tournamentBenchmarkMetricTabs(analytics) {
    return `<div class="tb-metric-tabs">${Object.entries(analytics.metricDefinitions).map(([key, definition]) => `<button class="${tournamentBenchmarkMetric === key ? "active" : ""}" data-action="set-tournament-benchmark-metric" data-benchmark-metric="${key}"><span>${escapeHTML(definition.short)}</span><strong>${escapeHTML(definition.label)}</strong><small>${escapeHTML(definition.description)}</small></button>`).join("")}</div>`;
  }
  ...
  function tournamentBenchmarkPlayerPanel(analytics) {
    ...
    if (!tournamentBenchmarkSelectedPlayer || !analytics.players.some(row => row.name === tournamentBenchmarkSelectedPlayer)) tournamentBenchmarkSelectedPlayer = analytics.players[0].name;
    const row = analytics.players.find(item => item.name === tournamentBenchmarkSelectedPlayer) || analytics.players[0];
    const identity = tournamentBenchmarkIdentity(row);
    const maxPPG = Math.max(3, ...row.editions.map(item => item.ppg));
    return `<section class="panel tb-player-lab">
      <div class="panel-header"><div><div class="eyebrow">PLAYER PERFORMANCE LAB</div><h3 class="panel-title">${intelligenceCopy("Oyuncu Turnuva Profili", "Player Tournament Profile")}</h3><div class="panel-subtitle">${intelligenceCopy("Aynı oyuncunun farklı turnuva ortamlarındaki maç başı seviyesi.", "The same player's per-match level across different tournament environments.")}</div></div><label class="tb-player-select"><span>${intelligenceCopy("Oyuncu", "Player")}</span><select id="tournamentBenchmarkPlayerSelect">${analytics.players.map(item => `<option value="${escapeHTML(item.name)}" ${item.name === row.name ? "selected" : ""}>${escapeHTML(item.name)}</option>`).join("")}</select></label></div>
      <div class="tb-player-hero"><div class="tb-player-rank"><span>#${row.benchmarkRank}</span><small>NO-CROWN RANK</small></div><div><h3>${escapeHTML(row.name)}</h3><div class="tb-identity-tags">${identity.map(tag => `<span>${escapeHTML(tag)}</span>`).join("")}</div></div><div class="tb-player-score"><strong>${benchmarkNumber(row.benchmarkScore, 1)}</strong><small>BENCHMARK</small></div></div>
      <div class="tb-player-kpis"><div><span>PPG</span><strong>${benchmarkNumber(row.ppg, 2)}</strong><small>${row.points} ${intelligenceCopy("puan", "points")} · ${row.games} ${intelligenceCopy("maç", "matches")}</small></div><div><span>CLUTCH</span><strong>${benchmarkNumber(row.clutchPPG, 2)}</strong><small>${row.clutchGames} ${intelligenceCopy("büyük maç", "big matches")}</small></div><div><span>ERA INDEX</span><strong>${benchmarkNumber(row.eraIndex, 0)}</strong><small>${row.aboveAverage}/${row.editionCount} ${intelligenceCopy("turnuvada ortalama üstü", "editions above average")}</small></div><div><span>CONSISTENCY</span><strong>${benchmarkNumber(row.consistency, 0)}</strong><small>${row.editionCount} ${intelligenceCopy("turnuva örneklemi", "edition sample")}</small></div></div>
      <div class="tb-edition-track">${row.editions.map(item => {
        const width = Math.max(4, item.ppg / maxPPG * 100);
        const relative = item.relativePPG;
        return `<article class="${relative >= 0 ? "above" : "below"}"><header><span>FIFA ${String(item.edition).padStart(2, "0")}</span><strong>${benchmarkNumber(item.ppg, 2)} PPG</strong></header><div class="tb-track"><i style="width:${width}%"></i><b style="left:${Math.min(100, item.editionAveragePPG / maxPPG * 100)}%"></b></div><footer><span>${item.games} ${intelligenceCopy("maç", "matches")}</span><em>${relative >= 0 ? "+" : ""}${benchmarkNumber(relative, 2)} ${intelligenceCopy("turnuva ort.", "vs edition avg")}</em><strong>${benchmarkNumber(item.percentile, 0)}p</strong></footer></article>`;
      }).join("")}</div>
      <div class="tb-profile-footer"><div><span>${intelligenceCopy("Zirve Turnuvası", "Peak Edition")}</span><strong>${row.peakEdition ? `FIFA ${String(row.peakEdition.edition).padStart(2, "0")}` : "–"}</strong><small>${row.peakEdition ? `${benchmarkNumber(row.peakEdition.ppg, 2)} PPG · ${benchmarkNumber(row.peakEdition.percentile, 0)}. ${intelligenceCopy("yüzdelik", "percentile")}` : "–"}</small></div><div><span>${intelligenceCopy("Dönem Hareketi", "Era Movement")}</span><strong>${row.momentum > .15 ? intelligenceCopy("Yükseliş", "Rising") : row.momentum < -.15 ? intelligenceCopy("Gerileme", "Declining") : intelligenceCopy("Dengeli", "Stable")}</strong><small>${row.momentum >= 0 ? "+" : ""}${benchmarkNumber(row.momentum, 2)} PPG</small></div><div><span>${intelligenceCopy("Kupa Yakınlığı", "Near-Crown Record")}</span><strong>${row.finals} ${intelligenceCopy("final", "finals")}</strong><small>${row.podiums} ${intelligenceCopy("podyum · şampiyonluk yok", "podiums · no title")}</small></div></div>
    </section>`;
  }

  function tournamentEditionCard(edition) {
    const tempo = edition.goalsPerMatch >= 7 ? intelligenceCopy("Yüksek tempo", "High tempo") : edition.goalsPerMatch <= 4.5 ? intelligenceCopy("Kontrollü", "Controlled") : intelligenceCopy("Dengeli", "Balanced");
    return `<article class="tb-edition-card"><header><div><span>EDITION</span><strong>${String(edition.edition).padStart(2, "0")}</strong></div><em>${escapeHTML(tempo)}</em></header><h3>${edition.nonChampionLeader ? escapeHTML(edition.nonChampionLeader.name) : "–"}</h3><p>${intelligenceCopy("Şampiyon hariç turnuva lideri", "Edition leader excluding champion")}</p><div class="tb-edition-metrics"><div><span>GOL/M</span><strong>${benchmarkNumber(edition.goalsPerMatch, 2)}</strong></div><div><span>${intelligenceCopy("Yakın Maç", "Close Games")}</span><strong>${benchmarkNumber(edition.closeRate, 0)}%</strong></div><div><span>${intelligenceCopy("Denge", "Balance")}</span><strong>${edition.balanceIndex}</strong></div><div><span>${intelligenceCopy("Fark/M", "Margin/M")}</span><strong>${benchmarkNumber(edition.avgMargin, 2)}</strong></div></div><footer><span>${edition.matches.length} ${intelligenceCopy("maç", "matches")}</span><span>${edition.players.length} ${intelligenceCopy("oyuncu", "players")}</span><span>${benchmarkNumber(edition.drawRate, 1)}% ${intelligenceCopy("beraberlik", "draws")}</span></footer></article>`;
  }

  function tournamentBenchmarkHeatmap(analytics) {
    const editions = analytics.editionRows;
    const rows = analytics.players;
    if (!rows.length || !editions.length) return "";
    return `<section class="panel tb-heatmap-panel"><div class="panel-header"><div><div class="eyebrow">CROSS-EDITION MATRIX</div><h3 class="panel-title">${intelligenceCopy("Turnuvalar Arası Performans Haritası", "Cross-Tournament Performance Map")}</h3><div class="panel-subtitle">${intelligenceCopy("Hücrelerde PPG görünür; renk, oyuncunun o turnuva ortalamasına göre konumunu gösterir.", "Cells show PPG; colour represents position versus that edition's average.")}</div></div><div class="tb-heat-legend"><span class="down">−</span><small>${intelligenceCopy("Ortalama", "Average")}</small><span class="up">+</span></div></div><div class="tb-heatmap-scroll"><div class="tb-heatmap" style="--tb-editions:${editions.length}"><div class="tb-heat-head"><strong>${intelligenceCopy("Oyuncu", "Player")}</strong>${editions.map(edition => `<span>F${String(edition.edition).padStart(2, "0")}</span>`).join("")}</div>${rows.map(row => `<div class="tb-heat-row"><strong>${escapeHTML(row.name)}</strong>${editions.map(edition => {
      const item = row.editions.find(value => value.edition === edition.edition);
      if (!item) return `<span class="empty">–</span>`;
      const intensity = Math.min(1, Math.abs(item.relativePPG) / 1.15);
      return `<span class="${item.relativePPG >= 0 ? "positive" : "negative"}" style="--tb-intensity:${intensity.toFixed(2)}" title="FIFA ${edition.edition}: ${benchmarkNumber(item.ppg, 2)} PPG · ${item.relativePPG >= 0 ? "+" : ""}${benchmarkNumber(item.relativePPG, 2)} vs avg">${benchmarkNumber(item.ppg, 2)}</span>`;
  ...
  function renderTournamentPlayerAtlas(analytics) {
    ...
    return `<section class="tb-hero"><div class="tb-hero-grid"></div><div class="tb-hero-copy"><div class="eyebrow">FIFA 1–9 · PERFORMANCE ATLAS</div><h2>${intelligenceCopy("Şampiyonsuz Taht", "The No-Crown Throne")}</h2><p>${intelligenceCopy("Şampiyon olmuş oyuncular sıralamadan çıkarıldı. Geriye kalan bütün oyuncular; toplam hacimle değil, maç başı üretim, turnuva içi yüzdelik, büyük maç seviyesi ve dönemler arası istikrarla karşılaştırılıyor.", "Every champion is removed from the ranking. Remaining players are compared through per-match production, tournament percentile, big-match level and cross-era consistency—not raw volume.")}</p><div class="tb-hero-tags"><span>CHAMPIONS EXCLUDED</span><span>PER-MATCH NORMALIZED</span><span>ERA ADJUSTED</span><span>LIVE FIFA 09</span></div></div><div class="tb-hero-mark"><span>NO</span><strong>CROWN</strong><small>PERFORMANCE LEAGUE</small></div></section>
      ...
      ${tournamentBenchmarkPlayerPanel(analytics)}
  ...
  function renderLiveStatsScopeControls() {
    if (Number(seasonSystem().activeEdition || 10) >= 10 || !finalChapterIsEnabled()) return "";
    const scopes = [
  ...
  function renderLiveStatistics() {
    ...
    if (Number(seasonSystem().activeEdition || 10) < 10 && !state.current.league.generated) {
  ...
  function renderLivePlayerCard(row) {
    ...
    const primaryValue = fifa10Active ? row.ppg.toFixed(3) : row.points;
    const primaryLabel = fifa10Active ? `PPG (${row.points})` : "PERF P";
  ...
  function livePerformanceTable(rows) {
    const fifa10Active = Number(seasonSystem().activeEdition || 10) >= 10;
    return `<div class="table-wrap"><table><thead><tr><th>#</th><th class="player-col">Oyuncu</th><th>O</th><th>G</th><th>B</th><th>M</th><th>AG</th><th>YG</th><th>AV</th><th>${fifa10Active ? "PPG (P)" : "Perf P"}</th><th>${fifa10Active ? "Grup" : "PPG"}</th><th>Form</th></tr></thead><tbody>${rows.map(row => `<tr><td>${row.rank}</td><td class="player-col"><span class="player-name">${escapeHTML(row.name)}</span><small class="table-subline">${escapeHTML(row.officialPosition || "")}</small></td><td>${row.games}</td><td>${row.wins}</td><td>${row.draws}</td><td>${row.losses}</td><td>${row.gf}</td><td>${row.ga}</td><td class="${row.gd > 0 ? "gd-positive" : row.gd < 0 ? "gd-negative" : ""}">${formatGD(row.gd)}</td><td class="points-cell">${fifa10Active ? `${row.ppg.toFixed(3)} (${row.points})` : row.points}</td><td>${fifa10Active ? escapeHTML(row.officialPosition?.replace("FIFA 10 · ", "") || "–") : row.ppg.toFixed(2)}</td><td>${formHTML(row.form)}</td></tr>`).join("")}</tbody></table></div>`;
  }
  ...
  function buildLiveTournamentAnalytics(scope = liveStatsScope) {
    ...
    const fifa10Active = Number(seasonSystem().activeEdition || 10) >= 10;
    const phaseOrder = { "fifa10-group": 1, league: 1, gold: 2, silver: 3, knockout: 4, final: 5 };

    const activePlayers = fifa10Active ? fifa10IntegratedParticipants() : filledParticipants();
    ...
    if (fifa10Active) {
      fifa10IntegratedParticipants().forEach(player => officialMap.set(player.id, `FIFA 10 · Grup ${player.group || "–"}`));
    } else if (state.current.phase2.generated) {
    ...
    const players = [...playerRows.values()].map(row => {
      ...
      ? b.ppg-a.ppg || b.points-a.points || b.gd-a.gd || b.gf-a.gf || b.wins-a.wins || a.name.localeCompare(b.name,"tr")
    ...
    return {
      ...
      records: {
        pointsLeader:fifa10Active ? players.find(row => row.games > 0) || null : maxBy(players,row=>row.points,row=>row.games>0),
  ...
  function buildFormAnalytics(windowSize = 20, scope = "current") {
    ...
    const currentNames = activeCompetitionPlayerNames();
  ...
  function buildFormMatchHistory() {
    ...
    const historicalEditions = new Set((historical.editions || []).map(item => Number(item.edition)));
    const archivedMatches = archivedSeasonMatches()
      .filter(match => !historicalEditions.has(Number(match.edition)) && Number(match.edition) !== Number(seasonSystem().activeEdition || 10))
      .map((match, index) => ({
        ...match,
        isFinalChapter: Number(match.edition) === 9,
        chapterStage: "",
    ...
    const currentMatches = currentCompleted.map((match,index) => {
      ...
      return {
        id:match.id,
        edition:Number(match.edition || seasonSystem().activeEdition || 10),
        editionLabel:`FIFA ${Number(match.edition || seasonSystem().activeEdition || 10)}`,
        stage:currentMatchStageLabel(match),
  ...
  async function shareFormStatistics() {
    ...
    const lines = [
      `FIFA ${seasonSystem().activeEdition || 10} · SON ${formWindowSize} MAÇ FORM DURUMU`,
    ...
    const shareData = { title:`FIFA ${seasonSystem().activeEdition || 10} Son ${formWindowSize} Form`, text:lines.join("\n"), url:window.location.href };
    if (navigator.share) {
      try { await navigator.share(shareData); return; } catch (error) { if (error?.name === "AbortError") return; }
    }
    window.open(`https://wa.me/?text=${encodeURIComponent(lines.join("\n"))}`, "_blank", "noopener,noreferrer");
  ...
  function oddsStageKey(match) {
    if (match.phase === "fifa10-group") return "league";
    if (match.phase === "final" || match.phase === "knockout") return "knockout";
    return match.phase || "league";
  }

  function oddsStageLabel(match) {
    if (match.phase === "fifa10-group") return `FIFA 10 · Grup ${match.group || "–"} · ${match.leg || match.round || 1}. Devre · ${match.stars || "–"}★`;
  ...
  function intelligenceNames() {
    const roster = activeCompetitionPlayerNames();
  ...
  function simulationFingerprint() {
    ...
    return JSON.stringify({
      edition: seasonSystem().activeEdition || 10,
      players: activeCompetitionPlayerNames(),
  ...
  function playerCardMetricLabel(key) {
    const labels = {
      ...
      mental:intelligenceCopy("Mental / Baskı","Mental / Pressure"),
      teamUsage:intelligenceCopy("Takım Kullanımı","Team Usage"),
      form:intelligenceCopy("Güncel Form","Current Form"),
      legacy:intelligenceCopy("Miras / İtibar","Legacy / Reputation"),
      goalProduction:intelligenceCopy("Gol Üretimi","Goal Production"),
      scoringEfficiency:intelligenceCopy("Skor Verimliliği","Scoring Efficiency"),
      bigScorePotential:intelligenceCopy("Büyük Skor Potansiyeli","Big-Score Potential"),
      strongOpponentAttack:intelligenceCopy("Güçlü Rakibe Karşı Hücum","Attack vs Strong Opponents"),
      underdogAttack:intelligenceCopy("Underdog Hücum Gücü","Underdog Attack"),
      scoringConsistency:intelligenceCopy("Kaliteli Skor İstikrarı","Quality Scoring Consistency"),
      favouriteProduction:intelligenceCopy("Favoriyken Hücum Üretimi","Attack as Favourite"),
      highOutputFrequency:intelligenceCopy("Yüksek Üretim Sıklığı","High-Output Frequency"),
      goalPrevention:intelligenceCopy("Gol Önleme","Goal Prevention"),
      expectedDefence:intelligenceCopy("Beklentiye Göre Savunma","Defence vs Expectation"),
      strongOpponentDefence:intelligenceCopy("Güçlü Rakibe Karşı Savunma","Defence vs Strong Opponents"),
      favouriteControl:intelligenceCopy("Favoriyken Maç Kontrolü","Control as Favourite"),
      collapsePrevention:intelligenceCopy("Çöküş Önleme","Collapse Prevention"),
      lossLimitation:intelligenceCopy("Mağlubiyet Sınırlandırma","Loss Limitation"),
      lowConcedeRate:intelligenceCopy("Düşük Gol Yeme Oranı","Low-Concede Rate"),
      defensiveConsistency:intelligenceCopy("Kaliteli Savunma İstikrarı","Quality Defensive Consistency"),
      strongOpponentPerformance:intelligenceCopy("Güçlü Rakip Performansı","Strong-Opponent Performance"),
      weakOpponentDelivery:intelligenceCopy("Zayıf Rakibe Karşı Beklentiyi Karşılama","Delivery vs Weaker Opponents"),
      evenMatchPerformance:intelligenceCopy("Eşit Güçte Maç Performansı","Even-Match Performance"),
      underdogPerformance:intelligenceCopy("Underdog Performansı","Underdog Performance"),
      favouriteResponsibility:intelligenceCopy("Favori Sorumluluğu","Favourite Responsibility"),
      knockoutPerformance:intelligenceCopy("Play-off / Eleme Performansı","Play-off / Knockout Performance"),
      finalPerformance:intelligenceCopy("Final Performansı","Final Performance"),
      semiFinalPerformance:intelligenceCopy("Yarı Final Performansı","Semi-final Performance"),
      championshipImpact:intelligenceCopy("Şampiyona Etkisi","Championship Impact"),
      bigMatchPerformance:intelligenceCopy("Büyük Maç Performansı","Big-Match Performance"),
      leaguePerformance:intelligenceCopy("Lig Aşaması Performansı","League-Stage Performance"),
      groupPerformance:intelligenceCopy("İkinci Aşama Performansı","Second-Stage Performance"),
      closeMatchManagement:intelligenceCopy("Yakın Maç Yönetimi","Close-Match Management"),
      pressureManagement:intelligenceCopy("Baskı Yönetimi","Pressure Management"),
      comebackPower:intelligenceCopy("Geri Dönüş Gücü","Comeback Power"),
      bounceBack:intelligenceCopy("Mağlubiyet Sonrası Tepki","Bounce-Back Response"),
      streakStrength:intelligenceCopy("Seri Gücü","Streak Strength"),
      focus:intelligenceCopy("Odaklanma","Focus"),
      criticalMentality:intelligenceCopy("Kritik Maç Mentalitesi","Critical-Match Mentality"),
      weakTeamEfficiency:intelligenceCopy("Zayıf Takımla Verim","Efficiency with Weaker Teams"),
      strongTeamEfficiency:intelligenceCopy("Güçlü Takımla Verim","Efficiency with Strong Teams"),
      teamDiversity:intelligenceCopy("Başarılı Takım Çeşitliliği","Successful Team Diversity"),
      selectionEfficiency:intelligenceCopy("Takım Seçimi Verimliliği","Team-Selection Efficiency"),
      independentImpact:intelligenceCopy("Takımdan Bağımsız Etki","Team-Independent Impact"),
      balancePerformance:intelligenceCopy("Takım Güç Dengesi Performansı","Team-Balance Performance"),
      crossTeamConsistency:intelligenceCopy("Takımlar Arası İstikrar","Cross-Team Consistency"),
      fifaPerformance:intelligenceCopy("Turnuva Performansı","Edition Performance"),
      fifa9Performance:intelligenceCopy("FIFA 9 Performansı","FIFA 9 Performance"),
      careerPerformance:intelligenceCopy("Kariyer Performansı","Career Performance"),
      currentAbility:intelligenceCopy("Current Ability","Current Ability"),
      currentForm:intelligenceCopy("Current Form","Current Form"),
      resultPerformance:intelligenceCopy("Sonuç Performansı","Result Performance"),
      expectationPerformance:intelligenceCopy("Beklenti Üzeri Performans","Performance Above Expectation"),
      opponentQuality:intelligenceCopy("Rakip Kalitesi","Opponent Quality"),
      ...
      historicStrong:intelligenceCopy("Tarihî Güçlü Rakip Başarısı","Historic Strong-Opponent Success"),
  ...
  function playerCardReferenceLabel(score) {
    const value = Number(score) || 0;
    if (value >= 90) return intelligenceCopy("Tarihî / Efsanevi","Historic / Legendary");
    ...
    if (value >= 30) return intelligenceCopy("Zayıf","Weak");
    return intelligenceCopy("Çok Zayıf","Very Weak");
  }

  function playerCardFormLabel(score, trend = 0) {
    const value = Number(score) || 0;
    if (value >= 84) return intelligenceCopy("Alev Alev","Red Hot");
    if (value >= 74) return intelligenceCopy("Çok Formda","Excellent Form");
    if (value >= 64) return intelligenceCopy("Formda","In Form");
    if (value >= 54) return trend > 4 ? intelligenceCopy("Yükselişte","Improving") : intelligenceCopy("Dengeli","Steady");
    if (value >= 44) return trend < -4 ? intelligenceCopy("Düşüşte","Declining") : intelligenceCopy("Dalgalı","Inconsistent");
  ...
  function playerCardClamp(value, min = 0, max = 100) {
    return intelligenceClamp(Number(value) || 0,min,max);
  }
  ...
  function playerCardWeightedAvailable(items, fallback = 50) {
    let sum = 0;
    let weight = 0;
    (items || []).forEach(item=>{
      if (!item || item.value == null || !Number.isFinite(Number(item.value))) return;
      const itemWeight = Number(item.weight) || 1;
      sum += Number(item.value) * itemWeight;
      weight += itemWeight;
    });
    return weight ? sum/weight : fallback;
  }

  function playerCardShrink(value, sample, anchor = 55, halfLife = 8) {
    const count = Math.max(0,Number(sample)||0);
    const weight = count/(count+halfLife);
    return playerCardClamp(anchor+((Number(value)||anchor)-anchor)*weight,5,94);
  }
  ...
  function playerCardPercentile(values, value) {
    ...
    return list.filter(item=>item<=value).length/list.length;
  }

  function playerCardCalibrate(values, value, mean = 60, targetSd = 10, minimum = 25, maximum = 94) {
    const list = (values || []).map(Number).filter(Number.isFinite);
    const average = playerCardMean(list,value);
    const sd = playerCardStd(list) || 8;
    return playerCardClamp(mean+((Number(value)||average)-average)/sd*targetSd,minimum,maximum);
  }

  function playerCardValidTeam(name) {
    const value = normalizeTeamName(name || "");
    return Boolean(value) && !/^(no team|n\/a|none|unknown|takım yok|team yok|-|—)$/i.test(value);
  }

  function playerCardRadar(values) {
    const keys = ["attack","defense","competition","mental","teamUsage","form"];
    const centre = 120;
    const radius = 83;
    const point = (key,index,scale=1) => {
      const angle = -Math.PI/2+index*Math.PI*2/keys.length;
      const value = playerCardClamp(values[key])/100*scale;
      return `${(centre+Math.cos(angle)*radius*value).toFixed(1)},${(centre+Math.sin(angle)*radius*value).toFixed(1)}`;
    };
    const grids = [1,.75,.5,.25].map(scale=>{
      ...
      return `<polygon points="${points}" class="player-card-radar-grid"></polygon>`;
    }).join("");
    const axes = keys.map((key,index)=>{
      const angle = -Math.PI/2+index*Math.PI*2/keys.length;
      const x = centre+Math.cos(angle)*radius;
      const y = centre+Math.sin(angle)*radius;
      const lx = centre+Math.cos(angle)*(radius+25);
      const ly = centre+Math.sin(angle)*(radius+25);
      return `<line x1="${centre}" y1="${centre}" x2="${x}" y2="${y}"></line><text x="${lx}" y="${ly}" text-anchor="middle">${escapeHTML(playerCardMetricLabel(key).toUpperCase())}</text>`;
    }).join("");
    return `<svg class="player-card-radar" viewBox="0 0 240 240"><g>${grids}${axes}</g><polygon points="${keys.map((key,index)=>point(key,index)).join(" ")}" class="player-card-radar-value"></polygon></svg>`;
  }

  function buildPlayerCardTeamPowerModel(matches, elo) {
    const rows = new Map();
    const recordMap = elo.recordMap || new Map();
    const ensure = (key,team,edition) => {
      if (!rows.has(key)) rows.set(key,{team,edition,games:0,points:0,gf:0,ga:0,residual:0,players:new Set()});
      return rows.get(key);
    };
    matches.forEach(match=>{
      const homeTeam = normalizeTeamName(match.homeTeam || "");
      const awayTeam = normalizeTeamName(match.awayTeam || "");
      if (!playerCardValidTeam(homeTeam) || !playerCardValidTeam(awayTeam)) return;
      const record = recordMap.get(match.id);
      const expected = record?.expectedHome ?? .5;
      const actual = match.homeScore>match.awayScore ? 1 : match.homeScore===match.awayScore ? .5 : 0;
      const expectedMargin = (expected-.5)*5;
      const marginResidual = Math.tanh(((match.homeScore-match.awayScore)-expectedMargin)/4);
      const effect = (actual-expected)*.72+marginResidual*.28;
      [
        [homeTeam,match.homeName,match.homeScore,match.awayScore,actual,effect],
        [awayTeam,match.awayName,match.awayScore,match.homeScore,actual===.5?.5:1-actual,-effect]
      ].forEach(([team,player,gf,ga,outcome,residual])=>{
        const row = ensure(`${match.edition}|${team}`,team,Number(match.edition)||0);
        row.games += 1;
        row.gf += Number(gf)||0;
        row.ga += Number(ga)||0;
        row.residual += Number(residual)||0;
        row.players.add(player);
        row.points += outcome===1 ? 3 : outcome===.5 ? 1 : 0;
      });
    });
    const rawValues = [...rows.values()].map(row=>50+(row.residual/Math.max(1,row.games))*34+(((row.points/Math.max(1,row.games))-1.5)*6)+(((row.gf-row.ga)/Math.max(1,row.games))*1.5));
    const rawMean = playerCardMean(rawValues,50);
    const rawSd = playerCardStd(rawValues)||7;
    const model = new Map();
    rows.forEach((row,key)=>{
      const raw = 50+(row.residual/Math.max(1,row.games))*34+(((row.points/Math.max(1,row.games))-1.5)*6)+(((row.gf-row.ga)/Math.max(1,row.games))*1.5);
      const calibrated = playerCardClamp(52+(raw-rawMean)/rawSd*8,30,80);
      const weight = row.games/(row.games+6);
      model.set(key,{...row,score:50+(calibrated-50)*weight,confidence:Math.round(playerCardClamp(25+row.games*7+row.players.size*3,20,95))});
    });
    return {
      get(team,edition) {
        const normalized = normalizeTeamName(team || "");
        if (!playerCardValidTeam(normalized)) return {score:50,confidence:0,games:0,known:false};
        const row = model.get(`${edition}|${normalized}`);
        return row ? {...row,known:true} : {score:50,confidence:10,games:0,known:false};
      }
    };
  }
  ...
  function buildPlayerIdentityCards() {
    ...
    const pressure = buildPressureChamber();
    const power = buildPowerExchange();
    const formAnalytics = buildFormAnalytics(20,"all");
    const teamAnalytics = buildTeamAnalytics();
    const achMap = new Map(achievements.players.map(row=>[row.name,row]));
    const pressureMap = pressure.playerMap || new Map();
    const powerMap = new Map((power.players||[]).map(row=>[row.name,row]));
    const teamMap = new Map((teamAnalytics.players||[]).map(row=>[row.name,row]));
    const teamModel = buildPlayerCardTeamPowerModel(matches,elo);
    const currentEdition = Number(seasonSystem().activeEdition || 10);
    const historicalEditionMap = new Map((historical.editions||[]).map(row=>[Number(row.edition),row]));
    const contexts = new Map(names.map(name=>[name,[]]));
    const strengths = [];

    const clamp = (value,min=5,max=94)=>playerCardClamp(value,min,max);
    const safeMean = (values,fallback=50)=>playerCardMean(values,fallback);
    const rate = (rows,predicate)=>rows.length?rows.filter(predicate).length/rows.length:0;
    const resultPoints = row=>row.result==="W"?3:row.result==="D"?1:0;
    const logistic = value=>1/(1+Math.pow(10,-value/400));
    const outcome = (gf,ga)=>gf>ga?1:gf===ga?.5:0;

    const stageClass = stage=>{
      const value=String(stage||"").toLocaleLowerCase("tr");
      if ((value.includes("final")||value.includes("şampiyon"))&&!value.includes("semi")&&!value.includes("yarı")) return "final";
      if (value.includes("semi")||value.includes("yarı")) return "semi";
      if (value.includes("quarter")||value.includes("çeyrek")||value.includes("knockout")||value.includes("eleme")||value.includes("play")) return "knockout";
      if (value.includes("3rd")||value.includes("third")||value.includes("üçüncü")) return "placement";
      if (value.includes("gold")||value.includes("silver")||value.includes("altın")||value.includes("gümüş")) return "group";
      return "league";
    };
    const stageWeight = stage=>{
      const type=stageClass(stage);
      if(type==="final") return 1.80;
      if(type==="semi") return 1.50;
      if(type==="knockout") return 1.25;
      if(type==="placement") return 1.15;
      if(type==="group") return 1.10;
      return 1;
    };
    const stageLabel = type=>({
      league:intelligenceCopy("Lig / İlk Aşama","League / First Stage"),
      group:intelligenceCopy("Altın / Gümüş Grup","Gold / Silver Group"),
      knockout:intelligenceCopy("Play-off / Eleme","Play-off / Knockout"),
      semi:intelligenceCopy("Yarı Final","Semi-final"),
      final:intelligenceCopy("Final","Final"),
      placement:intelligenceCopy("Sıralama Maçı","Placement Match")
    }[type]||type);

    const editionMatchesMap = new Map();
    matches.forEach(match=>{
      const edition=Number(match.edition)||0;
      if(!editionMatchesMap.has(edition)) editionMatchesMap.set(edition,[]);
      editionMatchesMap.get(edition).push(match);
    });
    const editionGoalPriors=new Map([...editionMatchesMap.entries()].map(([edition,editionMatches])=>[
      edition,
      Math.max(3.2,Math.min(11, safeMean(editionMatches.map(item=>Number(item.homeScore||0)+Number(item.awayScore||0)),6)))
    ]));

    matches.forEach((match,index)=>{
      const edition=Number(match.edition)||0;
      const record=elo.recordMap.get(match.id);
      const homeElo=record?.beforeHome??elo.playerMap.get(match.homeName)?.rating??1500;
      const awayElo=record?.beforeAway??elo.playerMap.get(match.awayName)?.rating??1500;
      const homeTeam=teamModel.get(match.homeTeam,edition);
      const awayTeam=teamModel.get(match.awayTeam,edition);
      const known=homeTeam.known&&awayTeam.known;
      const homeStrength=homeElo+(known?(homeTeam.score-50)*5.3:0);
      const awayStrength=awayElo+(known?(awayTeam.score-50)*5.3:0);
      strengths.push(homeStrength,awayStrength);
      const expectedHome=logistic(homeStrength-awayStrength);
      const goalPrior=editionGoalPriors.get(edition)||6;
      const totalGoals=clamp(goalPrior+Math.abs(expectedHome-.5)*1.05,3.2,11);
      const homeShare=playerCardClamp(.5+(expectedHome-.5)*.72,.25,.75);
      const xHome=totalGoals*homeShare;
      const xAway=totalGoals*(1-homeShare);
      const add=(name,isHome)=>{
        if(!contexts.has(name)) contexts.set(name,[]);
        const gf=Number(isHome?match.homeScore:match.awayScore)||0;
        const ga=Number(isHome?match.awayScore:match.homeScore)||0;
        const expected=isHome?expectedHome:1-expectedHome;
        const actual=outcome(gf,ga);
        const xGF=isHome?xHome:xAway;
        const xGA=isHome?xAway:xHome;
        const ownTeam=isHome?homeTeam:awayTeam;
        const oppTeam=isHome?awayTeam:homeTeam;
        const opponent=isHome?match.awayName:match.homeName;
        const opponentStrength=isHome?awayStrength:homeStrength;
        const resultResidual=actual-expected;
        const attackResidual=Math.tanh((gf-xGF)/(1.9+xGF*.22));
        const defenceResidual=Math.tanh((xGA-ga)/(1.9+xGA*.22));
        const marginResidual=Math.tanh(((gf-ga)-(xGF-xGA))/3.2);
        const adjusted=clamp(50+(resultResidual*.58+attackResidual*.15+defenceResidual*.12+marginResidual*.15)*46);
        const adjustedAttack=clamp(50+attackResidual*27+resultResidual*7+marginResidual*4);
        const adjustedDefence=clamp(50+defenceResidual*28+resultResidual*6+marginResidual*4);
        const type=stageClass(match.stage);
        contexts.get(name).push({
          index,edition,stage:String(match.stage||""),stageType:type,stageLabel:stageLabel(type),
          opponent,gf,ga,xGF,xGA,result:gf>ga?"W":gf===ga?"D":"L",
          expected,adjusted,attack:adjustedAttack,defence:adjustedDefence,
          resultResidual,attackResidual,defenceResidual,marginResidual,
          ownTeam:ownTeam.score,oppTeam:oppTeam.score,known,
          teamDisadvantage:known&&ownTeam.score+4<oppTeam.score,
          teamAdvantage:known&&ownTeam.score>oppTeam.score+4,
          underdog:expected<=.42,favourite:expected>=.58,balanced:expected>.42&&expected<.58,
          opponentStrength,strongOpponent:false,weakOpponent:false,
          weight:stageWeight(match.stage)
        });
      };
      add(match.homeName,true);
      add(match.awayName,false);
    });

    const sortedStrengths=strengths.slice().sort((a,b)=>a-b);
    const highStrength=sortedStrengths[Math.floor(sortedStrengths.length*.70)]||1550;
    const lowStrength=sortedStrengths[Math.floor(sortedStrengths.length*.30)]||1450;
    contexts.forEach(rows=>rows.forEach(row=>{
      row.strongOpponent=row.opponentStrength>=highStrength;
      row.weakOpponent=row.opponentStrength<=lowStrength;
    }));

    function midrankScore(values,value,invert=false,minimum=8,maximum=92) {
      const list=(values||[]).map(Number).filter(Number.isFinite).sort((a,b)=>a-b);
      ...
      let lower=0,equal=0;
      list.forEach(item=>{
        if(item<target-1e-9) lower+=1;
        else if(Math.abs(item-target)<=1e-9) equal+=1;
      });
      let percentile=(lower+equal*.5)/list.length;
      if(invert) percentile=1-percentile;
      return clamp(minimum+percentile*(maximum-minimum),minimum,maximum);
    }

    function absolutePositive(value,maximumValue) {
      if(!Number.isFinite(Number(value))||!maximumValue) return null;
      return clamp(5+Math.max(0,Math.min(1,Number(value)/maximumValue))*89);
    }

    function absoluteMargin(value,scale=3) {
      return clamp(50+Math.tanh((Number(value)||0)/scale)*42);
    }

    function relativeGoalScore(value,baseline,invert=false) {
      const actual=Math.max(0,Number(value)||0);
      const reference=Math.max(.2,Number(baseline)||.2);
      const ratio=Math.log((actual+.35)/(reference+.35));
      return clamp(50+(invert?-1:1)*ratio*28);
    }

    function weightedStats(rows) {
      if(!rows.length) return {
        matches:0,wins:0,draws:0,losses:0,winRate:0,ppg:0,avgGF:0,avgGA:0,gd:0,
        weightedWinRate:0,weightedPpg:0,weightedGF:0,weightedGA:0,weightedGD:0,
        scoreRate:0,twoPlus:0,threePlus:0,fourPlus:0,sixPlus:0,lowConcede:0,
        heavyConcede:0,sixConcede:0,severeLoss:0,maxGF:0,avgLossMargin:null,
        goalStd:0,concedeStd:0,expectation:50,attackExpectation:50,defenceExpectation:50
      };
      ...
      const weightTotal=rows.reduce((sum,row)=>sum+(row.weight||1),0)||1;
      const weightedPoints=rows.reduce((sum,row)=>sum+resultPoints(row)*(row.weight||1),0);
      const weightedWins=rows.reduce((sum,row)=>sum+(row.result==="W"?1:0)*(row.weight||1),0);
      const weightedGF=rows.reduce((sum,row)=>sum+row.gf*(row.weight||1),0)/weightTotal;
      const weightedGA=rows.reduce((sum,row)=>sum+row.ga*(row.weight||1),0)/weightTotal;
      const avgGF=safeMean(rows.map(row=>row.gf),0);
      const avgGA=safeMean(rows.map(row=>row.ga),0);
      const lossRows=rows.filter(row=>row.result==="L");
      return {
        matches:rows.length,wins,draws,losses,
        winRate:wins/rows.length*100,
        ppg:rows.reduce((sum,row)=>sum+resultPoints(row),0)/rows.length,
        avgGF,avgGA,gd:avgGF-avgGA,
        weightedWinRate:weightedWins/weightTotal*100,
        weightedPpg:weightedPoints/weightTotal,
        weightedGF,weightedGA,weightedGD:weightedGF-weightedGA,
        scoreRate:rate(rows,row=>row.gf>0),
        twoPlus:rate(rows,row=>row.gf>=2),
        threePlus:rate(rows,row=>row.gf>=3),
        fourPlus:rate(rows,row=>row.gf>=4),
        sixPlus:rate(rows,row=>row.gf>=6),
        lowConcede:rate(rows,row=>row.ga<=2),
        heavyConcede:rate(rows,row=>row.ga>=5),
        sixConcede:rate(rows,row=>row.ga>=6),
        severeLoss:rate(rows,row=>row.ga-row.gf>=4),
        maxGF:Math.max(...rows.map(row=>row.gf)),
        avgLossMargin:lossRows.length?safeMean(lossRows.map(row=>row.ga-row.gf),0):null,
        goalStd:playerCardStd(rows.map(row=>row.gf)),
        concedeStd:playerCardStd(rows.map(row=>row.ga)),
        expectation:safeMean(rows.map(row=>row.adjusted),50),
        attackExpectation:safeMean(rows.map(row=>row.attack),50),
        defenceExpectation:safeMean(rows.map(row=>row.defence),50)
      };
    }

    function subsetRaw(rows) {
      if(!rows.length) return null;
      const stats=weightedStats(rows);
      const points=absolutePositive(stats.weightedPpg,3);
      const wins=absolutePositive(stats.weightedWinRate,100);
      const margin=absoluteMargin(stats.weightedGD,3);
      return clamp(points*.50+wins*.22+margin*.18+stats.expectation*.10);
    }

    function longestRuns(rows) {
      let currentWin=0,maxWin=0,currentUnbeaten=0,maxUnbeaten=0;
      rows.forEach(row=>{
        if(row.result==="W"){currentWin+=1;currentUnbeaten+=1;}
        else if(row.result==="D"){currentWin=0;currentUnbeaten+=1;}
        else {currentWin=0;currentUnbeaten=0;}
        maxWin=Math.max(maxWin,currentWin);
        maxUnbeaten=Math.max(maxUnbeaten,currentUnbeaten);
      });
      return {maxWin,maxUnbeaten,currentWin,currentUnbeaten};
    }

    function stageAchievement(name,edition,rows) {
      const meta=historicalEditionMap.get(Number(edition));
      if(meta?.champion===name) return {score:94,label:intelligenceCopy("Şampiyon","Champion"),rank:1};
      if(meta?.runnerUp===name) return {score:84,label:intelligenceCopy("Finalist","Runner-up"),rank:2};
      if(meta?.third===name) return {score:72,label:intelligenceCopy("Üçüncü","Third Place"),rank:3};
      if(meta?.fourth===name) return {score:68,label:intelligenceCopy("Yarı Finalist","Semi-finalist"),rank:4};
      const finalRows=rows.filter(row=>row.stageType==="final");
      if(finalRows.some(row=>row.result==="W")) return {score:94,label:intelligenceCopy("Şampiyon","Champion"),rank:1};
      if(finalRows.length) return {score:84,label:intelligenceCopy("Finalist","Finalist"),rank:2};
      if(rows.some(row=>row.stageType==="semi")) return {score:70,label:intelligenceCopy("Yarı Finalist","Semi-finalist"),rank:4};
      if(rows.some(row=>row.stageType==="knockout")) return {score:60,label:intelligenceCopy("Eleme Aşaması","Knockout Stage"),rank:8};
      if(rows.some(row=>row.stageType==="group")) return {score:52,label:intelligenceCopy("İkinci Aşama","Second Stage"),rank:null};
      return {score:42,label:intelligenceCopy("Lig Aşaması","League Stage"),rank:null};
    }

    function rollingStats(rows,count) {
      const slice=rows.slice(-count);
      return weightedStats(slice);
    }

    const editionParticipantRows=new Map();
    contexts.forEach((rows,name)=>{
      rows.forEach(row=>{
        const key=Number(row.edition);
        if(!editionParticipantRows.has(key)) editionParticipantRows.set(key,new Map());
        if(!editionParticipantRows.get(key).has(name)) editionParticipantRows.get(key).set(name,[]);
        editionParticipantRows.get(key).get(name).push(row);
      });
    });

    const rawEditionProfiles=new Map();
    editionParticipantRows.forEach((playerRowsMap,edition)=>{
      const rawProfiles=[];
      playerRowsMap.forEach((rows,name)=>{
        const sorted=rows.slice().sort((a,b)=>a.index-b.index);
        const stats=weightedStats(sorted);
        const strongRows=sorted.filter(row=>row.strongOpponent);
        const weakRows=sorted.filter(row=>row.weakOpponent);
        const evenRows=sorted.filter(row=>row.balanced);
        const underdogRows=sorted.filter(row=>row.underdog||row.teamDisadvantage);
        const favouriteRows=sorted.filter(row=>row.favourite);
        const knockoutRows=sorted.filter(row=>row.stageType==="knockout");
        const semiRows=sorted.filter(row=>row.stageType==="semi");
        const finalRows=sorted.filter(row=>row.stageType==="final");
        const placementRows=sorted.filter(row=>row.stageType==="placement");
        const bigRows=sorted.filter(row=>["knockout","semi","final"].includes(row.stageType));
        const leagueRows=sorted.filter(row=>row.stageType==="league");
        const groupRows=sorted.filter(row=>row.stageType==="group");
        const closeRows=sorted.filter(row=>Math.abs(row.gf-row.ga)<=1);
        const afterLoss=[];
        for(let i=1;i<sorted.length;i++) if(sorted[i-1].result==="L") afterLoss.push(sorted[i]);
        const known=sorted.filter(row=>row.known);
        const weakTeamRows=known.filter(row=>row.ownTeam<=45||row.teamDisadvantage);
        const strongTeamRows=known.filter(row=>row.ownTeam>=63);
        const disadvantageRows=known.filter(row=>row.teamDisadvantage);
        const achievement=stageAchievement(name,edition,sorted);
        const runs=longestRuns(sorted);
        const last5=rollingStats(sorted,5);
        const last10=rollingStats(sorted,10);
        const previous5=weightedStats(sorted.slice(-10,-5));
        const contextBucketMap=new Map();
        known.forEach(row=>{
          const difference=row.ownTeam-row.oppTeam;
          const bucket=difference>=10?"heavyAdv":difference>=4?"adv":difference>-4?"even":difference>-10?"disadv":"heavyDisadv";
          if(!contextBucketMap.has(bucket)) contextBucketMap.set(bucket,[]);
          contextBucketMap.get(bucket).push(row);
        });
        const contextScores=[...contextBucketMap.values()].filter(list=>list.length>=2).map(list=>subsetRaw(list));
        const teamPowerGroups=new Map();
        known.forEach(row=>{
          const bucket=Math.round(row.ownTeam/5)*5;
          if(!teamPowerGroups.has(bucket)) teamPowerGroups.set(bucket,[]);
          teamPowerGroups.get(bucket).push(row);
        });
        const teamPowerScores=[...teamPowerGroups.values()].filter(list=>list.length>=2).map(list=>subsetRaw(list));
        const teamInfo=teamMap.get(name)||{teams:[]};
        const successfulTeams=(teamInfo.teams||[]).filter(team=>(team.wins||0)>0).length;
        const powerBands=new Set(known.map(row=>Math.round(row.ownTeam/10)*10)).size;
        const teamDiversityRaw=known.length>=5?clamp(26+Math.min(34,successfulTeams*5)+Math.min(20,powerBands*5)+Math.min(10,(teamInfo.teams||[]).length*1.5)):null;
        const pressureRow=pressureMap.get(name)||{};
        const closeRaw=closeRows.length>=3?subsetRaw(closeRows):null;
        const comebackRaw=(pressureRow.highPressureGames||0)>=3&&((pressureRow.comebackWins||0)>0||(pressureRow.lateWins||0)>0)
          ?clamp(30+Math.min(36,(pressureRow.comebackWins||0)*8)+Math.min(18,(pressureRow.lateWins||0)*4))
          :null;
        rawProfiles.push({
          name,edition,rows:sorted,stats,achievement,runs,
          raw:{
            weightedPpg:stats.weightedPpg,weightedWinRate:stats.weightedWinRate,weightedGD:stats.weightedGD,
            weightedGF:stats.weightedGF,weightedGA:stats.weightedGA,scoreRate:stats.scoreRate,
            twoPlus:stats.twoPlus,threePlus:stats.threePlus,fourPlus:stats.fourPlus,sixPlus:stats.sixPlus,
            maxGF:stats.maxGF,lowConcede:stats.lowConcede,heavyConcede:stats.heavyConcede,
            sixConcede:stats.sixConcede,severeLoss:stats.severeLoss,avgLossMargin:stats.avgLossMargin,
            goalStd:stats.goalStd,concedeStd:stats.concedeStd,
            attackExpectation:stats.attackExpectation,defenceExpectation:stats.defenceExpectation,
            expectation:stats.expectation,
            strongAttack:strongRows.length>=3?weightedStats(strongRows).weightedGF:null,
            strongAttackExpectation:strongRows.length>=3?weightedStats(strongRows).attackExpectation:null,
            underdogAttack:underdogRows.length>=3?weightedStats(underdogRows).weightedGF:null,
            underdogAttackExpectation:underdogRows.length>=3?weightedStats(underdogRows).attackExpectation:null,
            favouriteAttack:favouriteRows.length>=3?weightedStats(favouriteRows).weightedGF:null,
            strongDefence:strongRows.length>=3?weightedStats(strongRows).weightedGA:null,
            strongDefenceExpectation:strongRows.length>=3?weightedStats(strongRows).defenceExpectation:null,
            favouriteDefence:favouriteRows.length>=3?weightedStats(favouriteRows).weightedGA:null,
            strongPerformance:strongRows.length>=3?subsetRaw(strongRows):null,
            weakPerformance:weakRows.length>=3?subsetRaw(weakRows):null,
            evenPerformance:evenRows.length>=3?subsetRaw(evenRows):null,
            underdogPerformance:underdogRows.length>=3?subsetRaw(underdogRows):null,
            favouritePerformance:favouriteRows.length>=3?subsetRaw(favouriteRows):null,
            knockoutPerformance:knockoutRows.length>=2?subsetRaw(knockoutRows):null,
            semiPerformance:semiRows.length?subsetRaw(semiRows):null,
            finalPerformance:finalRows.length?subsetRaw(finalRows):null,
            leaguePerformance:leagueRows.length?subsetRaw(leagueRows):null,
            groupPerformance:groupRows.length?subsetRaw(groupRows):null,
            placementPerformance:placementRows.length?subsetRaw(placementRows):null,
            bigMatchPerformance:bigRows.length?subsetRaw(bigRows)*.72+achievement.score*.28:achievement.score,
            closePerformance:closeRaw,
            pressureManagement:bigRows.length?subsetRaw(bigRows)*.65+(pressureRow.mentalIndex||50)*.35:(pressureRow.mentalIndex||50),
            comebackPower:comebackRaw,
            bounceBack:afterLoss.length>=3?subsetRaw(afterLoss):null,
            streakStrength:clamp(32+Math.min(30,runs.maxWin*7)+Math.min(22,runs.maxUnbeaten*3)),
            focus:favouriteRows.length>=3?subsetRaw(favouriteRows)-rate(favouriteRows,row=>row.result==="L")*12:null,
            criticalMentality:bigRows.length?subsetRaw(bigRows):null,
            weakTeamEfficiency:weakTeamRows.length>=3?subsetRaw(weakTeamRows):null,
            strongTeamEfficiency:strongTeamRows.length>=3?subsetRaw(strongTeamRows):null,
            teamDiversity:teamDiversityRaw,
            selectionEfficiency:known.length>=5?subsetRaw(known):null,
            independentImpact:disadvantageRows.length>=3?subsetRaw(disadvantageRows):null,
            balancePerformance:known.length>=8&&contextScores.length>=3?clamp(safeMean(contextScores,50)-playerCardStd(contextScores)*.45):null,
            crossTeamConsistency:known.length>=8&&teamPowerScores.length>=3?clamp(safeMean(teamPowerScores,50)*.65+(88-playerCardStd(teamPowerScores)*1.2)*.35):null,
            last5Ppg:last5.weightedPpg,last5WinRate:last5.weightedWinRate,last5GD:last5.weightedGD,
            last5GF:last5.weightedGF,last5GA:last5.weightedGA,last5Expectation:last5.expectation,
            last10Ppg:last10.weightedPpg,last10WinRate:last10.weightedWinRate,last10GD:last10.weightedGD,
            last10GF:last10.weightedGF,last10GA:last10.weightedGA,last10Expectation:last10.expectation,
            previous5Ppg:previous5.weightedPpg
          },
          samples:{
            strong:strongRows.length,weak:weakRows.length,even:evenRows.length,underdog:underdogRows.length,
            favourite:favouriteRows.length,knockout:knockoutRows.length,semi:semiRows.length,final:finalRows.length,
            big:bigRows.length,known:known.length
          }
        });
      });
      rawEditionProfiles.set(edition,rawProfiles);
    });

    function editionBaseline(edition) {
      const editionMatches=editionMatchesMap.get(Number(edition))||[];
      const sides=[];
      editionMatches.forEach(match=>{
        sides.push({gf:Number(match.homeScore)||0,ga:Number(match.awayScore)||0});
        sides.push({gf:Number(match.awayScore)||0,ga:Number(match.homeScore)||0});
      });
      return {
        avgGF:safeMean(sides.map(row=>row.gf),3),
        avgGA:safeMean(sides.map(row=>row.ga),3),
        scoreRate:rate(sides,row=>row.gf>0),
        twoPlus:rate(sides,row=>row.gf>=2),
        threePlus:rate(sides,row=>row.gf>=3),
        fourPlus:rate(sides,row=>row.gf>=4),
        sixPlus:rate(sides,row=>row.gf>=6),
        lowConcede:rate(sides,row=>row.ga<=2),
        heavyConcede:rate(sides,row=>row.ga>=5),
        severeLoss:rate(sides,row=>row.ga-row.gf>=4)
      };
    }

    function createEditionProfile(rawProfile,peerProfiles) {
      const {name,edition,rows,stats,raw,samples,achievement}=rawProfile;
      const baseline=editionBaseline(edition);
      const values=(key)=>peerProfiles.map(profile=>profile.raw[key]).filter(value=>value!=null&&Number.isFinite(Number(value)));
      const score=(key,invert=false)=>raw[key]==null?null:midrankScore(values(key),raw[key],invert);
      const hybrid=(key,absolute,invert=false,peerWeight=.65)=>raw[key]==null?null:clamp(score(key,invert)*peerWeight+absolute*(1-peerWeight));

      const weightedGoalRank=score("weightedGF");
      const goalProduction=clamp(
        hybrid("weightedGF",relativeGoalScore(stats.weightedGF,baseline.avgGF),false,.62)*.55+
        score("scoreRate")*.20+
        score("twoPlus")*.15+
        absolutePositive(stats.weightedGF,Math.max(1,baseline.avgGF*2.2))*.10
      );
      const scoringEfficiencyRaw=hybrid("attackExpectation",raw.attackExpectation,false,.70);
      const scoringEfficiency=scoringEfficiencyRaw==null?null:Math.min(scoringEfficiencyRaw,goalProduction+12);
      const bigScorePotential=clamp(score("fourPlus")*.34+score("sixPlus")*.34+score("maxGF")*.22+(score("strongAttack")??50)*.10);
      const strongOpponentAttack=raw.strongAttack==null?null:clamp(score("strongAttack")*.68+(score("strongAttackExpectation")??50)*.32);
      const underdogAttack=raw.underdogAttack==null?null:clamp(score("underdogAttack")*.68+(score("underdogAttackExpectation")??50)*.32);
      const favouriteProduction=raw.favouriteAttack==null?null:score("favouriteAttack");
      const highOutputFrequency=clamp(score("twoPlus")*.42+score("threePlus")*.36+score("sixPlus")*.22);
      let scoringConsistency=clamp(goalProduction*.64+midrankScore(values("goalStd"),raw.goalStd,true)*.36);
      if(stats.scoreRate<baseline.scoreRate*.55) scoringConsistency=Math.min(scoringConsistency,35);
      let attackScore=playerCardWeightedAvailable([
        {value:goalProduction,weight:40},{value:scoringEfficiency,weight:15},{value:bigScorePotential,weight:10},
        {value:strongOpponentAttack,weight:10},{value:underdogAttack,weight:8},{value:scoringConsistency,weight:7},
        {value:favouriteProduction,weight:5},{value:highOutputFrequency,weight:5}
      ],goalProduction);
      const gfPercentile=playerCardPercentile(values("weightedGF"),raw.weightedGF);
      if(gfPercentile<=.10) attackScore=Math.min(attackScore,28);
      else if(gfPercentile<=.20) attackScore=Math.min(attackScore,39);
      else if(gfPercentile<=.30) attackScore=Math.min(attackScore,49);
      attackScore=Math.round(clamp(attackScore));

      const goalPrevention=clamp(
        hybrid("weightedGA",relativeGoalScore(stats.weightedGA,baseline.avgGA,true),true,.65)*.60+
        score("lowConcede")*.25+
        score("heavyConcede",true)*.15
      );
      const expectedDefenceRaw=hybrid("defenceExpectation",raw.defenceExpectation,false,.70);
      const expectedDefence=expectedDefenceRaw==null?null:Math.min(expectedDefenceRaw,goalPrevention+12);
      const strongOpponentDefence=raw.strongDefence==null?null:clamp(score("strongDefence",true)*.68+(score("strongDefenceExpectation")??50)*.32);
      const favouriteControl=raw.favouriteDefence==null?null:score("favouriteDefence",true);
      const collapsePrevention=clamp(score("severeLoss",true)*.55+score("sixConcede",true)*.45);
      const lossLimitation=raw.avgLossMargin==null?null:score("avgLossMargin",true);
      const lowConcedeRate=score("lowConcede");
      let defensiveConsistency=clamp(goalPrevention*.65+midrankScore(values("concedeStd"),raw.concedeStd,true)*.35);
      if(stats.heavyConcede>baseline.heavyConcede*1.6) defensiveConsistency=Math.min(defensiveConsistency,38);
      let defenseScore=playerCardWeightedAvailable([
        {value:goalPrevention,weight:40},{value:expectedDefence,weight:10},{value:strongOpponentDefence,weight:10},
        {value:favouriteControl,weight:5},{value:collapsePrevention,weight:15},{value:lossLimitation,weight:5},
        {value:lowConcedeRate,weight:10},{value:defensiveConsistency,weight:5}
      ],goalPrevention);
      const gaPercentile=playerCardPercentile(values("weightedGA"),raw.weightedGA);
      if(gaPercentile>=.90) defenseScore=Math.min(defenseScore,28);
      else if(gaPercentile>=.80) defenseScore=Math.min(defenseScore,39);
      else if(gaPercentile>=.70) defenseScore=Math.min(defenseScore,49);
      defenseScore=Math.round(clamp(defenseScore));

      const contextScore=key=>raw[key]==null?null:clamp(score(key)*.70+raw[key]*.30);
      const strongOpponentPerformance=contextScore("strongPerformance");
      const weakOpponentDelivery=contextScore("weakPerformance");
      const evenMatchPerformance=contextScore("evenPerformance");
      const underdogPerformance=contextScore("underdogPerformance");
      const favouriteResponsibility=contextScore("favouritePerformance");
      const knockoutPerformance=contextScore("knockoutPerformance");
      const semiFinalPerformance=contextScore("semiPerformance");
      const finalPerformance=contextScore("finalPerformance");
      const championshipImpact=achievement.score;
      const competitionScore=Math.round(playerCardWeightedAvailable([
        {value:strongOpponentPerformance,weight:18},{value:underdogPerformance,weight:15},
        {value:evenMatchPerformance,weight:14},{value:favouriteResponsibility,weight:10},
        {value:weakOpponentDelivery,weight:8},{value:knockoutPerformance,weight:10},
        {value:semiFinalPerformance,weight:10},{value:finalPerformance,weight:10},
        {value:championshipImpact,weight:5}
      ],50));

      const closeMatchManagement=contextScore("closePerformance");
      const pressureManagement=contextScore("pressureManagement");
      const comebackPower=contextScore("comebackPower");
      const bounceBack=contextScore("bounceBack");
      const streakStrength=contextScore("streakStrength");
      const focus=contextScore("focus");
      const criticalMentality=contextScore("criticalMentality");
      const mentalScore=Math.round(playerCardWeightedAvailable([
        {value:closeMatchManagement,weight:20},{value:pressureManagement,weight:22},
        {value:comebackPower,weight:10},{value:bounceBack,weight:13},{value:streakStrength,weight:12},
        {value:focus,weight:10},{value:criticalMentality,weight:13}
      ],45));

      const weakTeamEfficiency=contextScore("weakTeamEfficiency");
      const strongTeamEfficiency=contextScore("strongTeamEfficiency");
      const teamDiversity=contextScore("teamDiversity");
      const selectionEfficiency=contextScore("selectionEfficiency");
      const independentImpact=contextScore("independentImpact");
      const balancePerformance=contextScore("balancePerformance");
      const crossTeamConsistency=contextScore("crossTeamConsistency");
      const teamUsageScore=Math.round(playerCardWeightedAvailable([
        {value:weakTeamEfficiency,weight:18},{value:strongTeamEfficiency,weight:10},
        {value:teamDiversity,weight:14},{value:selectionEfficiency,weight:18},
        {value:independentImpact,weight:18},{value:balancePerformance,weight:12},
        {value:crossTeamConsistency,weight:10}
      ],45));

      const ppgScore=hybrid("weightedPpg",absolutePositive(stats.weightedPpg,3),false,.62);
      const winScore=hybrid("weightedWinRate",absolutePositive(stats.weightedWinRate,100),false,.62);
      const gdScore=hybrid("weightedGD",absoluteMargin(stats.weightedGD,3),false,.66);
      const resultPerformance=clamp(ppgScore*.45+winScore*.25+gdScore*.20+achievement.score*.10);

      const expectationPerformance=clamp(score("expectation")*.70+raw.expectation*.30);
      const opponentQuality=clamp(35+safeMean(rows.map(row=>playerCardPercentile(strengths,row.opponentStrength)),.5)*50);

      const bigMatchPeerScore=contextScore("bigMatchPerformance");
      const bigMatchFloor=achievement.score>=90?80:achievement.score>=80?68:achievement.score>=68?58:achievement.score>=60?50:35;
      const bigMatchPerformance=Math.round(Math.max(bigMatchFloor,clamp((bigMatchPeerScore??achievement.score)*.72+achievement.score*.28)));

      const last5Ppg=hybrid("last5Ppg",absolutePositive(raw.last5Ppg,3),false,.55);
      const last5Win=hybrid("last5WinRate",absolutePositive(raw.last5WinRate,100),false,.55);
      const last5Gd=hybrid("last5GD",absoluteMargin(raw.last5GD,3),false,.60);
      const last5Attack=hybrid("last5GF",relativeGoalScore(raw.last5GF,baseline.avgGF),false,.60);
      const last5Defense=hybrid("last5GA",relativeGoalScore(raw.last5GA,baseline.avgGA,true),true,.60);
      const last5Context=hybrid("last5Expectation",raw.last5Expectation,false,.70);
      const last5Form=clamp(last5Ppg*.40+last5Gd*.20+last5Attack*.15+last5Defense*.15+last5Context*.10);
      const last10Ppg=hybrid("last10Ppg",absolutePositive(raw.last10Ppg,3),false,.55);
      const last10Win=hybrid("last10WinRate",absolutePositive(raw.last10WinRate,100),false,.55);
      const last10Gd=hybrid("last10GD",absoluteMargin(raw.last10GD,3),false,.60);
      const last10Attack=hybrid("last10GF",relativeGoalScore(raw.last10GF,baseline.avgGF),false,.60);
      const last10Defense=hybrid("last10GA",relativeGoalScore(raw.last10GA,baseline.avgGA,true),true,.60);
      const last10Context=hybrid("last10Expectation",raw.last10Expectation,false,.70);
      const last10Form=clamp(last10Ppg*.40+last10Gd*.20+last10Attack*.15+last10Defense*.15+last10Context*.10);
      const currentForm=Math.round(clamp(last5Form*.65+last10Form*.35));

      const performance=Math.round(clamp(
        resultPerformance*.40+
        attackScore*.20+
        defenseScore*.15+
        bigMatchPerformance*.15+
        expectationPerformance*.05+
        opponentQuality*.05
      ));

      const categories=[
        {key:"attack",label:playerCardMetricLabel("attack"),score:attackScore,attributes:[
          {key:"goalProduction",label:playerCardMetricLabel("goalProduction"),value:goalProduction},
          {key:"scoringEfficiency",label:playerCardMetricLabel("scoringEfficiency"),value:scoringEfficiency},
          {key:"bigScorePotential",label:playerCardMetricLabel("bigScorePotential"),value:bigScorePotential},
          {key:"strongOpponentAttack",label:playerCardMetricLabel("strongOpponentAttack"),value:strongOpponentAttack},
          {key:"underdogAttack",label:playerCardMetricLabel("underdogAttack"),value:underdogAttack},
          {key:"scoringConsistency",label:playerCardMetricLabel("scoringConsistency"),value:scoringConsistency},
          {key:"favouriteProduction",label:playerCardMetricLabel("favouriteProduction"),value:favouriteProduction},
          {key:"highOutputFrequency",label:playerCardMetricLabel("highOutputFrequency"),value:highOutputFrequency}
        ]},
        {key:"defense",label:playerCardMetricLabel("defense"),score:defenseScore,attributes:[
          {key:"goalPrevention",label:playerCardMetricLabel("goalPrevention"),value:goalPrevention},
          {key:"expectedDefence",label:playerCardMetricLabel("expectedDefence"),value:expectedDefence},
          {key:"strongOpponentDefence",label:playerCardMetricLabel("strongOpponentDefence"),value:strongOpponentDefence},
          {key:"favouriteControl",label:playerCardMetricLabel("favouriteControl"),value:favouriteControl},
          {key:"collapsePrevention",label:playerCardMetricLabel("collapsePrevention"),value:collapsePrevention},
          {key:"lossLimitation",label:playerCardMetricLabel("lossLimitation"),value:lossLimitation},
          {key:"lowConcedeRate",label:playerCardMetricLabel("lowConcedeRate"),value:lowConcedeRate},
          {key:"defensiveConsistency",label:playerCardMetricLabel("defensiveConsistency"),value:defensiveConsistency}
        ]},
        {key:"competition",label:playerCardMetricLabel("competition"),score:competitionScore,attributes:[
          {key:"strongOpponentPerformance",label:playerCardMetricLabel("strongOpponentPerformance"),value:strongOpponentPerformance},
          {key:"weakOpponentDelivery",label:playerCardMetricLabel("weakOpponentDelivery"),value:weakOpponentDelivery},
          {key:"evenMatchPerformance",label:playerCardMetricLabel("evenMatchPerformance"),value:evenMatchPerformance},
          {key:"underdogPerformance",label:playerCardMetricLabel("underdogPerformance"),value:underdogPerformance},
          {key:"favouriteResponsibility",label:playerCardMetricLabel("favouriteResponsibility"),value:favouriteResponsibility},
          {key:"knockoutPerformance",label:playerCardMetricLabel("knockoutPerformance"),value:knockoutPerformance},
          {key:"semiFinalPerformance",label:playerCardMetricLabel("semiFinalPerformance"),value:semiFinalPerformance},
          {key:"finalPerformance",label:playerCardMetricLabel("finalPerformance"),value:finalPerformance},
          {key:"championshipImpact",label:playerCardMetricLabel("championshipImpact"),value:championshipImpact}
        ]},
        {key:"mental",label:playerCardMetricLabel("mental"),score:mentalScore,attributes:[
          {key:"closeMatchManagement",label:playerCardMetricLabel("closeMatchManagement"),value:closeMatchManagement},
          {key:"pressureManagement",label:playerCardMetricLabel("pressureManagement"),value:pressureManagement},
          {key:"comebackPower",label:playerCardMetricLabel("comebackPower"),value:comebackPower},
          {key:"bounceBack",label:playerCardMetricLabel("bounceBack"),value:bounceBack},
          {key:"streakStrength",label:playerCardMetricLabel("streakStrength"),value:streakStrength},
          {key:"focus",label:playerCardMetricLabel("focus"),value:focus},
          {key:"criticalMentality",label:playerCardMetricLabel("criticalMentality"),value:criticalMentality}
        ]},
        {key:"teamUsage",label:playerCardMetricLabel("teamUsage"),score:teamUsageScore,attributes:[
          {key:"weakTeamEfficiency",label:playerCardMetricLabel("weakTeamEfficiency"),value:weakTeamEfficiency},
          {key:"strongTeamEfficiency",label:playerCardMetricLabel("strongTeamEfficiency"),value:strongTeamEfficiency},
          {key:"teamDiversity",label:playerCardMetricLabel("teamDiversity"),value:teamDiversity},
          {key:"selectionEfficiency",label:playerCardMetricLabel("selectionEfficiency"),value:selectionEfficiency},
          {key:"independentImpact",label:playerCardMetricLabel("independentImpact"),value:independentImpact},
          {key:"balancePerformance",label:playerCardMetricLabel("balancePerformance"),value:balancePerformance},
          {key:"crossTeamConsistency",label:playerCardMetricLabel("crossTeamConsistency"),value:crossTeamConsistency}
        ]}
      ];

      const stageBreakdown={
        league:raw.leaguePerformance==null?null:Math.round(contextScore("leaguePerformance")),
        group:raw.groupPerformance==null?null:Math.round(contextScore("groupPerformance")),
        knockout:knockoutPerformance==null?null:Math.round(knockoutPerformance),
        semi:semiFinalPerformance==null?null:Math.round(semiFinalPerformance),
        final:finalPerformance==null?null:Math.round(finalPerformance),
        achievement:achievement.score,
        achievementLabel:achievement.label
      };

      const confidence=Math.round(clamp(
        30+Math.min(42,stats.matches*2.4)+
        (stats.matches?samples.known/stats.matches*16:0)+
        Math.min(8,samples.strong*1.2)+
        Math.min(6,samples.big*2)
      ,20,98));

      return {
        edition,name,rows,performance,attack:attackScore,defense:defenseScore,
        competition:competitionScore,mental:mentalScore,teamUsage:teamUsageScore,
        resultPerformance:Math.round(resultPerformance),
        expectationPerformance:Math.round(expectationPerformance),
        opponentQuality:Math.round(opponentQuality),
        bigMatchPerformance,
        currentForm,
        categories,stats:{
          matches:stats.matches,wins:stats.wins,draws:stats.draws,losses:stats.losses,
          winRate:stats.winRate,ppg:stats.ppg,avgGF:stats.avgGF,avgGA:stats.avgGA,gd:stats.gd,
          weightedPpg:stats.weightedPpg,weightedWinRate:stats.weightedWinRate,
          weightedGF:stats.weightedGF,weightedGA:stats.weightedGA,weightedGD:stats.weightedGD
        },
        stageBreakdown,achievement,confidence,samples
      };
    }

    const editionProfilesByName=new Map(names.map(name=>[name,[]]));
    rawEditionProfiles.forEach((profiles,edition)=>{
      profiles.forEach(rawProfile=>{
        editionProfilesByName.get(rawProfile.name)?.push(createEditionProfile(rawProfile,profiles));
      });
    });

    const rawPlayers=names.map(name=>{
      const rows=(contexts.get(name)||[]).sort((a,b)=>a.index-b.index);
      const editionProfiles=(editionProfilesByName.get(name)||[]).sort((a,b)=>a.edition-b.edition);
      const editionWeight=profile=>Math.sqrt(Math.max(1,profile.stats.matches))*(.72+profile.edition/Math.max(9,currentEdition)*.28);
      const careerAttack=playerCardWeightedAvailable(editionProfiles.map(profile=>({value:profile.attack,weight:editionWeight(profile)})),35);
      const careerDefense=playerCardWeightedAvailable(editionProfiles.map(profile=>({value:profile.defense,weight:editionWeight(profile)})),35);
      const careerCompetition=playerCardWeightedAvailable(editionProfiles.map(profile=>({value:profile.competition,weight:editionWeight(profile)})),35);
      const careerMental=playerCardWeightedAvailable(editionProfiles.map(profile=>({value:profile.mental,weight:editionWeight(profile)})),35);
      const careerTeamUsage=playerCardWeightedAvailable(editionProfiles.map(profile=>({value:profile.teamUsage,weight:editionWeight(profile)})),35);
      const careerPerformance=playerCardWeightedAvailable(editionProfiles.map(profile=>({value:profile.performance,weight:editionWeight(profile)})),25);
      const careerBigMatch=playerCardWeightedAvailable(editionProfiles.map(profile=>({value:profile.bigMatchPerformance,weight:Math.sqrt(Math.max(1,profile.stats.matches))})),35);
      const currentProfile=editionProfiles.find(profile=>profile.edition===currentEdition)||editionProfiles[editionProfiles.length-1]||null;
      const currentForm=currentProfile?.currentForm||25;
      const baselineAbility=careerAttack*.24+careerDefense*.20+careerCompetition*.24+careerMental*.17+careerTeamUsage*.15;
      const rawCurrentAbility=clamp(baselineAbility*.85+currentForm*.15);
      const careerRow=allTime.playerMap.get(name)||{};
      const e=elo.playerMap.get(name)||{rating:1500,peak:1500,timeline:[]};
      const ach=achMap.get(name)||{prestige:achievementPrestigeLevel(0),xp:0,unlocked:[],metrics:{}};
      const px=powerMap.get(name)||{powerIndex:100,change:0,championProbability:0,finalProbability:0};
      const teams=teamMap.get(name)||{teams:[]};
      const peakPercent=playerCardPercentile(elo.players.map(row=>row.peak),e.peak);
      const legacyAttrs={
        trophies:clamp(20+Math.min(50,(careerRow.titles||0)*18)+Math.min(16,(careerRow.finals||0)*4)),
        peakLevel:clamp(25+peakPercent*68),
        careerEfficiency:Math.round(careerPerformance),
        historicStrong:Math.round(careerBigMatch),
        longevity:clamp(18+Math.min(58,(careerRow.games||0)*.82))
      };
      const legacy=legacyAttrs.trophies*.35+legacyAttrs.peakLevel*.25+legacyAttrs.careerEfficiency*.20+legacyAttrs.historicStrong*.10+legacyAttrs.longevity*.10;
      const knownRatio=rows.length?rows.filter(row=>row.known).length/rows.length:0;
      const confidence=clamp(30+Math.min(40,rows.length*.9)+knownRatio*18+Math.min(10,editionProfiles.length*1.8),20,98);
      return {
        name,rows,editionProfiles,currentProfile,currentForm,rawCurrentAbility,careerPerformance,careerBigMatch,
        legacy,legacyAttrs,careerGroups:{attack:careerAttack,defense:careerDefense,competition:careerCompetition,mental:careerMental,teamUsage:careerTeamUsage},
        careerRow,e,ach,px,teams,confidence
      };
    });

    return rawPlayers.map(player=>{
      const sampleReliability=.55+.45*(player.rows.length/(player.rows.length+18))*(.78+.22*Math.min(1,player.editionProfiles.length/4));
      const currentAbility=Math.round(clamp(50+(player.rawCurrentAbility-50)*sampleReliability));
      const prestige=player.ach.prestige||achievementPrestigeLevel(player.ach.xp||0);
      ...
      const allAttributes=player.editionProfiles.flatMap(profile=>profile.categories.flatMap(category=>category.attributes.filter(attribute=>attribute.value!=null).map(attribute=>({...attribute,edition:profile.edition,group:category.label}))));
      const averagedAttributes=new Map();
      allAttributes.forEach(attribute=>{
        if(!averagedAttributes.has(attribute.key)) averagedAttributes.set(attribute.key,[]);
        averagedAttributes.get(attribute.key).push(Number(attribute.value));
      });
      const careerAttributes=[...averagedAttributes.entries()].map(([key,values])=>({
        key,label:playerCardMetricLabel(key),value:Math.round(safeMean(values,50))
      })).sort((a,b)=>b.value-a.value);
      const pros=careerAttributes.slice(0,5);
      const cons=careerAttributes.slice(-5).reverse();
      const currentProfile=player.currentProfile;
      const role=(currentProfile?.bigMatchPerformance||0)>=80?intelligenceCopy("Büyük Maç Oyuncusu","Big-Match Player"):
        (currentProfile?.categories.find(category=>category.key==="competition")?.attributes.find(attribute=>attribute.key==="strongOpponentPerformance")?.value||0)>=78?intelligenceCopy("Dev Katili","Giant Killer"):
        player.careerGroups.mental>=75?intelligenceCopy("Baskı Ustası","Pressure Master"):
        player.careerGroups.attack>=75?intelligenceCopy("Skor Üreticisi","Score Producer"):
        player.careerGroups.defense>=75?intelligenceCopy("Kontrol Uzmanı","Control Specialist"):
        player.currentForm>=78?intelligenceCopy("Momentum Avcısı","Momentum Hunter"):
        intelligenceCopy("Dengeli Rakip","Balanced Competitor");
      const topTeams=(player.teams.teams||[]).slice().sort((a,b)=>(b.games||0)-(a.games||0)).slice(0,5);
      const maxScore=player.rows.slice().sort((a,b)=>b.gf-a.gf)[0];
      const hardestWin=player.rows.filter(row=>row.result==="W").sort((a,b)=>b.opponentStrength-a.opponentStrength)[0];
      const latestProfile=player.editionProfiles.find(profile=>profile.edition===currentEdition)||player.editionProfiles[player.editionProfiles.length-1]||null;
      return {
        name:player.name,
        initials:player.name.split(/\s+/).map(part=>part[0]).slice(0,2).join("").toUpperCase(),
        currentAbility,
        fifa9Performance:Math.round(latestProfile?.performance||25),
        careerPerformance:Math.round(player.careerPerformance),
        currentForm:Math.round(player.currentForm),
        bigMatchPerformance:Math.round(latestProfile?.bigMatchPerformance||player.careerBigMatch||35),
        careerBigMatch:Math.round(player.careerBigMatch),
        legacy:Math.round(player.legacy),
        confidence:Math.round(player.confidence),
        formLabel:playerCardFormLabel(player.currentForm),
        editionProfiles:player.editionProfiles,
        categories:latestProfile?.categories||[],
        careerGroups:Object.fromEntries(Object.entries(player.careerGroups).map(([key,value])=>[key,Math.round(value)])),
        pros,cons,role,cardTier,prestige,modelKey,modelLabel:playerCardModelLabel(modelKey),
        activeTitle:player.ach.activeTitle||prestige.name,xp:player.ach.xp||0,badges:player.ach.unlocked||[],
        career:player.careerRow,elo:player.e.rating,eloPeak:player.e.peak,eloTimeline:(player.e.timeline||[]).slice(-20).map(item=>item.rating),
        powerIndex:player.px.powerIndex||100,powerChange:player.px.change||0,championProbability:player.px.championProbability||0,finalProbability:player.px.finalProbability||0,
        favoriteTeam:topTeams[0]?.team||"–",uniqueTeams:(player.teams.teams||[]).length,topTeams,
        formResults:(latestProfile?.rows||[]).slice(-10).map(row=>({result:row.result,opponent:row.opponent})),
        legacyAttrs:player.legacyAttrs,
        quickFacts:{
          maxScore:maxScore?`${maxScore.gf} - ${maxScore.ga}`:"–",
          hardestWin:hardestWin?`${hardestWin.gf} - ${hardestWin.ga}`:"–",
          winStreak:formAnalytics.playerMap.get(player.name)?.currentWinStreak||0,
          unbeaten:formAnalytics.playerMap.get(player.name)?.currentUnbeatenStreak||0
        },
        modelNote:intelligenceCopy(
          "Her FIFA turnuvası kendi oyuncu, gol, puan ve aşama dağılımı içinde değerlendirilir. Yarı final ve final maçları yüksek ağırlık taşır; takım/rakip düzeltmesi ana sonucu tersine çeviremez.",
          "Every FIFA tournament is evaluated within its own player, scoring, points and stage distribution. Semi-finals and finals carry higher weight, while team/opponent adjustments cannot overturn the actual result."
        )
      };
    }).sort((a,b)=>b.currentAbility-a.currentAbility||b.fifa9Performance-a.fifa9Performance||b.elo-a.elo);
  }

  function playerCardFormRibbon(results) {
    const list=(results||[]).slice(-10);
    return list.length?list.map(item=>`<span class="form-${String(item.result||"D").toLowerCase()}">${escapeHTML(item.result||"D")}</span>`).join(""):`<span class="player-card-form-empty">${intelligenceCopy("Maç bekleniyor","Awaiting matches")}</span>`;
  }

  function playerCardTimelineSvg(values) {
    const list=(values||[]).map(Number).filter(Number.isFinite);
    if(!list.length)return `<div class="intel-no-badge">${intelligenceCopy("Elo verisi yok","No Elo data")}</div>`;
  ...
  function renderPlayerCardSection() {
    ...
    const playedEditions=selected.editionProfiles.map(profile=>profile.edition);
    const activeEdition = Number(seasonSystem().activeEdition || 10);
    if(!playedEditions.includes(Number(intelligencePlayerEdition))) intelligencePlayerEdition=playedEditions.includes(activeEdition)?activeEdition:(playedEditions[playedEditions.length-1]||activeEdition);
    const editionProfile=selected.editionProfiles.find(profile=>profile.edition===Number(intelligencePlayerEdition))||selected.editionProfiles[selected.editionProfiles.length-1];
    const displayMetricValue=value=>value==null?'N/A':Math.round(Number(value)||0);
    const metricCard=category=>`<article class="fm30-metric-card group-${category.key}"><header><h4>${escapeHTML(category.label)}</h4><strong>${Math.round(Number(category.score)||0)}</strong><span>${escapeHTML(playerCardReferenceLabel(category.score))}</span></header><div>${category.attributes.map((item,index)=>`<div class="fm30-metric-row ${item.value==null?'is-na':''}"><em>${index+1}</em><label>${escapeHTML(item.label)}</label><i><b style="width:${item.value==null?0:item.value}%"></b></i><strong>${displayMetricValue(item.value)}</strong></div>`).join("")}</div></article>`;
    const summaryCards=[
      ["currentAbility",selected.currentAbility,"#d98aff"],
      ["fifaPerformance",editionProfile?.performance||25,"#65b8ff"],
      ["careerPerformance",selected.careerPerformance,"#d9dce2"],
      ["currentForm",selected.currentForm,"#ffc93f"],
      ["bigMatchPerformance",editionProfile?.bigMatchPerformance||35,"#68e2a5"],
      ["legacy",selected.legacy,"#ff9c79"]
    ];
    const stats=editionProfile?.stats||{matches:0,wins:0,draws:0,losses:0,winRate:0,ppg:0,avgGF:0,avgGA:0,gd:0};
    ...
    return `<div class="fm30-page fm32-page">
      <section class="fm30-toolbar"><div><div class="eyebrow">FIFA TOURNAMENT INTELLIGENCE · V35</div><h2>${intelligenceCopy("Turnuva İçi Gerçek Performans Merkezi","Tournament-Native Performance Centre")}</h2></div><div class="fm32-toolbar-controls"><select id="intelPlayerCardSelect">${cards.map(row=>`<option value="${escapeHTML(row.name)}" ${row.name===selected.name?"selected":""}>${escapeHTML(row.name)} · CA ${row.currentAbility} · F${activeEdition} ${row.fifa9Performance}</option>`).join("")}</select><select id="intelPlayerEditionSelect">${selected.editionProfiles.map(profile=>`<option value="${profile.edition}" ${profile.edition===Number(intelligencePlayerEdition)?"selected":""}>FIFA ${profile.edition} · ${profile.performance}/100</option>`).join("")}</select></div></section>
      ...
      <div class="fm30-layout">
        ...
        <main class="fm30-main-column">
          <section class="fm30-summary-grid">${summaryCards.map(([key,value,color])=>`<article style="--summary-color:${color}"><span>${key==='fifaPerformance'?`FIFA ${editionProfile?.edition||intelligencePlayerEdition} ${intelligenceCopy('PERFORMANSI','PERFORMANCE')}`:escapeHTML(playerCardMetricLabel(key).toUpperCase())}</span><strong>${Math.round(value)}<small>/100</small></strong><p>${escapeHTML(playerCardReferenceLabel(value))}</p></article>`).join("")}</section>
          <div class="fm30-confidence"><span>${intelligenceCopy("MODEL GÜVENİ","MODEL CONFIDENCE")}</span><strong>${selected.confidence>=85?intelligenceCopy("ÇOK YÜKSEK","VERY HIGH"):selected.confidence>=70?intelligenceCopy("YÜKSEK","HIGH"):intelligenceCopy("ORTA","MEDIUM")}</strong><b>%${selected.confidence}</b><small>${escapeHTML(selected.modelNote)}</small></div>
          <section class="fm30-criteria"><header><h3>FIFA ${editionProfile?.edition||intelligencePlayerEdition} · ${intelligenceCopy("Detaylı Kriterler","Detailed Criteria")}</h3><span>${intelligenceCopy("PUANLAMA: 0–100","RATING: 0–100")}</span></header><div class="fm30-criteria-grid">${(editionProfile?.categories||[]).map(metricCard).join("")}<article class="fm30-scale"><h4>${intelligenceCopy("Puan Aralıkları Rehberi","Rating Scale Guide")}</h4>${[[90,'Tarihî / Efsanevi'],[82,'Elite'],[74,'Çok Güçlü'],[66,'Güçlü'],[58,'Ortalama Üstü'],[50,'Ortalama'],[42,'Ortalama Altı'],[30,'Zayıf'],[0,'Çok Zayıf']].map(([value,label])=>`<p><b>${value}${value?' +':''}</b><span>${escapeHTML(intelligenceLanguage()==='en'?playerCardReferenceLabel(value):label)}</span></p>`).join("")}</article></div></section>
          <section class="fm32-edition-comparison"><header><h3>${intelligenceCopy("Edisyon Karşılaştırması","Edition Comparison")}</h3><span>${intelligenceCopy("Tüm katıldığı FIFA turnuvaları","All FIFA tournaments entered")}</span></header><div class="fm32-edition-table v33"><div class="head"><span>${intelligenceCopy("EDİSYON","EDITION")}</span><span>${intelligenceCopy("PERF.","PERF.")}</span><span>${intelligenceCopy("HÜCUM","ATTACK")}</span><span>${intelligenceCopy("SAVUNMA","DEFENCE")}</span><span>${intelligenceCopy("BÜYÜK MAÇ","BIG MATCH")}</span><span>${intelligenceCopy("MAÇ","MATCHES")}</span><span>${intelligenceCopy("G/MAÇ","GF/M")}</span><span>${intelligenceCopy("YG/MAÇ","GA/M")}</span></div>${selected.editionProfiles.map(profile=>`<button class="row ${profile.edition===Number(intelligencePlayerEdition)?'active':''}" data-action="set-player-edition" data-player-edition="${profile.edition}"><span>FIFA ${profile.edition}</span><strong>${profile.performance}</strong><span>${profile.attack}</span><span>${profile.defense}</span><span>${profile.bigMatchPerformance}</span><span>${profile.stats.matches}</span><span>${profile.stats.avgGF.toFixed(2)}</span><span>${profile.stats.avgGA.toFixed(2)}</span></button>`).join("")}</div></section>
        </main>
        <aside class="fm30-right-column">
          ...
          <article class="fm30-panel"><h4>${intelligenceCopy("Kariyer Zaman Çizelgesi (Elo)","Career Timeline (Elo)")}</h4>${playerCardTimelineSvg(selected.eloTimeline)}</article>
  ...
  function intelligencePlayerId(name) {
    ...
    return fifa10IntegratedParticipants().find(player => player.name.trim() === target)?.id
      || state.current.participants.find(player => player.name.trim() === target)?.id
      || "";
  }
  ...
  function buildPressureChamber() {
    ...
    const players = names.map(name => {
      ...
      records.forEach(record => {
        ...
        const currentWeight = Number(record.match.edition) === Number(seasonSystem().activeEdition || 10) ? .18 : 0;
  ...
  function reconcileAfterStageEdit(phase) {
    ...
    if (phase === "league" && state.current.phase2.generated) {
      ...
      if (!arraySame(gold,state.current.phase2.goldIds) || !arraySame(silver,state.current.phase2.silverIds)) {
        ...
        state.current.knockout = defaultState().current.knockout;
        state.current.finalChapter = defaultFinalChapter();
        return "League Phase sıralaması değiştiği için Altın/Gümüş ve Final Chapter hazırlığı sıfırlandı.";
      }
    }
    if (["league","gold","silver"].includes(phase) && state.current.knockout.generated && !finalChapterIsEnabled()) {
  ...
  function saveMatch(form) {
    ...
    if (!match) return;
    const lockReason = finalChapterEditLockReason(match);
    if (lockReason) { toast(lockReason, "error"); return; }
    const data = new FormData(form);
    ...
    const nextHomeTeam = String(data.get("homeTeam") || "").trim();
    const nextAwayTeam = String(data.get("awayTeam") || "").trim();
    const teamError = validateFinalChapterTeams(match, nextHomeTeam, nextAwayTeam);
    if (teamError) { toast(teamError, "error"); return; }
    const fifa10TeamError = validateFifa10MatchTeams(match, nextHomeTeam, nextAwayTeam);
    if (fifa10TeamError) { toast(fifa10TeamError, "error"); return; }
  ...
  function clearMatch(matchId) {
    ...
    if (match.phase === "fifa10-group") {
      ...
      syncFifa10ResultCompatibility(match, true);
    }
  ...
  async function shareSite() {
    const activeEdition = seasonSystem().activeEdition || 10;
    const shareData = {
      title: `FIFA ${activeEdition} Tournament Hub`,
      text: `FIFA ${activeEdition} canlı fikstür, sonuçlar ve genel PPG sıralaması`,
      url: window.location.href
    };
    if (navigator.share) {
      try { await navigator.share(shareData); return; } catch (error) { if (error?.name === "AbortError") return; }
...
${shareData.url}`)}`;
  ...
  function exportJSON() {
    ...
    download(`FIFA_Universe_Backup_${new Date().toISOString().slice(0,10)}.json`, JSON.stringify(payload, null, 2), "application/json;charset=utf-8");
    toast("FIFA Universe yedeği indirildi.", "success");
  }

  function fifa9CurrentMatches() {
    return [...leagueMatches(), ...goldMatches(), ...silverMatches(), ...allKnockoutGames()];
  }

  function fifa10CurrentMatches() {
    const integration = ensureFifa10IntegratedTournament();
    return (integration.draw?.fixtures || []).map(match => {
      match.edition = 10;
      match.phase = "fifa10-group";
  ...
  function renderFinalPollPage() {
    ...
    <section class="fc-format-journey" aria-label="Final Chapter turnuva akışı">
      ...
      <article class="fc-stage playoff"><span>02</span><div><small>ANA ELEME</small><h3>8 Oyuncu · 4 × BO3</h3><p>Sultan oynamaz · şanslı tur yok</p></div></article>
      <div class="fc-journey-arrow">›</div>
      <article class="fc-stage playoff"><span>03</span><div><small>SON BİLET PLAY-OFF</small><h3>4 Kaybeden</h3><p>2 yarı final + final · tamamı BO3</p></div></article>
      <div class="fc-journey-arrow">›</div>
      <article class="fc-stage quarter"><span>04</span><div><small>ÇEYREK FİNAL</small><h3>8 Oyuncu · BO3</h3><p>4.5★ kura · takım tekrarı yok</p></div></article>
      <div class="fc-journey-arrow">›</div>
  ...
  document.addEventListener("click", event => {
    ...
    if (type === "activate-final-chapter") { activateFinalChapter(); return; }
    if (type === "fc-cancel-format") { cancelFinalChapter(); return; }
    if (type === "fc-set-draw-policy") { setFinalChapterDrawPolicy(action.dataset.policy); return; }
    if (type === "fc-auto-draw-stage" || type === "fc-draw-stage") { drawFinalChapterStage(action.dataset.stage); return; }
    if (type === "fc-open-manual-draw") { openFinalChapterManualDraw(action.dataset.stage); return; }
    if (type === "fc-lock-stage") { lockFinalChapterStage(action.dataset.stage); return; }
    if (type === "fc-draw-teams") { drawFinalChapterGameTeams(action.dataset.matchId); return; }
    if (type === "open-fc-team-pools") { openFinalChapterTeamPools(); return; }
    ...
    if (type === "remove-fifa10-player") { removeFifa10DraftPlayer(action.dataset.playerId); return; }
    if (type === "open-honour-editor") { openHonourEditor(action.dataset.recordId || ""); return; }
    ...
    if (type === "set-player-edition") {
      const edition = Number(action.dataset.playerEdition);
      if (Number.isFinite(edition)) intelligencePlayerEdition = edition;
      if (activeView === "intelligence") renderIntelligenceCentre();
      return;
    }
    ...
    if (type === "set-tournament-benchmark-metric") {
      tournamentBenchmarkMetric = ["benchmark","ppg","clutch","consistency","era"].includes(action.dataset.benchmarkMetric) ? action.dataset.benchmarkMetric : "benchmark";
      if (activeView === "benchmark") renderTournamentBenchmark();
      return;
    }
    if (type === "select-tournament-benchmark-player") {
      tournamentBenchmarkSelectedPlayer = action.dataset.playerName || tournamentBenchmarkSelectedPlayer;
      if (activeView === "benchmark") renderTournamentBenchmark();
      return;
    }
    ...
    if (type === "reset-current") { const preservedSeasonSystem = seasonSystem(); state = defaultState(); state.seasonSystem = preservedSeasonSystem; saveState(); closeModal(); toast("Güncel FIFA 09 turnuvası sıfırlandı; Kupa Müzesi ve FIFA 10 taslağı korundu.", "success"); navTo("setup"); }
  });
  ...
  document.addEventListener("submit", event => {
    ...
    if (event.target.id === "finalChapterPoolsForm") { event.preventDefault(); saveFinalChapterTeamPools(event.target); return; }
    if (event.target.id === "finalChapterManualDrawForm") { event.preventDefault(); saveFinalChapterManualDraw(event.target); return; }
  ...
  document.addEventListener("change", event => {
    ...
    if (event.target.id === "intelPlayerEditionSelect") {
      intelligencePlayerEdition = Number(event.target.value) || intelligencePlayerEdition;
      if (activeView === "intelligence") renderIntelligenceCentre();
      return;
    }
    ...
    if (event.target.id === "tournamentBenchmarkPlayerSelect") { tournamentBenchmarkSelectedPlayer = event.target.value; if (activeView === "benchmark") renderTournamentBenchmark(); return; }
  ...
  window.FIFA_APP_CONTEXT = {
    ...
    buildLiveTournamentAnalytics: (scope = "all") => buildLiveTournamentAnalytics(scope)
  };
  ...
  window.FIFA_FINAL_CHAPTER = {
    ...
    activate: activateFinalChapter,
    drawStage: drawFinalChapterStage,
    lockStage: lockFinalChapterStage,
  ...
  async function initializeCloud() {
    ...
    if (state.fifa10StandaloneOperations?.transferredFrom) {
      toast("Acil sonuç ekranındaki FIFA 10 skorları ana turnuva sistemine aktarıldı.", "success");
    }
  }
