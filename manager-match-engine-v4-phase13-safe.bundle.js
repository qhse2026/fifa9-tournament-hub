...
(() => {
  ...
  function clearCatchupTimer() {
    if (runtime.catchupTimer) globalThis.clearTimeout?.(runtime.catchupTimer);
    runtime.catchupTimer = null;
    runtime.catchupBusy = false;
  }

  function scheduleCatchup(targetSecond) {
    if (!runtime.engine) return;
    runtime.pendingTargetSecond = Math.max(
      Number(runtime.pendingTargetSecond || 0),
      Math.max(0, Number(targetSecond || 0))
    );
    if (runtime.catchupBusy) return;

    runtime.catchupBusy = true;
    const work = () => {
      runtime.catchupTimer = null;
      if (!runtime.enabled || !runtime.engine) {
        runtime.catchupBusy = false;
        return;
      }
      ...
      try {
        while (
          runtime.engine &&
          runtime.engine.status === "live" &&
          runtime.engine.clockSeconds + 0.01 < runtime.pendingTargetSecond &&
          steps < 4
        ) {
          const remaining = runtime.pendingTargetSecond - runtime.engine.clockSeconds;
          runtime.engine.step(Math.min(1, remaining));
          steps += 1;

          const elapsed = (globalThis.performance?.now?.() ?? Date.now()) - started;
          if (elapsed >= 8) break;
        }
        runtime.catchupSlices += 1;
        runtime.lastCatchupError = null;
      } catch (error) {
        runtime.lastCatchupError = error?.message || String(error);
        console.error("[ME4 SAFE] Catch-up failed", error);
      }
      ...
      if (
        ...
        runtime.catchupTimer = globalThis.setTimeout?.(work, 0);
      } else {
        runtime.catchupBusy = false;
      }
    };
  ...
  function attach(career, fixture, targetSecond = 0) {
    ...
    runtime.pendingTargetSecond = second;
    runtime.lastLegacySecond = second;
    runtime.engine.setBallAuthority(false, { source: "PHASE13_SAFE_FULL_AUTHORITY", nextDecisionIn: 0.8 });

    if (second > 0.25) scheduleCatchup(second);
    else applyV4OfficialState(fixture);
  ...
  function poll() {
    ...
    if (!runtime.engine || runtime.fixtureId !== fixture.id || legacySecond < runtime.lastLegacySecond) {
      clearCatchupTimer();
      attach(career, fixture, legacySecond);
    } else {
      runtime.lastLegacySecond = legacySecond;
      if (legacySecond > runtime.engine.clockSeconds + 0.01) scheduleCatchup(legacySecond);
    }

    const lagSeconds = runtime.engine
      ? Math.max(0, legacySecond - Number(runtime.engine.clockSeconds || 0))
      : 0;

    // Never overwrite the official screen while V4 is materially behind.
    if (runtime.engine && lagSeconds <= 1.25) {
      ...
      runtime.diagnostics = {
        fixtureId: fixture.id,
        authority: "V4_SAFE_CATCHUP",
        legacySecond,
        v4Second: Number(runtime.engine?.clockSeconds || 0),
        lagSeconds: Math.round(lagSeconds * 100) / 100,
        catchupBusy: runtime.catchupBusy,
  ...
  function getSnapshot() {
    return {
      ...
      safeRuntime: {
        pendingTargetSecond: runtime.pendingTargetSecond,
        catchupBusy: runtime.catchupBusy,
        catchupSlices: runtime.catchupSlices,
...
(() => {
  ...
  function persistAnalysis() {
    ...
    context.fixture.matchEngine.matchAnalysis = {
      ...
      momentum: report.momentum.slice(-24),
      shotMap: report.shotMap.slice(-80),
      passNetwork: report.passNetwork.slice(0, 50),
      averagePositions: report.averagePositions,
      timeline: report.timeline.slice(-120),
  ...
  function enable() {
    LIVE.enabled = true;
    if (!LIVE.timer) LIVE.timer = globalThis.setInterval?.(poll, 1500);
    startObserver();
    poll();
    return getStatus();
  }
...
(() => {
  ...
  const VERSION = "4.0.0-phase13-safe.1";
  ...
  function ensureBadge(){ if(typeof document==="undefined")return; const hud=document.querySelector("#managerMatchMount .me11-hud"); if(!hud||hud.querySelector("[data-me13-badge]"))return; const badge=document.createElement("span"); badge.dataset.me13Badge=""; badge.textContent="ME13 · SAFE RUNTIME"; badge.style.cssText="padding:5px 8px;border-radius:7px;background:#2a513c;color:#dff7df;font:900 9px Inter,system-ui;border:1px solid rgba(170,240,180,.28)"; hud.appendChild(badge); }
  function persist(){ const ctx=activeContext(); if(!ctx)return; const now=Date.now(); if(now-LIVE.lastPersistAt<5000)return; LIVE.lastPersistAt=now; ctx.fixture.matchEngine.matchEngineV4=ctx.fixture.matchEngine.matchEngineV4||{}; Object.assign(ctx.fixture.matchEngine.matchEngineV4,{balanceOfficial:true,balanceVersion:VERSION,balanceManifest:PROFILE_VERSION,balanceProfile:"masterpiece"}); }
