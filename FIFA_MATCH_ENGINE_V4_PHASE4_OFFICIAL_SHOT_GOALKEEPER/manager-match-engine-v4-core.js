(() => {
  ...
  function goalkeeperShotDecision(engine, goalkeeper, shooter, context) {
    ...
    const goalProbability = clamp(context.xgot * keeperQualityFactor * 0.72 + difficultyBoost + typeBoost + sightPenalty * 0.018, 0.018, 0.62);
    const saveProbability = clamp(1 - goalProbability, 0.18, 0.975);
    const holdProbability = clamp(0.14 + profile.handling / 118 + goalkeeper.attributes.composure / 520 - context.shotPower / 170 - context.xgot * 0.20, 0.14, 0.88);
  ...
  function computeShotPlan(engine, shooter, source = "OPEN_PLAY", debug = {}) {
    ...
    const firstTime = Boolean(debug.firstTime || source === "CROSS" || source === "CORNER");
    const isHeader = Boolean(debug.isHeader || source === "CORNER");
    ...
    const sourceFactor = source === "CORNER" ? 0.70 : source === "CROSS" ? 0.84 : 1;
    ...
    const context = { start, goalPoint, targetY, targetZ, distance: distanceValue, pressure, opportunity, shotType: type, shotPower, duration, xg, xgot, sightObstruction, firstTime, isHeader };
    let blocker = shotBlockCandidate(engine, shooter, start, goalPoint, duration);
    if (!blocker) {
  ...
  function executeDecision(engine) {
    if (engine.restart || engine.activeBallAction || engine.activeShotAction || engine.looseBallState || engine.ballAuthoritySuspended) return;
  ...
  function stepEngine(engine, deltaSeconds) {
    ...
    while (engine.accumulator >= FIXED_STEP && engine.status === "live") {
      ...
      if (!engine.restart && !engine.activeBallAction && !engine.activeShotAction && !engine.looseBallState && !engine.ballAuthoritySuspended && engine.nextDecisionIn <= 0) executeDecision(engine);
