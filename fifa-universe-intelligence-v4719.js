(() => {
  ...
  function officialMatches(payload, draw) {
    ...
    const currentPlayers = new Map((draw?.participants || []).map(player => [String(player.id), player.name]));
    const championshipState = payload?.seasonSystem?.fifa10Draft?.championshipOS;
    Object.values(championshipState?.rounds || {}).flat().forEach(series => {
      (series.matches || []).filter(match => match.completed && !match.notRequired).forEach(match => {
        source.push({
          id: `F10-KO-${match.id}`,
          edition: 10,
          editionLabel: "FIFA 10",
          stage: `FIFA 10 · ${String(series.round || "Championship").toUpperCase()} · ${series.label || series.id} · ${match.stars || "–"}★`,
          homeName: currentPlayers.get(String(series.homeId)) || "",
          awayName: currentPlayers.get(String(series.awayId)) || "",
          homeTeam: match.homeTeam || "",
          awayTeam: match.awayTeam || "",
          homeScore: Number(match.homeScore),
          awayScore: Number(match.awayScore),
          stars: Number(match.stars),
          winnerName: Number(match.homeScore) > Number(match.awayScore)
            ? currentPlayers.get(String(series.homeId)) || ""
            : currentPlayers.get(String(series.awayId)) || ""
        });
      });
    });
    const seen = new Set();
  ...
  function renderPlayers(data) {
    ...
    return `<section class="fui-section" data-panel="players">
      ...
      <p class="fui-method">${ui("PPR resmî Standing Rating’in yerine geçmez. Rakip gücü, takımın Bayes-düzeltilmiş performansı, GD/M ve beklenen sonuca göre performansı birleştiren analitik göstergedir. Küçük örneklemler 50 merkezine daraltılır.", "PPR does not replace official Standing Rating. It is an analytical indicator combining opposition strength, Bayesian-adjusted team performance, GD/M and performance versus expectation. Small samples are shrunk toward 50.")}</p>
    </section>`;
  }
  ...
  function downloadMedia() {
    ...
    anchor.href = url;
    anchor.download = `FIFA_UNIVERSE_${String(story?.type || "MEDIA").toUpperCase()}_V4719.svg`;
